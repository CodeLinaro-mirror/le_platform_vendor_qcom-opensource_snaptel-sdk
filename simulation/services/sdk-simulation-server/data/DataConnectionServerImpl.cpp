...
grpc::Status DataConnectionServerImpl::InitService(ServerContext *context,
    ...
    std::string cbStatus = rootObj["IDataConnectionManager"]["IsSubsystemReady"].asString();
...
grpc::Status DataConnectionServerImpl::SetDefaultProfile(ServerContext *context,
    ...
    if (data.status == telux::common::Status::SUCCESS) {
        LOG(DEBUG, __FUNCTION__, " updated json with profileId:", request->profile_id());
...
grpc::Status DataConnectionServerImpl::GetDefaultProfile(ServerContext *context,
    const dataStub::GetDefaultProfileRequest *request, dataStub::GetDefaultProfileReply *response) {
    ...
    if (data.status == telux::common::Status::SUCCESS) {
        profileId
            = data.stateRootObj["IDataConnectionManager"]["getDefaultProfile"]["profileId"].asInt();
        slotID = static_cast<SlotId>(
            data.stateRootObj["IDataConnectionManager"]["getDefaultProfile"]["slotId"].asInt());
    }
...
grpc::Status DataConnectionServerImpl::SetRoamingMode(ServerContext *context,
    const dataStub::SetRoamingModeRequest *request, dataStub::DefaultReply *response) {
    ...
    if (data.status == telux::common::Status::SUCCESS) {
        LOG(DEBUG, __FUNCTION__, " updated json with roaming_mode:", request->roaming_mode());
    ...
    if (data.status == telux::common::Status::SUCCESS) {
        roamingMode
            = data.stateRootObj["IDataConnectionManager"]["requestRoamingMode"]["isRoamingEnabled"]
                  .asBool();
        profileId
            = data.stateRootObj["IDataConnectionManager"]["requestRoamingMode"]["profileId"].asInt();
    }
...
void DataConnectionServerImpl::getInactiveInterfaces() {
    LOG(DEBUG, __FUNCTION__);
    std::shared_ptr<SimulationConfigParser> config = std::make_shared<SimulationConfigParser>();
    ...
    while (getline(iss, parsedVal, DELIMITER)) {
        if (std::find(activeNwIfaces_.begin(), activeNwIfaces_.end(), parsedVal)
            == activeNwIfaces_.end()) {
            parsedVal.erase(
                remove_if(parsedVal.begin(), parsedVal.end(), ::isspace), parsedVal.end());
            inactiveNwIfaces_.push_back(parsedVal);
        }
    }
}

bool DataConnectionServerImpl::getIpv4Address(const std::string &ifaceName, std::string &ipAddress,
    std::string &gatewayAddress, std::string &dnsPrimaryAddress, std::string &dnsSecondaryAddress,
    uint16_t &mtuValue) {
    LOG(DEBUG, __FUNCTION__);
            ...
            if (ifaceName == ifName) {
                ...
                for (auto idx = 0; idx < addr.nscount; idx++) {
                    if (addr.nsaddr_list[idx].sin_family == AF_INET) {
                        inet_ntop(AF_INET, &addr.nsaddr_list[idx].sin_addr.s_addr, addressStr,
                            INET_ADDRSTRLEN);
...
bool DataConnectionServerImpl::getIpv6Address(const std::string &ifaceName, std::string &ipAddress,
    std::string &gatewayAddress, std::string &dnsPrimaryAddress, std::string &dnsSecondaryAddress,
    uint16_t &mtuValue) {
    LOG(DEBUG, __FUNCTION__);
    bool ifaceFound = false;
    ...
    for (ifaddr = ifaceAddresses; ifaddr != NULL; ifaddr = ifaddr->ifa_next) {
        if ((ifaddr->ifa_addr != NULL) && (ifaddr->ifa_addr->sa_family == AF_INET6)) {
            ...
            if (ifaceName == currentIfName) {
                ...
                if (inet_ntop(AF_INET6, &ipAddr->sin6_addr, ipAddrStr, INET6_ADDRSTRLEN) == NULL) {
                    LOG(DEBUG, __FUNCTION__, " Failed to convert IPv6 address: %s",
                        strerror(errno));
                    continue;
                }
                ipAddress = ipAddrStr;
                ...
                std::string command = "ip -6 route | grep 'default[ \t]' | awk '{print $3}'";
                FILE *fp            = popen(command.c_str(), "r");
                ...
                if (fp) {
                    ...
                    LOG(DEBUG, __FUNCTION__, " popen failed for gateway command: %s",
                        strerror(errno));
                }
...
void DataConnectionServerImpl::triggerStartDataCallEvent(int profileId, int slotId,
    ...
    if ((slotId == SLOT_ID_1) && (dataCallsSlot1_.find(profileId) != dataCallsSlot1_.end())) {
    ...
    if (ipFamilyType == DataUtilsStub::convertIpFamilyEnumToString(::dataStub::IpFamilyType::IPV4)
        || ipFamilyType
    ...
    if (ipFamilyType == DataUtilsStub::convertIpFamilyEnumToString(::dataStub::IpFamilyType::IPV6)
        || ipFamilyType
               == DataUtilsStub::convertIpFamilyEnumToString(::dataStub::IpFamilyType::IPV4V6)) {
        ...
        if ((!dataCall) || (ipFamilyMismatch)) {
            auto f = std::async(std::launch::deferred,
                [this, profileId, slotId, ipFamilyType, data, client_id, ifaceName]() {
                    ...
                    this->triggerStartDataCallEvent(
                        profileId, slotId, ipFamilyType, client_id, ifaceName);
                }).share();
            taskQ_->add(f);
        }
    }
...
void DataConnectionServerImpl::triggerStopDataCallEvent(
...
grpc::Status DataConnectionServerImpl::StopDatacall(ServerContext *context,
    ...
    if (data.status == telux::common::Status::SUCCESS
        ...
        if (dataCall) {
            LOG(DEBUG, __FUNCTION__, " datacall ref_count::", dataCall->ownersId.size());
            ...
            auto f                = std::async(
                std::launch::async, [this, profileId, slotId, ipFamilyType, ifaceName, data]() {
                    std::this_thread::sleep_for(std::chrono::milliseconds(data.cbDelay));
...
void DataConnectionServerImpl::clearCachedDataCall(
    ...
    for (auto itr = dataCallsMap.begin(); itr != dataCallsMap.end();) {
        ...
        if (stopAllCalls) {
            this->triggerStopDataCallEvent(
                profileId, callObj->slotId, callObj->ipFamilyType, callObj->ifaceName);
            ++itr;
        } else {
...
bool DataConnectionServerImpl::updateThrottleApnInfo() {
    ...
    if (data.status == telux::common::Status::SUCCESS) {
        ...
        for (auto idx = 0; idx < size; idx++) {
            ...
            apnThrottleInfo.apn
                = data.stateRootObj[subsystem][method]["apnThrottleInfo"][idx]["apnName"].asString();
            int profile_id_size
                = data.stateRootObj[subsystem][method]["apnThrottleInfo"][idx]["profileIds"].size();
            for (auto pidx = 0; pidx < profile_id_size; pidx++) {
                apnThrottleInfo.profileIds.emplace_back(
                    data.stateRootObj[subsystem][method]["apnThrottleInfo"][idx]["profileIds"][pidx]
                        .asInt());
            }
            apnThrottleInfo.ipv4Time
                = data.stateRootObj[subsystem][method]["apnThrottleInfo"][idx]["ipv4time"].asInt();
            apnThrottleInfo.ipv6Time
                = data.stateRootObj[subsystem][method]["apnThrottleInfo"][idx]["ipv6time"].asInt();
            apnThrottleInfo.isBlocked
                = data.stateRootObj[subsystem][method]["apnThrottleInfo"][idx]["isBlocked"].asBool();
            apnThrottleInfo.mcc
                = data.stateRootObj[subsystem][method]["apnThrottleInfo"][idx]["mcc"].asString();
            apnThrottleInfo.mnc
                = data.stateRootObj[subsystem][method]["apnThrottleInfo"][idx]["mnc"].asString();
...
grpc::Status DataConnectionServerImpl::RequestThrottledApnInfo(ServerContext *context,
    ...
    if (data.status == telux::common::Status::SUCCESS) {
        ...
        for (auto itr = throttleInfo.begin(); itr != throttleInfo.end(); itr++) {
            dataStub::APNThrottleInfo *call
                = response->mutable_apn_throttle_info_list()->add_rep_apn_throttle_info();
...
void DataConnectionServerImpl::triggerThrottledApnInfoChangedEvent(
    ...
    LOG(DEBUG, __FUNCTION__,
        " Throttled APN info list size: ", response->rep_apn_throttle_info_size());
