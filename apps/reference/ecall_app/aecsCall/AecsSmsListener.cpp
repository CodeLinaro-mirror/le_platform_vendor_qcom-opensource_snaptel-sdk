...
void AecsSmsListener::retryMsdOverSms(int phoneId) {
    ...
    std::cout << "Starting MSD-over-SMS retry: interval="
        << (msdRetryIntervalSec_ / SEC_PER_MIN) << " min, duration="
        << (msdRetryDurationSec_ / SEC_PER_MIN) << " min..." << std::endl;
    ...
    {
        std::lock_guard<std::mutex> lk(msdRetryMutex_);
        msdNextRetryAt_ = std::chrono::steady_clock::now()
            + std::chrono::seconds(msdRetryIntervalSec_);
        msdRetryScheduled_ = true;
    }
    ...
    msdRetryThread_ = std::thread([this] {
        while (!msdRetryStop_.load()) {
            ...
            msdRetryCv_.wait(lk, [this]{
                return msdRetryStop_.load() || msdRetryScheduled_;
            });
            if (msdRetryStop_.load()) break;
