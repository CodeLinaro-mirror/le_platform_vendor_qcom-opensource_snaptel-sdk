...
bool LocationManagerServerImpl::init() {
    ...
    if (!fileExists(filePath)) {
        filePath = std::string(DEFAULT_SIM_FILE_PREFIX) + std::string(DEFAULT_SIM_CSV_FILE_PATH)
                   + fileName;
        if (!fileExists(filePath)) {
...
grpc::Status LocationManagerServerImpl::InitService(ServerContext *context,
    ...
    Json::Value rootNode;
    telux::common::ErrorCode errorCode = JsonParser::readFromJsonFile(rootNode, LOC_MGR_API_JSON);
...
void LocationManagerServerImpl::updateStreamRequest() {
    ...
    if (bufferingInitialized_) {
        ...
        if (clientSize == 1) {
            ...
            // Starting the stream.
            auto f = std::async(std::launch::deferred, [=]() { this->startStreaming(); }).share();
            taskQ_.add(f);
        }
    }
}
...
grpc::Status LocationManagerServerImpl::RegisterLocationSystemInfo(ServerContext *context,
    ...
    auto f
        = std::async(std::launch::async, [this]() { this->triggerSysinfoUpdateEvent(); }).share();
    taskQ_.add(f);
    return grpc::Status::OK;
}
...
grpc::Status LocationManagerServerImpl::RequestEnergyConsumedInfo(ServerContext *context,
    ...
    CommonUtils::getValues(
        rootNode, "ILocationManager", "requestEnergyConsumedInfo", status, errorCode, cbDelay);
    response->set_status(static_cast<::commonStub::Status>(status));
    response->set_error(static_cast<::commonStub::ErrorCode>(errorCode));
    response->set_delay(cbDelay);
    if (errorCode == ErrorCode::SUCCESS) {
        uint32_t validity = std::stoi(telux::common::CommonUtils::readSystemDataValue(
            "loc/ILocationManager", "0", {"ILocationManager", "GnssEnergyConsumedInfo", "valid"}));

        uint32_t energyConsumed
            = std::stoi(telux::common::CommonUtils::readSystemDataValue("loc/ILocationManager", "0",
...
grpc::Status LocationManagerServerImpl::GetYearOfHw(ServerContext *context,
    ...
    telux::common::ErrorCode errorCode;
    int cbDelay;
    CommonUtils::getValues(rootNode, "ILocationManager", "getYearOfHw", status, errorCode, cbDelay);
    response->set_status(static_cast<::commonStub::Status>(status));
    response->set_error(static_cast<::commonStub::ErrorCode>(errorCode));
    response->set_delay(cbDelay);
    if (errorCode == ErrorCode::SUCCESS) {
        ...
        if (yearOfHw == 0) {
            ...
            CommonUtils::writeSystemDataValue<string>(
                "loc/ILocationManager", std::to_string(yearOfHw), {"ILocationManager", "yearOfHw"});
        }
...
void LocationManagerServerImpl::handleDisasterCrisisReport(std::string event) {
    ...
    Json::Value rootNode;
    telux::common::ErrorCode errorCode = JsonParser::readFromJsonFile(rootNode, event);
    if (errorCode == ErrorCode::SUCCESS) {
        ...
        ::locStub::GnssReportDCType type = static_cast<::locStub::GnssReportDCType>(dctype);

        auto f = std::async(std::launch::async, [this, type, numValidBits, prnValid, prn, data]() {
