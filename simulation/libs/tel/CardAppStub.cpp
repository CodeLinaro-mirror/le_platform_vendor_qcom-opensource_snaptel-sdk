...
telux::common::Status CardAppStub::changeCardPassword(CardLockType lockType, std::string oldPwd,
    ...
    if (lockType == CardLockType::PIN1 || lockType == CardLockType::PIN2) {
        ...
        if ((status == telux::common::Status::SUCCESS) && (isCallbackNeeded)) {
            auto f1 = std::async(std::launch::async, [this, callback, error, retrycount, delay]() {
...
void CardAppStub::invokeCallback(
    PinOperationResponseCb callback, telux::common::ErrorCode error, int retryCount, int delay) {
    std::this_thread::sleep_for(std::chrono::milliseconds(delay));
    auto f1 = std::async(std::launch::async, [this, error, retryCount, callback]() {
...
telux::common::Status CardAppStub::unlockCardByPuk(
...
telux::common::Status CardAppStub::unlockCardByPin(
...
void CardAppStub::invokeCallback(
    ...
    std::this_thread::sleep_for(std::chrono::milliseconds(delay));
    auto f1 = std::async(std::launch::async, [this, error, state, callback]() {
...
telux::common::Status CardAppStub::queryPin1LockState(QueryPin1LockResponseCb callback) {
    ...
    if ((status == telux::common::Status::SUCCESS) && (isCallbackNeeded)) {
        auto f1 = std::async(std::launch::async, [this, callback, error, delay, state]() {
...
void CardAppStub::invokeCallback(bool isavailable, bool isenabled, QueryFdnLockResponseCb callback,
    telux::common::ErrorCode error, int delay) {
    std::this_thread::sleep_for(std::chrono::milliseconds(delay));
    auto f = std::async(std::launch::async, [this, isavailable, isenabled, error, callback]() {
...
telux::common::Status CardAppStub::queryFdnLockState(QueryFdnLockResponseCb callback) {
    ...
    if ((status == telux::common::Status::SUCCESS) && (isCallbackNeeded)) {
        auto f = std::async(
            std::launch::async, [this, isavailable, isenabled, callback, error, delay]() {
...
telux::common::Status CardAppStub::setCardLock(
    CardLockType lockType, std::string password, bool isEnabled, PinOperationResponseCb callback) {
    if (password.empty()) {
        return telux::common::Status::FAILED;
    }
    std::string aid = getAppId();
    ...
    if ((status == telux::common::Status::SUCCESS) && (isCallbackNeeded)) {
        bool IsCardInfoChanged = static_cast<int>(response.iscardinfochanged());
        auto f = std::async(std::launch::async, [this, error, retrycount, callback, delay]() {
...
void CardAppStub::invokeCallback(
    ...
    std::this_thread::sleep_for(std::chrono::milliseconds(delay));
    auto f = std::async(std::launch::async, [this, error, retrycount, callback]() {
        callback(retrycount, error);
    }).share();
    taskQ_->add(f);
}
...
bool CardAppStub::match(CardAppStatus &cardAppStatus) {
    LOG(DEBUG, "Card App Status: appType = ", static_cast<int>(cardAppStatus.appType),
        ", appState = ", static_cast<int>(cardAppStatus.appState), ", aid = ", (cardAppStatus.aid));
...
telux::common::Status CardAppStub::updateCardApp(CardAppStatus &cardAppStatus) {
    ...
    if (!isCardAppChanged) {
        ...
        LOG(DEBUG, "Previous Card State: ", cardAppStatus_.appState,
            " Current Card State: ", cardAppStatus.appState);
