...
void SensorFeatureManagerServerImpl::triggerFeatureEvent(
    std::string featureName, int id, std::string eventString) {
    LOG(DEBUG, __FUNCTION__);
...
std::string SensorFeatureManagerServerImpl::readBufferedEventStringFromFile(
    std::string fileName, int eventId) {
    LOG(DEBUG, __FUNCTION__);
    ...
    if (!fileExists(csvFilePath)) {
        csvFilePath
            = std::string(DEFAULT_SIM_FILE_PREFIX) + std::string(DEFAULT_SIM_CSV_FILE_PATH) + file;
        if (!fileExists(csvFilePath)) {
...
grpc::Status SensorFeatureManagerServerImpl::GetFeatureList(ServerContext *context,
    ...
    if (errorCode == ErrorCode::SUCCESS) {
        ...
        std::string cbStatus
            = rootNode["ISensorFeatureManager"]["getAvailableFeatures"]["status"].asString();
