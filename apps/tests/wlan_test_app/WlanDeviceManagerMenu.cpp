    ...
    if (menuOptionsAdded_ == false) {
        ...
        std::shared_ptr<ConsoleAppCommand> getRegulatoryParams
            = std::make_shared<ConsoleAppCommand>(
                ConsoleAppCommand("6", "get_regulatory_params", {},
                    std::bind(
                        &WlanDeviceManagerMenu::getRegulatoryParams, this, std::placeholders::_1)));
    ...
    if (retCode == telux::common::ErrorCode::SUCCESS) {
        ...
        if (status.size() > 0) {
            for (auto &ifStatus : status) {
                std::cout << "------------------------------------------" << std::endl;
                std::cout << "device: " << WlanUtils::getWlanDeviceName(ifStatus.device)
                          << std::endl;
...
void WlanDeviceManagerMenu::setTxPower(std::vector<std::string> userInput) {
    ...
    telux::common::ErrorCode retCode
        = wlanDeviceManager_->setTxPower(static_cast<uint32_t>(txPower));
