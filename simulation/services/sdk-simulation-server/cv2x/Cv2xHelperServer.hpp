...
class Cv2xServerUtil {
 public:
    ...
    static telux::common::ErrorCode stateJasonRead(std::string stateCfgFile, Json::Value &data);
    template <typename T>
    static void apiJsonReader(std::string cfg, std::string subsys, std::string apiName, T *res) {
        Json::Value rootNode;
        ...
        if (telux::common::ErrorCode::SUCCESS == err) {
            telux::common::CommonUtils::getValues(rootNode, subsys, apiName, status, err, cbDelay);
...
class Cv2xServerEvtListener final : public IServerEventListener {
 public:
    ...
    telux::common::Status registerListener(std::weak_ptr<telux::cv2x::ICv2xListener> l);
    telux::common::Status deregisterListener(std::weak_ptr<telux::cv2x::ICv2xListener> l);
