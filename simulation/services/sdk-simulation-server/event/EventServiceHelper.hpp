    ...
    void eventWriter(typename eventService::EventResponse eventResponse) {
        std::lock_guard<std::mutex> lck(clientMtx_);
        {
            for (auto &client : clients_) {
                ...
                if (find(clientObj.filters.begin(), clientObj.filters.end(), eventResponse.filter())
                    != clientObj.filters.end()) {
                    if (clientObj.clientWriter) {
                        LOG(DEBUG, __FUNCTION__, ":: writing for filter:", eventResponse.filter(),
 ...
 public:
    ...
    grpc::Status InjectEvent(ServerContext *context,
        ...
        auto f = std::async(std::launch::deferred, [this, msg]() {
            ...
            serverEventManager.handleEventNotifications(msg);
        }).share();
        taskQ_.add(f);
    ...
    grpc::Status isServiceAvailable(ServerContext *context, const google::protobuf::Empty *request,
        google::protobuf::Empty *response) override {
        LOG(DEBUG, __FUNCTION__);
