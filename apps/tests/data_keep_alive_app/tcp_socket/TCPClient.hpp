...
template <typename T> class TCPClientWorker {
...
public:
  TCPClient(std::shared_ptr<TCPClientWorker<T>> worker, int serverPort,
            std::string serverIpAddr, int clientPort = 0,
            std::string clientIpAddr = "", std::string clientInterface = "")
      : worker_(worker), serverPort_(serverPort), strServerAddr_(serverIpAddr),
        clientPort_(clientPort), strClientAddr_(clientIpAddr),
  ...
  bool connectTo() {
    ...
    if (!strClientAddr_.empty()) {
      if (inet_pton(AF_INET, strClientAddr_.c_str(),
                    &(v4ClientAddr_.sin_addr))) {
      ...
      } else if (inet_pton(AF_INET6, strClientAddr_.c_str(),
                           &(v6ClientAddr_.sin6_addr))) {
        ...
        std::cout << " Unexpected ip " << strClientAddr_;
        std::lock_guard<std::mutex> lock(clientExitMutex_);
        clientExited_ = true;
    ...
    if (inet_pton(AF_INET, strServerAddr_.c_str(), &(v4ServerAddr_.sin_addr))) {
      ...
      sockSize = sizeof(sockaddr_in);
    } else if (inet_pton(AF_INET6, strServerAddr_.c_str(),
                         &(v6ServerAddr_.sin6_addr))) {
      domain = AF_INET6;
      ...
      sockSize = sizeof(sockaddr_in6);
        ...
        std::cout << " Unexpected ip " << strServerAddr_;
        std::lock_guard<std::mutex> lock(clientExitMutex_);
        clientExited_ = true;
        clientExitCondition_.notify_one();
      return false;
    ...
    while (true) {
      ...
      int retval = select(std::max(socket_, exitPipe_[0]) + 1, &read_fds, NULL,
                          NULL, NULL);
      if (retval == -1) {
  ...
  void disconnect() {
    ...
    // Check if thread is actually running before signaling
    bool shouldWait = false;
    {
      std::lock_guard<std::mutex> lock(clientExitMutex_);
      shouldWait = !clientExited_;
    }
    ...
    {
      std::unique_lock<std::mutex> lock(clientExitMutex_);
      // Close exit pipe file descriptors atomically
      int pipe0 = exitPipe_[0];
      int pipe1 = exitPipe_[1];
      exitPipe_[0] = -1;
      exitPipe_[1] = -1;

      if (pipe0 != -1) close(pipe0);
      if (pipe1 != -1) close(pipe1);
    }
  }

  void sendMessage(T *msg) {
    if (socket_ != -1) {
      if (send(socket_, static_cast<const void *>(msg), sizeof(T), 0) !=
          sizeof(T)) {
