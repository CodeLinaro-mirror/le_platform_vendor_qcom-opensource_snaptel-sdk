...
void CallManagerServerImpl::getJsonForSystemData(
    int phoneId, std::string &jsonfilename, Json::Value &rootObj) {
    jsonfilename = jsonObjSystemStateFileName_[phoneId];
    rootObj      = jsonObjSystemStateSlot_[phoneId];
}

void CallManagerServerImpl::getJsonForApiResponseSlot(
    int phoneId, std::string &jsonfilename, Json::Value &rootObj) {
...
grpc::Status CallManagerServerImpl::MakeCall(ServerContext *context,
    ...
    if (readStatus.ok()) {
        ...
        CommonUtils::getValues(jsonObjApiResponse, CALL_MANAGER, apiInput, status, error, cbDelay);
        ...
        if (status == telux::common::Status::SUCCESS) {
            ...
            if (isAecsCall) {
                ...
                if (!emergencyModeEnabled) {
                    ...
                    response->set_status(
                        static_cast<commonStub::Status>(telux::common::Status::NOTALLOWED));
            ...
            if (callIndex != CALL_INDEX_INVALID) {
                auto f = std::async(
                    std::launch::async, [this, phoneId, callIndex, isAecsCall, error]() {
...
grpc::Status CallManagerServerImpl::Answer(
    ServerContext *context, const telStub::AnswerRequest *request, telStub::AnswerReply *response) {
    std::string jsonObjApiResponseFileName = "";
    Json::Value jsonObjApiResponse;
    int phoneId             = request->phone_id();
    ...
    if (readStatus.ok()) {
        ...
        bool isCallback = true;
        int cbDelay;
        CommonUtils::getValues(jsonObjApiResponse, CALL_MANAGER, "answer", status, error, cbDelay);
        if (cbDelay == -1) {
            isCallback = false;
        }
        std::shared_ptr<CallInfo> info = findMatchingCall(phoneId, callIndex);
...
void CallManagerServerImpl::changeCallStateofActiveCalls(int phoneId, int callIndex) {
    std::shared_ptr<CallInfo> newCall;
    LOG(DEBUG, __FUNCTION__, " phoneId ", phoneId, " callIndex ", callIndex);
    for (auto callIterator = std::begin(calls_); callIterator != std::end(calls_); ++callIterator) {
        if (((*callIterator)->index != callIndex) && ((*callIterator)->phoneId == phoneId)) {
            if ((*callIterator)->callState == CallState::CALL_ACTIVE) {
                std::this_thread::sleep_for(std::chrono::milliseconds(300));
                changeCallState((*callIterator)->phoneId, "CALL_HOLD", (*callIterator)->index);
            }
        } else {
...
grpc::Status CallManagerServerImpl::MakeECall(ServerContext *context,
    ...
    } else if (makeEcallApiType == CallApi::makeSelfTestERAGLONASSECallWithRawMsd) {
    ...
    if (readStatus.ok()) {
        getJsonForApiResponseSlot(phoneId, jsonObjApiResponseFileName, jsonObjApiResponse);
        CommonUtils::getValues(jsonObjApiResponse, CALL_MANAGER, input, status, error, cbDelay);
        if (cbDelay == -1) {
            isCallback = false;
        }
...
grpc::Status CallManagerServerImpl::SetConfig(ServerContext *context,
    ...
    if (readStatus.ok()) {
        ...
        if (request->is_num_type_valid()) {
            ...
            rootObj[CALL_MANAGER]["eCallConfig"]["numType"] = static_cast<int>(config.numType);
            JsonParser::writeToJsonFile(rootObj, jsonfilename);
            jsonObjSystemStateSlot_[SLOT_1] = rootObj;
        ...
        if (request->is_overridden_num_valid()) {
            ...
            rootObj[CALL_MANAGER]["eCallConfig"]["overriddenNum"] = config.overriddenNum;
            JsonParser::writeToJsonFile(rootObj, jsonfilename);
            jsonObjSystemStateSlot_[SLOT_1] = rootObj;
        }
        if (request->is_use_canned_msd_valid()) {
            config.useCannedMsd                                  = request->use_canned_msd();
            rootObj[CALL_MANAGER]["eCallConfig"]["useCannedMsd"] = config.useCannedMsd;
            JsonParser::writeToJsonFile(rootObj, jsonfilename);
            jsonObjSystemStateSlot_[SLOT_1] = rootObj;
        }
        if (request->is_gnss_update_interval_valid()) {
            config.gnssUpdateInterval = request->gnss_update_interval();
            rootObj[CALL_MANAGER]["eCallConfig"]["gnssUpdateInterval"] = config.gnssUpdateInterval;
            JsonParser::writeToJsonFile(rootObj, jsonfilename);
            jsonObjSystemStateSlot_[SLOT_1] = rootObj;
        }
        ...
        CommonUtils::getValues(
            jsonObjApiResponse, CALL_MANAGER, "setECallConfig", status, error, cbDelay);
        response->set_status(static_cast<commonStub::Status>(status));
    }
    return readStatus;
}
...
grpc::Status CallManagerServerImpl::GetConfig(ServerContext *context,
    ...
    if (readStatus.ok()) {
        ...
        response->set_mute_rx_audio(rootObj[CALL_MANAGER]["eCallConfig"]["muteRxAudio"].asBool());
        ...
        response->set_use_canned_msd(rootObj[CALL_MANAGER]["eCallConfig"]["useCannedMsd"].asBool());
        ...
        response->set_t2_timer(rootObj[CALL_MANAGER]["eCallConfig"]["T2Timer"].asInt());
        response->set_is_t7_timer_valid(true);
        response->set_t7_timer(rootObj[CALL_MANAGER]["eCallConfig"]["T7Timer"].asInt());
        response->set_is_t9_timer_valid(true);
        response->set_t9_timer(rootObj[CALL_MANAGER]["eCallConfig"]["T9Timer"].asInt());
        response->set_is_msd_version_valid(true);
        response->set_msd_version(rootObj[CALL_MANAGER]["eCallConfig"]["msdVersion"].asInt());
        getJsonForApiResponseSlot(SLOT_1, jsonObjApiResponseFileName, jsonObjApiResponse);
        CommonUtils::getValues(
            jsonObjApiResponse, CALL_MANAGER, "getECallConfig", status, error, cbDelay);
        response->set_status(static_cast<commonStub::Status>(status));
...
grpc::Status CallManagerServerImpl::UpdateEcallHlapTimer(ServerContext *context,
    ...
    if (readStatus.ok()) {
        ...
        getJsonForApiResponseSlot(phoneId, jsonObjApiResponseFileName, jsonObjApiResponse);
        CommonUtils::getValues(
            jsonObjApiResponse, CALL_MANAGER, "updateEcallHlapTimer", status, error, cbDelay);
        if (cbDelay == -1) {
            isCallback = false;
        }
        response->set_status(static_cast<commonStub::Status>(status));
...
grpc::Status CallManagerServerImpl::RequestEcallHlapTimer(ServerContext *context,
    ...
    if (readStatus.ok()) {
        getJsonForApiResponseSlot(phoneId, jsonObjApiResponseFileName, jsonObjApiResponse);
        CommonUtils::getValues(
            jsonObjApiResponse, CALL_MANAGER, "requestEcallHlapTimer", status, error, cbDelay);
        getJsonForSystemData(phoneId, jsonfilename, rootObj);
        if (type == ::telStub::HlapTimerType::T10_TIMER) {
...
grpc::Status CallManagerServerImpl::RequestECallHlapTimerStatus(ServerContext *context,
    ...
    if (readStatus.ok()) {
        ...
        bool isCallback = true;
        int cbDelay;
        CommonUtils::getValues(jsonObjApiResponse, CALL_MANAGER, "requestECallHlapTimerStatus",
...
grpc::Status CallManagerServerImpl::ExitEcbm(ServerContext *context,
    ...
    if (readStatus.ok()) {
        getJsonForApiResponseSlot(phoneId, jsonObjApiResponseFileName, jsonObjApiResponse);
        CommonUtils::getValues(
            jsonObjApiResponse, CALL_MANAGER, "exitEcbm", status, error, cbDelay);
        getJsonForSystemData(phoneId, jsonfilename, rootObj);
...
grpc::Status CallManagerServerImpl::RequestEcbm(ServerContext *context,
    ...
    if (readStatus.ok()) {
        getJsonForApiResponseSlot(phoneId, jsonObjApiResponseFileName, jsonObjApiResponse);
        CommonUtils::getValues(
            jsonObjApiResponse, CALL_MANAGER, "requestEcbm", status, error, cbDelay);
        getJsonForSystemData(phoneId, jsonfilename, rootObj);
        ::telStub::EcbMode_Mode mode = static_cast<::telStub::EcbMode_Mode>(
            rootObj[CALL_MANAGER]["ecbm"]["ecbMode"].asInt());
        response->set_ecbmode(mode);
...
void CallManagerServerImpl::logCallDetails(std::shared_ptr<CallInfo> call) {
    LOG(DEBUG, __FUNCTION__, " SlotId = ", static_cast<int>(call->phoneId),
        " Call Info: remotePartyNumber = ", call->remotePartyNumber, ", callIndex = ", call->index,
...
std::shared_ptr<CallInfo> CallManagerServerImpl::findMatchingCall(int slotId, int callIndex) {
    ...
    iter = std::find_if(std::begin(calls_), std::end(calls_),
        [=](std::shared_ptr<CallInfo> call) { return match(call, slotId, callIndex); });
...
bool CallManagerServerImpl::findMatchingCall(CallInfo callToCompare) {
    ...
    std::lock_guard<std::mutex> lock(callManagerMutex_);
    iter = std::find_if(std::begin(calls_), std::end(calls_),
        [=](std::shared_ptr<CallInfo> call) { return match(call, callToCompare); });
    ...
    for (auto callIterator = std::begin(calls_); callIterator != std::end(calls_); ++callIterator) {
        if ((*callIterator)->phoneId == phoneId) {
            if ((*callIterator)->callState == CallState::CALL_WAITING) {
                changeCallState((*callIterator)->phoneId, "CALL_ACTIVE", (*callIterator)->index);
                foundCall = true;
                break;
            }
        }
...
grpc::Status CallManagerServerImpl::HangupForegroundResumeBackground(ServerContext *context,
    ...
    if (readStatus.ok()) {
        ...
        bool isCallback = true;
        int cbDelay;
        CommonUtils::getValues(jsonObjApiResponse, CALL_MANAGER, "hangupForegroundResumeBackground",
            status, error, cbDelay);
        if (cbDelay == -1) {
            isCallback = false;
        }
...
grpc::Status CallManagerServerImpl::Resume(
    ServerContext *context, const telStub::ResumeRequest *request, telStub::ResumeReply *response) {
    std::string jsonObjApiResponseFileName = "";
    Json::Value jsonObjApiResponse;
    int phoneId   = request->phone_id();
    int callIndex = request->call_index();
    ...
    if (readStatus.ok()) {
        getJsonForApiResponseSlot(phoneId, jsonObjApiResponseFileName, jsonObjApiResponse);
        CommonUtils::getValues(jsonObjApiResponse, CALL_MANAGER, "resume", status, error, cbDelay);
        if (cbDelay == -1) {
            isCallback = false;
        }
...
grpc::Status CallManagerServerImpl::Hold(
    ServerContext *context, const telStub::HoldRequest *request, telStub::HoldReply *response) {
    std::string jsonObjApiResponseFileName = "";
    Json::Value jsonObjApiResponse;
    int phoneId   = request->phone_id();
    int callIndex = request->call_index();
    ...
    if (readStatus.ok()) {
        getJsonForApiResponseSlot(phoneId, jsonObjApiResponseFileName, jsonObjApiResponse);
        CommonUtils::getValues(jsonObjApiResponse, CALL_MANAGER, "hold", status, error, cbDelay);
        if (cbDelay == -1) {
            isCallback = false;
        }
...
void CallManagerServerImpl::swapCalls(int callHoldIndex, int phoneIndex, int callActivateIndex) {
    for (auto callIterator = std::begin(calls_); callIterator != std::end(calls_); ++callIterator) {
        if (((*callIterator)->phoneId == phoneIndex) && ((*callIterator)->index == callHoldIndex)) {
            if ((*callIterator)->callState == CallState::CALL_ON_HOLD) {
                changeCallState((*callIterator)->phoneId, "CALL_ACTIVE", (*callIterator)->index);
            }
        }
...
grpc::Status CallManagerServerImpl::Swap(
    ServerContext *context, const telStub::SwapRequest *request, telStub::SwapReply *response) {
    LOG(DEBUG, __FUNCTION__);
    std::string jsonObjApiResponseFileName = "";
    Json::Value jsonObjApiResponse;
    ...
    if (readStatus.ok()) {
        getJsonForApiResponseSlot(SLOT_1, jsonObjApiResponseFileName, jsonObjApiResponse);
        CommonUtils::getValues(jsonObjApiResponse, CALL_MANAGER, "swap", status, error, cbDelay);
        if (cbDelay == -1) {
            isCallback = false;
        }
...
grpc::Status CallManagerServerImpl::HangupWaitingOrBackground(ServerContext *context,
    ...
    if (readStatus.ok()) {
        getJsonForApiResponseSlot(phoneId, jsonObjApiResponseFileName, jsonObjApiResponse);
        CommonUtils::getValues(
            jsonObjApiResponse, CALL_MANAGER, "hangupWaitingOrBackground", status, error, cbDelay);
        if (cbDelay == -1) {
            isCallback = false;
        }
...
grpc::Status CallManagerServerImpl::Hangup(
    ServerContext *context, const telStub::HangupRequest *request, telStub::HangupReply *response) {
    std::string jsonObjApiResponseFileName = "";
    Json::Value jsonObjApiResponse;
    telux::common::ErrorCode error;
    telux::common::Status status;
    ...
    if (readStatus.ok()) {
        getJsonForApiResponseSlot(phoneId, jsonObjApiResponseFileName, jsonObjApiResponse);
        CommonUtils::getValues(jsonObjApiResponse, CALL_MANAGER, "hangup", status, error, cbDelay);
        if (cbDelay == -1) {
            isCallback = false;
        }
        std::shared_ptr<CallInfo> info = findMatchingCall(phoneId, callIndex);
...
grpc::Status CallManagerServerImpl::Reject(
    ServerContext *context, const telStub::RejectRequest *request, telStub::RejectReply *response) {
    std::string jsonObjApiResponseFileName = "";
    Json::Value jsonObjApiResponse;
    int phoneId   = request->phone_id();
    int callIndex = request->call_index();
    ...
    if (readStatus.ok()) {
        getJsonForApiResponseSlot(phoneId, jsonObjApiResponseFileName, jsonObjApiResponse);
        CommonUtils::getValues(jsonObjApiResponse, CALL_MANAGER, "reject", status, error, cbDelay);
        if (cbDelay == -1) {
            isCallback = false;
        }
        std::shared_ptr<CallInfo> info = findMatchingCall(phoneId, callIndex);
...
grpc::Status CallManagerServerImpl::RejectWithSMS(ServerContext *context,
    ...
    if (readStatus.ok()) {
        getJsonForApiResponseSlot(phoneId, jsonObjApiResponseFileName, jsonObjApiResponse);
        CommonUtils::getValues(
            jsonObjApiResponse, CALL_MANAGER, "rejectSms", status, error, cbDelay);
        if (cbDelay == -1) {
            isCallback = false;
        }
        std::shared_ptr<CallInfo> info = findMatchingCall(phoneId, callIndex);
            ...
            if ((input[0] == "SUCCESS")
                ...
                if (isNGeCall) {
                    if ((callInfo_.isRegulatoryeCall) ||
                        ...
                        ecallStateMachine_->onEvent(ecallStateMachine_->createTelEvent(
                            EcallStateMachine::EventID::MSD_PULL_REQUEST_FROM_PSAP, "NGeCall",
                            phoneId));
                    }
                } else {
                    // CS eCall
...
grpc::Status CallManagerServerImpl::ModifyOrRespondToModifyCall(ServerContext *context,
    ...
    if (readStatus.ok()) {
        getJsonForApiResponseSlot(phoneId, jsonObjApiResponseFileName, jsonObjApiResponse);
        CommonUtils::getValues(jsonObjApiResponse, CALL_MANAGER, inputApi, status, error, cbDelay);
        if (cbDelay == -1) {
            isCallback = false;
        }
        std::shared_ptr<CallInfo> info = findMatchingCall(phoneId, callIndex);
    ...
    if (readStatus.ok()) {
        if (ecallStateMachine_ != nullptr) {
            ...
            if (T10Status == HlapTimerStatus::ACTIVE) {
                ecallStateMachine_->onEvent(ecallStateMachine_->createTelEvent(
                    EcallStateMachine::EventID::ON_NETWORK_DEREGISTRATION_REQUEST, "T10Timer",
                    phoneId));
            }
        }
        getJsonForApiResponseSlot(phoneId, jsonObjApiResponseFileName, jsonObjApiResponse);
...
void CallManagerServerImpl::handleOperatingModeChanged(::telStub::OperatingModeEvent event) {
    LOG(DEBUG, __FUNCTION__);
    ...
    if (opMode == telux::tel::OperatingMode::AIRPLANE) {
        ...
        if (readStatus.ok()) {
            if (ecallStateMachine_ != nullptr) {
                // Since, operating mode event is device specific, T10 timer status is checked
                // on both the slots as automotive device only supports one eCall at a time.
                for (int i = SLOT_1; i <= SLOT_2; i++) {
                    ...
                    // No event is sent for T9 timer because, T9 timer cannot be reset with
                    // operating mode change.
                    if (T10Status == HlapTimerStatus::ACTIVE) {
                        ecallStateMachine_->onEvent(ecallStateMachine_->createTelEvent(
                            EcallStateMachine::EventID::ON_OPERATING_MODE_AIRPLANE, "T10Timer", i));
        ...
        LOG(DEBUG, __FUNCTION__, " No handling required for other operating mode states ");
    }
}
...
void CallManagerServerImpl::handleIncomingCallRequest(std::string eventParams) {
    ...
    std::vector<telStub::RadioTechnology> psRatList
        = {telStub::RadioTechnology::RADIO_TECH_NR5G, telStub::RadioTechnology::RADIO_TECH_LTE};
    ...
    auto f = std::async(std::launch::async, [this, callInfo]() {
        this->changeCallState(
            callInfo.phoneId, Helper::getCallStateInString(callInfo.callState), callInfo.index);
    }).share();
    taskQ_->add(f);
}
...
std::vector<std::string> CallManagerServerImpl::parseUserInput() {
    ...
    if (readStatus.ok()) {
        ...
        std::string input = rootObj[CALL_MANAGER]["configureFailureForRegulatoryECall"].asString();
...
telux::common::Status CallManagerServerImpl::handleStateMachine(int phoneId, int callIndex) {
    ...
    if (callInfo_.isRegulatoryeCall != true) {
        ...
        ecallStateMachine_ = std::make_shared<EcallStateMachine>(shared_from_this(), input,
            callInfo_.isMsdTransmitted, callInfo_.isTpseCallOverIms, false, phoneId, callIndex,
            true, "SUCCESS", callInfo_.isEraGlonassSelfTestECall, false);
    } else {
        // Regulatory eCall
        std::vector<std::string> input = parseUserInput();
        ...
        ecallStateMachine_ = std::make_shared<EcallStateMachine>(shared_from_this(), input,
            callInfo_.isMsdTransmitted, isNGeCall, isALACKConfigEnabled, phoneId, callIndex, false,
            eCallRedialConfig, callInfo_.isEraGlonassSelfTestECall, false);
    }
    if (!ecallStateMachine_) {
...
void CallManagerServerImpl::updateEcallHlapTimer(
    int phoneId, std::string timer, HlapTimerStatus status) {
...
void CallManagerServerImpl::startTimer(std::string timer, int phoneId) {
    ...
    if ((timer == "T2Timer") || (timer == "T5Timer") || (timer == "T6Timer") || (timer == "T7Timer")
        || (timer == "T9Timer") || (timer == "T10Timer")) {
        startTimers(timer, phoneId);
        auto f = std::async(std::launch::async, [this, phoneId, timer]() {
            this->triggerECallInfoChangeEvent(phoneId, timer, HlapTimerEvent::STARTED);
        }).share();
        taskQ_->add(f);
    } else {
...
void CallManagerServerImpl::startTimers(std::string timer, int phoneId) {
    ...
    if (readStatus.ok()) {
        ...
        auto f = std::async(std::launch::async, [this, phoneId, delay, timer]() {
            ...
            while (!hlapTimerResetCv_.wait_until(lock, start + timeout, predicate)) {
        ...
        if (timer == "T2Timer") {
            ...
            for (int i = 0; i <= REST_TIMERS_ON_CALL_SETUP - 1; i++) {
                ...
                if (status == HlapTimerStatus::ACTIVE) {
                    ...
                    auto f = std::async(std::launch::async, [this, phoneId, timer, i, status]() {
                        std::this_thread::sleep_for(std::chrono::milliseconds(1000));
                        this->triggerECallInfoChangeEvent(
                            phoneId, timer[i], HlapTimerEvent::STOPPED);
                    }).share();
                    taskQ_->add(f);
                }
            }
...
void CallManagerServerImpl::expiryTimer(std::string timer, int phoneId) {
    ...
    updateEcallHlapTimer(phoneId, timer, HlapTimerStatus::INACTIVE);
    auto f = std::async(std::launch::async, [this, phoneId, timer]() {
        this->triggerECallInfoChangeEvent(phoneId, timer, HlapTimerEvent::EXPIRED);
    }).share();
    taskQ_->add(f);
}

void CallManagerServerImpl::sendEvent(std::string timer, std::string status, int phoneId) {
    ...
    if (status == "start") {
        ...
        auto f = std::async(std::launch::async, [this, phoneId, timer, status]() {
            std::this_thread::sleep_for(std::chrono::milliseconds(1000));
            this->triggerECallInfoChangeEvent(phoneId, timer, HlapTimerEvent::STOPPED);
        }).share();
        taskQ_->add(f);
    } else {
...
void CallManagerServerImpl::triggerECallInfoChangeEvent(
    int phoneId, std::string timer, HlapTimerEvent action) {
    LOG(DEBUG, __FUNCTION__, " phoneId: ", phoneId);
...
void CallManagerServerImpl::triggerCallInfoChangeEvent(
    int phoneId, int callIndex, bool retainCache) {
    ...
    if (call != nullptr) {
        ...
        if (call->callState == CallState::CALL_ENDED) {
            ...
            // Clear call cache in server
            auto f = std::async(
                std::launch::async, [this, phoneId, callIndex, causeCode, reason, retainCache]() {
                    std::this_thread::sleep_for(std::chrono::milliseconds(1000));
        ...
        LOG(ERROR, __FUNCTION__, " Call not found for phoneId ", phoneId, " and callIndex ",
            callIndex);
    }
}

void CallManagerServerImpl::onECallRedial(
    int phoneId, bool willECallRedial, telux::tel::ReasonType reason) {
...
grpc::Status CallManagerServerImpl::ConfigureECallRedial(ServerContext *context,
    ...
    if (readStatus.ok()) {
        ...
        getJsonForApiResponseSlot(SLOT_1, jsonObjApiResponseFileName, jsonObjApiResponse);
        CommonUtils::getValues(
            jsonObjApiResponse, CALL_MANAGER, "configureECallRedial", status, error, cbDelay);
        if (cbDelay == -1) {
            isCallback = false;
        }
        response->set_status(static_cast<commonStub::Status>(status));
...
grpc::Status CallManagerServerImpl::getECallRedialConfig(ServerContext *context,
    const google::protobuf::Empty *request, telStub::GetECallRedialResponse *response) {
    LOG(DEBUG, __FUNCTION__);
    ...
    if (readStatus.ok()) {
        ...
        getJsonForApiResponseSlot(SLOT_1, jsonObjApiResponseFileName, jsonObjApiResponse);
        CommonUtils::getValues(
            jsonObjApiResponse, CALL_MANAGER, "getECallRedialConfig", status, error, cbDelay);
        std::string callOrigTimeGapInString
            = rootObj["ICallManager"]["eCallRedialTimeGap"]["callOrigFailure"].asString();
        std::string callDropTimeGapInString
            = rootObj["ICallManager"]["eCallRedialTimeGap"]["callDrop"].asString();
        callOrigTimeGap = CommonUtils::convertStringToVector(callOrigTimeGapInString);
        callDropTimeGap = CommonUtils::convertStringToVector(callDropTimeGapInString);
    }
    // Create response
    response->set_error(static_cast<commonStub::ErrorCode>(error));
...
grpc::Status CallManagerServerImpl::updateECallPostTestRegistration(ServerContext *context,
    ...
    if (readStatus.ok()) {
        ...
        CommonUtils::getValues(jsonObjApiResponse, CALL_MANAGER,
            "updateECallPostTestRegistrationTimer", status, error, cbDelay);
        rootObj[CALL_MANAGER]["eCallPostTestRegistrationTimer"]["timer"] = timerDuration;
        JsonParser::writeToJsonFile(rootObj, jsonfilename);
        jsonObjSystemStateSlot_[phoneId] = rootObj;
        if (cbDelay == -1) {
            isCallback = false;
...
grpc::Status CallManagerServerImpl::getECallPostTestRegistrationTimer(ServerContext *context,
    ...
    if (readStatus.ok()) {
        ...
        getJsonForApiResponseSlot(phoneId, jsonObjApiResponseFileName, jsonObjApiResponse);
        CommonUtils::getValues(jsonObjApiResponse, CALL_MANAGER,
            "getECallPostTestRegistrationTimer", status, error, cbDelay);
        response->set_error(static_cast<commonStub::ErrorCode>(error));
        response->set_timer_duration(
            rootObj[CALL_MANAGER]["eCallPostTestRegistrationTimer"]["timer"].asInt());
    }
    return readStatus;
}

grpc::Status CallManagerServerImpl::restartECallHlapTimer(ServerContext *context,
    ...
    if (readStatus.ok()) {
        getJsonForApiResponseSlot(phoneId, jsonObjApiResponseFileName, jsonObjApiResponse);
        CommonUtils::getValues(
            jsonObjApiResponse, CALL_MANAGER, "restartECallHlapTimer", status, error, cbDelay);
        if (cbDelay == -1) {
            isCallback = false;
        }
        response->set_status(static_cast<commonStub::Status>(status));
        ...
        LOG(DEBUG, __FUNCTION__, " HLAP timer status ", static_cast<int>(hlapTimerStatus), " for ",
            timer);
        if ((hlapTimerStatus == HlapTimerStatus::INACTIVE) && (allowRestartOfTimer)) {
            response->set_error(static_cast<commonStub::ErrorCode>(error));
...
void CallManagerServerImpl::restartTimer(int phoneId, std::string timer, int timerDuration) {
    ...
    auto f = std::async(std::launch::async, [this, phoneId, timer, timerDuration]() {
        updateEcallHlapTimer(phoneId, timer, HlapTimerStatus::ACTIVE);
        this->triggerECallInfoChangeEvent(phoneId, timer, HlapTimerEvent::RESUMED);
...
telux::tel::ECallMode CallManagerServerImpl::getEcallOperatingMode(int phoneId) {
    JsonData data;
...
std::vector<std::shared_ptr<CallInfo>> CallManagerServerImpl::fetchSlotIdCalls(int phoneId) {
...
grpc::Status CallManagerServerImpl::getInProgressCalls(ServerContext *context,
    const telStub::GetInProgressCallsRequest *request, telStub::GetInProgressCallsData *response) {
    LOG(DEBUG, __FUNCTION__);
    int phoneId = request->phone_id();
...
void CallManagerServerImpl::changeCallState(
    int phoneId, std::string action, int index, bool retainCache) {
    LOG(DEBUG, __FUNCTION__, " phoneId ", phoneId);
...
void CallManagerServerImpl::triggerCallListAfterCallEnd(
    int phoneId, CallEndCause causeCode, AecsCallEndReason reason) {
...
bool CallManagerServerImpl::findCallAndUpdateCallState(int index, CallState action, int phoneId) {
    ...
    iter = std::find_if(std::begin(calls_), std::end(calls_),
        [=](std::shared_ptr<CallInfo> call) { return find(call, index, phoneId); });
...
bool CallManagerServerImpl::findCallAndUpdateRttMode(int index, RttMode mode, int phoneId) {
