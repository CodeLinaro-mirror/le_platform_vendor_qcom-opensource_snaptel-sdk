...
PhoneFactoryImplStub::~PhoneFactoryImplStub() {
    ...
    for (const auto &networkSelMgr : networkSelectionManagerMap_) {
        if (networkSelMgr.second != nullptr) {
            (std::static_pointer_cast<NetworkSelectionManagerStub>(networkSelMgr.second))->cleanup();
        }
    }
...
void PhoneFactoryImplStub::onPhoneManagerResponse(telux::common::ServiceStatus status) {
    ...
    LOG(INFO, __FUNCTION__, " Phone Manager initialization status: ", static_cast<int>(status));
    {
        std::lock_guard<std::recursive_mutex> lock(mutex_);
...
void PhoneFactoryImplStub::onCardManagerResponse(telux::common::ServiceStatus status) {
    ...
    LOG(INFO, __FUNCTION__, " Card Manager initialization status: ", static_cast<int>(status));
    {
        std::lock_guard<std::recursive_mutex> lock(mutex_);
...
std::shared_ptr<ISapCardManager> PhoneFactoryImplStub::getSapCardManager(
...
void PhoneFactoryImplStub::onSubscriptionManagerResponse(telux::common::ServiceStatus status) {
    ...
    LOG(INFO, __FUNCTION__,
        " Subscription Manager initialization status: ", static_cast<int>(status));
    {
        std::lock_guard<std::recursive_mutex> lock(mutex_);
...
std::shared_ptr<telux::tel::IServingSystemManager> PhoneFactoryImplStub::getServingSystemManager(
    ...
    if (mgr == servingSystemManagerMap_.end()) {
        ...
        servingSysMgrInitStatus_.emplace(slotId, telux::common::ServiceStatus::SERVICE_UNAVAILABLE);
        if (callback) {
...
std::shared_ptr<telux::tel::INetworkSelectionManager>
    PhoneFactoryImplStub::getNetworkSelectionManager(
        int slotId, telux::common::InitResponseCb callback) {
    LOG(DEBUG, __FUNCTION__, " slotId: ", slotId);
    if ((slotId < DEFAULT_SLOT_ID) || (slotId > MAX_SLOT_ID)) {
        LOG(ERROR, __FUNCTION__, " Invalid slotId: ", slotId);
    ...
    if (mgr == networkSelectionManagerMap_.end()) {
        ...
        networkSelMgrInitStatus_.emplace(slotId, telux::common::ServiceStatus::SERVICE_UNAVAILABLE);
        if (callback) {
...
void PhoneFactoryImplStub::onNetworkSelectionMgrInitResponse(
    int slotId, telux::common::ServiceStatus status) {
    std::vector<telux::common::InitResponseCb> networkSelMgrCallbacks;
    LOG(INFO, __FUNCTION__,
        " NetworkSelection Manager initalization status: ", static_cast<int>(status),
        " on phone: ", slotId);
    {
        std::lock_guard<std::recursive_mutex> lock(mutex_);
...
std::shared_ptr<IRemoteSimManager> PhoneFactoryImplStub::getRemoteSimManager(
    int slotId, telux::common::InitResponseCb callback) {
    return nullptr;
}
...
std::shared_ptr<ICellBroadcastManager> PhoneFactoryImplStub::getCellBroadcastManager(
...
void PhoneFactoryImplStub::onCellBroadcastManagerResponse(
    SlotId slotId, telux::common::ServiceStatus status) {
    std::vector<telux::common::InitResponseCb> cbMgrCallbacks;
    LOG(INFO, __FUNCTION__,
        " CellBroadcast Manager initialization status: ", static_cast<int>(status),
        " on slot: ", slotId);
    {
        std::lock_guard<std::recursive_mutex> lock(mutex_);
...
std::shared_ptr<IImsServingSystemManager> PhoneFactoryImplStub::getImsServingSystemManager(
    ...
    if (ims == imsServSysManagerMap_.end()) {
        ...
        imsServingSystemMgrInitStatus_.emplace(
            slotId, telux::common::ServiceStatus::SERVICE_UNAVAILABLE);
        if (callback) {
...
void PhoneFactoryImplStub::onServingSystemMgrInitResponse(
    int slotId, telux::common::ServiceStatus status) {
    std::vector<telux::common::InitResponseCb> servingSystemMgrCallbacks;
    LOG(INFO, __FUNCTION__,
        " ServingSystem Manager initalization status: ", static_cast<int>(status),
        " on phone: ", slotId);
    {
        std::lock_guard<std::recursive_mutex> lock(mutex_);
...
void PhoneFactoryImplStub::onImsServingSystemMgrInitResponse(
    SlotId slotId, telux::common::ServiceStatus status) {
    std::vector<telux::common::InitResponseCb> imsServingSystemMgrCallbacks;
    LOG(INFO, __FUNCTION__,
        " IMSServingSystem Manager initalization status: ", static_cast<int>(status),
        " on phone: ", slotId);
    {
        std::lock_guard<std::recursive_mutex> lock(mutex_);
...
void PhoneFactoryImplStub::onSuppSvcInitResponse(
    SlotId slotId, telux::common::ServiceStatus status) {
...
void PhoneFactoryImplStub::onApSimProfileManagerResponse(telux::common::ServiceStatus status) {
    ...
    LOG(INFO, __FUNCTION__,
        " ApSimProfile Manager initialization status: ", static_cast<int>(status));
    {
        std::lock_guard<std::recursive_mutex> lock(mutex_);
