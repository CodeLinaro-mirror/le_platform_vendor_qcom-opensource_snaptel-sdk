...
DataProfileManagerStub::DataProfileManagerStub(
    ...
    LOG(DEBUG, __FUNCTION__, "initializing DataProfileManagerStub for slotId:", slotId);
...
telux::common::Status DataProfileManagerStub::createProfile(
    const ProfileParams &profileParams, std::shared_ptr<IDataCreateProfileCallback> callback) {
    LOG(DEBUG, __FUNCTION__);
    ...
    request.mutable_tech_preference()->set_tech_preference(
        (::dataStub::TechPreference::TechPref)profileParams.techPref);
    request.mutable_auth_type()->set_auth_type(
        (::dataStub::AuthProtocolType::AuthProto)profileParams.authType);
    request.mutable_ip_family_type()->set_ip_family_type(
        (::dataStub::IpFamilyType::Type)profileParams.ipFamilyType);
    ...
    if (status == telux::common::Status::SUCCESS) {
        ...
        if (callback && (delay != SKIP_CALLBACK)) {
            ...
            LOG(DEBUG, __FUNCTION__, " created profile profileId:", profileId);
            auto f = std::async(std::launch::async, [this, callback, profileId, error]() {
...
telux::common::Status DataProfileManagerStub::deleteProfile(uint8_t profileId,
    ...
    request.mutable_profile()->mutable_tech_preference()->set_tech_preference(
        (::dataStub::TechPreference::TechPref)techPreference);
    ...
    if (status == telux::common::Status::SUCCESS) {
        ...
        if (callback && (delay != SKIP_CALLBACK)) {
            LOG(DEBUG, __FUNCTION__, " deleted profile profileId:", profileId);
            auto f = std::async(std::launch::async, [this, callback, error]() {
        ...
        if (error == telux::common::ErrorCode::SUCCESS) {
            this->onProfileUpdate(
                profileId, techPreference, ProfileChangeEvent::DELETE_PROFILE_EVENT);
        }
    }
...
telux::common::Status DataProfileManagerStub::modifyProfile(uint8_t profileId,
    ...
    if (status == telux::common::Status::SUCCESS) {
        ...
        if (callback && (delay != SKIP_CALLBACK)) {
            LOG(DEBUG, __FUNCTION__, " modified profile profileId:", profileId);
            auto f = std::async(std::launch::async, [this, callback, error]() {
                callback->commandResponse(error);
            }).share();
            taskQ_->add(f);
        ...
        if (error == telux::common::ErrorCode::SUCCESS) {
            this->onProfileUpdate(
                profileId, profileParams.techPref, ProfileChangeEvent::MODIFY_PROFILE_EVENT);
        }
    }
...
telux::common::Status DataProfileManagerStub::requestProfile(uint8_t profileId,
    ...
    if (status == telux::common::Status::SUCCESS) {
        ...
        ApnTypes apnTypes     = convertApnTypeStringToEnum(response.profile().apn_types());
        IpFamilyType ipFamily = static_cast<telux::data::IpFamilyType>(
            response.profile().ip_family_type().ip_family_type());
        TechPreference techPref = static_cast<telux::data::TechPreference>(
            response.profile().tech_preference().tech_preference());
        AuthProtocolType authType = static_cast<telux::data::AuthProtocolType>(
        ...
        if (callback && (delay != SKIP_CALLBACK)) {
            ...
            auto f = std::async(std::launch::async, [this, error, queryProfile, callback]() {
...
telux::common::Status DataProfileManagerStub::requestProfileList(
    ...
    if (status == telux::common::Status::SUCCESS) {
        ...
        for (int idx = 0; idx < response.profiles_size(); idx++) {
            ...
            ApnTypes apnTypes
                = convertApnTypeStringToEnum(response.mutable_profiles(idx)->apn_types());
            IpFamilyType ipFamily = static_cast<telux::data::IpFamilyType>(
        ...
        if (callback && (delay != SKIP_CALLBACK)) {
            auto f = std::async(std::launch::async, [this, callback, requestedProfiles, error]() {
...
telux::common::Status DataProfileManagerStub::queryProfile(
    const ProfileParams &profileParams, std::shared_ptr<IDataProfileListCallback> callback) {
    ...
    if (status == telux::common::Status::SUCCESS) {
        ...
        if (callback && (delay != SKIP_CALLBACK)) {
            auto f = std::async(std::launch::async, [this, callback, queriedProfiles, error]() {
...
void DataProfileManagerStub::onProfileUpdate(
    int profileId, TechPreference techPreference, ProfileChangeEvent event) {
    LOG(DEBUG, __FUNCTION__);
...
std::string DataProfileManagerStub::convertApnTypeEnumToString(telux::data::ApnTypes apnType) {
    LOG(DEBUG, __FUNCTION__);
...
telux::data::ApnTypes DataProfileManagerStub::convertApnTypeStringToEnum(std::string apn) {
    LOG(DEBUG, __FUNCTION__);
