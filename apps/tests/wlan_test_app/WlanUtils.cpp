...
void WlanUtils::printStaStatus(std::vector<telux::wlan::StaStatus> &staStatus) {
    if (staStatus.size() > 0) {
        ...
        for (auto &sta : staStatus) {
            ...
            std::cout << "Interface Status  : " << WlanUtils::getStaInterfaceStatus(sta.status)
                      << std::endl;
            std::cout
                << "Connection status : " << WlanUtils::getStaConnectionStatus(sta.connectionStatus)
                << std::endl;
        }
        std::cout << std::endl;
    } else {
...
void WlanUtils::printScanResult(const telux::wlan::StaScanResult &staScanResult) {
    std::cout << "--------------------------------------------" << std::endl;
    std::cout
        << "Id                                  : " << WlanUtils::getWlanId(staScanResult.staId)
        << std::endl;
    std::cout
        << "Batch index                         : " << static_cast<int>(staScanResult.batchIndex)
        << std::endl;
...
void WlanUtils::printApElementInfo(telux::wlan::ApElementInfoConfig ElementInfoConfig) {
    ...
    std::cout << "    Enabled             : " << ((ElementInfoConfig.isEnabled) ? "Yes" : "No")
              << std::endl;
    std::cout << "    Interworking Enabled: "
              << ((ElementInfoConfig.isInterworkingEnabled) ? "Yes" : "No") << std::endl;
    std::cout << "    Network Access Type: "
              << WlanUtils::apElementInfoAccessTypeToString(ElementInfoConfig.netAccessType)
              << std::endl;
    std::cout << "    Connectivity to Internet: " << ((ElementInfoConfig.internet) ? "Yes" : "No")
              << std::endl;
    std::cout
        << "    Additional step required for access: " << ((ElementInfoConfig.asra) ? "Yes" : "No")
        << std::endl;
    std::cout << "    Emergency services reachable: " << ((ElementInfoConfig.esr) ? "Yes" : "No")
              << std::endl;
...
void WlanUtils::printNetworkConfigs(
    ...
    if (networkConfigsInfo.size() > 0) {
        ...
        for (auto &networkConfig : networkConfigsInfo) {
            ...
            std::cout << "Radio Type          : " << RadioTypeToString(networkConfig.band)
                      << std::endl;
