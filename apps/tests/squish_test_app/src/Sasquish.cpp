...
static int writeGeneralLog(char *tmpLogStr, uint32_t maxBufSize, bsm_data *bs, FILE *myfp,
    bool isTx, uint64_t periodicityMs, bool validPkt, uint32_t RVsInRange, const char *timeStamp,
    ...
    if (isTx) {
        snprintf(tmpLogStr, maxBufSize,
            ...
            timeStamp, realworldTimeNow, monotonicTime, "Tx", cbr, 0, txInterval, bs->MsgCount,
 ...
 * @returns void
 */
static void updateHostBsmData(uint32_t id, uint32_t l2SrcAddr, SasquishTestData *hvSasquishData) {
    hostBsmData.id              = id;
    hostBsmData.Speed           = hvSasquishData->vehData.speed;
    hostBsmData.Latitude        = (signed int)(hvSasquishData->vehData.pos.posLat * 10000000.0);
    hostBsmData.Longitude       = (signed int)(hvSasquishData->vehData.pos.posLong * 10000000.0);
    hostBsmData.Heading_degrees = (unsigned int)(hvSasquishData->vehData.pos.heading / 0.0125);
 ...
 * @returns void
 */
static void updateBsmData(
    uint32_t id, uint32_t l2SrcAddr, SasquishTestData *sasquishData, bsm_data *bsmData) {
    bsmData->id              = id;
    bsmData->Speed           = sasquishData->vehData.speed;
    bsmData->Latitude        = (signed int)(sasquishData->vehData.pos.posLat * 10000000.0);
    bsmData->Longitude       = (signed int)(sasquishData->vehData.pos.posLong * 10000000.0);
    bsmData->Heading_degrees = (unsigned int)(sasquishData->vehData.pos.heading / 0.0125);
 ...
 * @returns void
 */
static void updateHostVehDataThr(
    long long hostVehDataInt, std::shared_ptr<ICongestionControlManager> congestionControlManager) {
    ...
    while (!stopApp) {
        congestionControlManager->updateHostVehicleData(
            currHvData->vehData.pos, currHvData->vehData.speed);
 ...
 * @returns void
 */
static void writeRVDataToLog(uint64_t timeSinceStart, SasquishTestData *sasquishRvData,
    ...
    bsm_data bsmData       = {0};
    updateBsmData(sasquishRvData->id, sasquishRvData->l2SrcAddr, sasquishRvData, &bsmData);
    writeGeneralLog(tmpLogStr, 200, &bsmData, myfp, false, 0, true, rvsInRange,
        SasquishUtils::getCurrentTimestampStr().c_str(), timeSinceStart, realWorldTime, 0.0, 0, 0,
        0, 0, sasquishRvData->l2SrcAddr);
    tmpSs << tmpLogStr << ",";
    tmpSs << ",";  // track err
    tmpSs << ",";  // veh dens
    tmpSs << ",";  // cqi
...
static void loadCongCtrlDataFn(std::shared_ptr<ICongestionControlManager> congestionControlManager,
    ...
    while (i < sasquishTestData.size() && !stopApp && dataIter != sasquishTestData.end()) {
        ...
        switch (dataIter->dataType) {
            case hostVehicleData:
                ...
                if (SasquishUtils::getSasquishVerbose()) {
                    ...
                    std::cout << " hv data\n";
                    std::cout << "rx time stamp of data is: " << dataIter->vehData.rxTimeStamp
                              << "\n";
                    std::cout << "msg cnt of data is: " << dataIter->vehData.currMsgCnt << "\n";
                    std::cout << "id of data is: " << dataIter->id << "\n";
                    std::cout << "pos lat of data is: " << dataIter->vehData.pos.posLat << "\n";
                    std::cout << "pos long of data is: " << dataIter->vehData.pos.posLong << "\n";
                    std::cout << "speed of data is: " << dataIter->vehData.speed << "\n";
                    std::cout << "heading of data is: " << dataIter->vehData.pos.heading << "\n";
                }
                // update host data to be latest each time we see HV data
                updateHostBsmData(dataIter->id, dataIter->l2SrcAddr, &(*dataIter));
                currHvData = dataIter;
                congestionControlManager->updateHostVehicleData(
                    dataIter->vehData.pos, dataIter->vehData.speed);
                if (!firstHvDataSeen) {
                    firstHvDataSeen = true;
                    std::thread hvDataFeedThrd(updateHostVehDataThr, 100, congestionControlManager);
            ...
            case remoteVehicleData:
                ...
                congestionControlManager->addCongestionControlData(rvTmpId,
                    dataIter->vehData.pos.posLat, dataIter->vehData.pos.posLong,
                    dataIter->vehData.pos.heading, dataIter->vehData.speed,
                    dataIter->vehData.rxTimeStamp, dataIter->vehData.currMsgCnt);
                if (writeRxLogs_) {
                    writeRVDataToLog(timeSinceStart, &(*dataIter), sasquishOutputHandler);
                }
                break;
...
static void writeSquishOutputToLog(uint64_t timeSinceStart,
    ...
    writeGeneralLog(tmpLogStr, 200, &hostBsmData, myfp, true, lastPeriodicity_, true, rvsInRange,
        SasquishUtils::getCurrentTimestampStr().c_str(), timeSinceStart, realWorldTime, 0.0, 0, 0,
        0, txInterval, l2SrcAddr);
    tmpSs << tmpLogStr << ",";
...
static void outputLoggerFn(std::shared_ptr<SasquishOutputHandler> sasquishOutputHandler,
    ...
    while (!stopApp) {
        ...
        writeSquishOutputToLog(timeSinceStart, congestionControlOutput, sasquishOutputHandler);
    }
}
...
void Sasquish::printUsage() {
    std::cout
        << "Mandatory Arguments:\n"
           ...
           "\t -v Verbose Mode\n"
           "Note: Please refer to /etc/ObeConfig.conf and its SQUISH (Congestion Control) section "
           "for "
 ...
 * @returns void
 */
void Sasquish::loadCongestionControlData(
    ...
    if (!fakeRVTempIds_) {
        ...
        if (fakeMsgCntMap.find(currFakeRVTempId) != fakeMsgCntMap.end()) {
            ...
            fakeMsgCntMap.insert(std::pair<int, int>(currFakeRVTempId, newFakeMsgCnt));
        } else {
 ...
 * @returns void
 */
void Sasquish::loadSquishInputData(std::shared_ptr<SasquishInputHandler> sasquishInputHandler) {
    int logs_read = 0;
    int threadId  = 0;
    string line;
    ...
    if (sasquishInputHandler && !stopApp) {
        ...
        while (sasquishInputHandler->readLineFromFile(line)) {
            ...
            if (not readCsvLine(line, &sasquishTestData)) {
                if (SasquishUtils::getSasquishVerbose()) {
                    cerr << "Error occurred when parsing the following csv line : " << line << endl;
                }
            } else {
                if (multithreaded_) {
        ...
        if (SasquishUtils::getSasquishVerbose()) {
            cout << logs_read << " congestionControl data entries read from file." << endl;
...
void Sasquish::initOutputCsv() {
    ...
    std::string strHeader = "TimeStamp,TimeStamp_ms,Time_monotonic,"
...
void Sasquish::readConfigFile() {
    ...
    if (configs.find("perInterval") != configs.end()) {
        congCtrlConfig_.perConfig.packetErrorInterval = stoi(configs["perInterval"]);
    }
    if (configs.find("perSubInterval") != configs.end()) {
        congCtrlConfig_.perConfig.packetErrorSubInterval = stoi(configs["perSubInterval"]);
    }
    ...
    if (configs.find("vDensityWeightFactor") != configs.end()) {
        congCtrlConfig_.densConfig.densWeightFactor = stod(configs["vDensityWeightFactor"]);
    }
    ...
    if (configs.find("enableSpsEnhancements") != configs.end()) {
        ...
        if (configs.find("spsEnhIntervalRound") != configs.end()) {
            congCtrlConfig_.spsEnhanceConfig.spsPeriodicity = stoi(configs["spsEnhIntervalRound"]);
        }
        if (configs.find("spsEnhDelayPerc") != configs.end()) {
            congCtrlConfig_.spsEnhanceConfig.changeFrequency = stoi(configs["spsEnhDelayPerc"]);
        }
        if (configs.find("spsEnhHysterPerc") != configs.end()) {
            congCtrlConfig_.spsEnhanceConfig.hysterPercent = stoi(configs["spsEnhHysterPerc"]);
        }
    }
    /*
    ...
    if (configs.find("totalFakeRVTempIds") != configs.end()) {
        totalFakeRVTempIds = stoi(configs["totalFakeRVTempIds"]);
    }
    if (configs.find("msgCntGap") != configs.end()) {
        msgCntGap = stoi(configs["msgCntGap"]);
    }

    if (configs.find("rvTransmitLossSimulation") != configs.end()) {
        rvTransmitLossSimulation = stoi(configs["rvTransmitLossSimulation"]);
    }
...
bool Sasquish::initSquishConfigs() {
    ...
    if (congestionControlConfigFileName_.empty()) {
        congestionControlManager_->updateCongestionControlType(CongestionControlType::SAE);
        congestionControlManager_->updateCbpConfig(
            DEFAULT_CBP_WEIGHT_FACTOR, DEFAULT_CBP_MEAS_INTERVAL);
        congestionControlManager_->updatePERConfig(
            DEFAULT_PER_MAX, DEFAULT_PER_INTERVAL, DEFAULT_PER_SUBINTERVAL);
        congestionControlManager_->updateDensConfig(
            DEFAULT_DENSITY_COEFFICIENT, DEFAULT_DENSITY_WEIGHT_FACTOR, DEFAULT_MIN_PER_RANGE);
        congestionControlManager_->updateTeConfig(DEFAULT_TX_RATE_CTRL_INTERVAL,
            DEFAULT_HV_TE_MIN_TIME_DIFF, DEFAULT_HV_TE_MAX_TIME_DIFF, DEFAULT_RV_TE_MIN_TIME_DIFF,
            DEFAULT_RV_TE_MAX_TIME_DIFF, DEFAULT_TE_MIN_THRESH, DEFAULT_TE_MAX_THRESH,
            DEFAULT_TE_ERR_SENSITIVITY);
        congestionControlManager_->updateIttConfig(DEFAULT_RESCHED_THRESH, DEFAULT_TIME_ACC,
            DEFAULT_MIN_ITT, DEFAULT_MAX_ITT, DEFAULT_TX_RAND);
    } else {
    ...
    if (multithreaded_) {
        for (long unsigned int i = 0; i < sasquishTestDataAll_.size(); i++) {
            if (SasquishUtils::getSasquishVerbose()) {
                ...
                std::cout << "size of this threads vector is: " << sasquishTestDataAll_[i].size()
                          << "\n";
            }
        ...
        std::thread feedThrd(loadCongCtrlDataFn, congestionControlManager_, sasquishTestDataAll_[0],
            sasquishOutputHandler_);
        feedThrd.detach();
    }
    sleep(1);
