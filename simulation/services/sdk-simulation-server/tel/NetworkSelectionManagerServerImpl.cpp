...
grpc::Status NetworkSelectionManagerServerImpl::RequestNetworkSelectionMode(ServerContext *context,
    ...
    if (data.status == telux::common::Status::SUCCESS) {
        int mode
            = data.stateRootObj[MANAGER]["NetworkSelectionMode"]["networkSelectionMode"].asInt();
...
grpc::Status NetworkSelectionManagerServerImpl::SetCoverageState(ServerContext *context,
    const ::telStub::SetCoverageStateRequest *request, ::telStub::SetCoverageStateReply *response) {
    LOG(DEBUG, __FUNCTION__);
...
grpc::Status NetworkSelectionManagerServerImpl::RequestPreferredNetworks(ServerContext *context,
    ...
    if (data.status == telux::common::Status::SUCCESS) {
        ...
        requestPreferredNetworks(
            request->phone_id(), preferredNetworks3gppInfo, staticPreferredNetworksInfo);
...
void NetworkSelectionManagerServerImpl::requestPreferredNetworks(int phoneId,
    ...
    for (int i = 0; i < 2; i++) {  // To fill the data of two JSON objects
        ...
        for (int j = 0; j < size; j++) {
            ...
            std::vector<int> data;
            std::string input
                = rootObj.stateRootObj[MANAGER][networkInfoType][j]["ratTypes"].asString();
...
void NetworkSelectionManagerServerImpl::handleNetworkScanResultsChanged(std::string eventParams) {
    ...
    try {
        ...
        networkScanResultsEvent.set_phone_id(phoneId);
        networkScanResultsEvent.set_status(static_cast<telStub::NetworkScanStatus>(scanStatus));
