...
grpc::Status ClientManagerServerImpl::InitService(ServerContext *context,
    ...
    std::string cbStatus = rootObj["IClientManager"]["IsSubsystemReady"].asString();
...
bool ClientManagerServerImpl::isMonitoringEnabled() {
    ...
    telux::common::ErrorCode err
        = JsonParser::readFromJsonFile(root, "system-state/data/IDataSettingsManagerState.json");

    if (err == telux::common::ErrorCode::SUCCESS) {
        bool enabled
            = root["IDataSettingsManager"]["isDeviceDataUsageMonitoringEnabled"]["enabled"].asBool();
...
grpc::Status ClientManagerServerImpl::GetDeviceDataUsageStats(ServerContext *context,
    const ::google::protobuf::Empty *request, dataStub::GetDeviceDataUsageStatsResponse *response) {
    ...
    if (!enabled) {
        ...
        return grpc::Status(
            grpc::StatusCode::INTERNAL, "isDeviceDataUsageMonitoringEnabled Disabled");
    } else {
...
grpc::Status ClientManagerServerImpl::ResetDataUsageStats(ServerContext *context,
...
void ClientManagerServerImpl::handleDeviceDataUsageReset(std::string event) {
    ...
    try {
        ...
        for (Json::Value::ArrayIndex i = stats.size(); i-- > 0;) {
            if (!stats[i].isMember("macAddress"))
                continue;
...
void ClientManagerServerImpl::handleDeviceDataUsageStatsUpdate(std::string event) {
    ...
    try {
        ...
        if (JsonParser::writeToJsonFile(root, CLIENT_MANAGER_STATE_JSON)
            != telux::common::ErrorCode::SUCCESS) {
            LOG(ERROR, __FUNCTION__, "Failed to write updated stats to JSON");
        } else {
