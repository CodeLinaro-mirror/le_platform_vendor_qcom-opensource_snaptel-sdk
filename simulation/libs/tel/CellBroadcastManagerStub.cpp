...
telux::common::Status CellBroadcastManagerStub::init(telux::common::InitResponseCb callback) {
    LOG(DEBUG, __FUNCTION__);
...
void CellBroadcastManagerStub::invokeInitResponseCallback(
...
telux::common::Status CellBroadcastManagerStub::registerListener(
...
telux::common::Status CellBroadcastManagerStub::updateMessageFilters(
    ...
    if (callback) {
        auto f1 = std::async(std::launch::async, [this, error, callback]() {
...
telux::common::Status CellBroadcastManagerStub::requestMessageFilters(
    ...
    telux::common::ErrorCode error           = telux::common::ErrorCode::SUCCESS;
    if (callback) {
        auto f1 = std::async(std::launch::async, [this, error, callback, filters]() {
...
void CellBroadcastManagerStub::invokeCallback(RequestFiltersResponseCallback callback,
    ...
    std::this_thread::sleep_for(std::chrono::milliseconds(DELAY));
    if (callback) {
        auto f = std::async(std::launch::async, [this, error, filters, callback]() {
...
telux::common::Status CellBroadcastManagerStub::setActivationStatus(
    bool activate, telux::common::ResponseCallback callback) {
    if (telux::common::ServiceStatus::SERVICE_AVAILABLE != getServiceStatus()) {
        LOG(ERROR, __FUNCTION__, " CellBroadCastManager is not ready");
        return telux::common::Status::NOTREADY;
    }
...
telux::common::Status CellBroadcastManagerStub::requestActivationStatus(
    ...
    telux::common::ErrorCode error = telux::common::ErrorCode::SUCCESS;
    if (callback) {
        auto f1 = std::async(std::launch::async, [this, error, callback, isActivated]() {
...
void CellBroadcastManagerStub::invokeCallback(RequestActivationStatusResponseCallback callback,
    ...
    std::this_thread::sleep_for(std::chrono::milliseconds(DELAY));
    if (callback) {
        auto f = std::async(std::launch::async, [this, error, isActivated, callback]() {
...
void CellBroadcastManagerStub::invokeCallback(
    telux::common::ResponseCallback callback, telux::common::ErrorCode error) {
    std::this_thread::sleep_for(std::chrono::milliseconds(DELAY));
    if (callback) {
