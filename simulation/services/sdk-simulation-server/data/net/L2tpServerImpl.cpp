...
grpc::Status L2tpServerImpl::InitService(ServerContext *context,
    ...
    std::string cbStatus                = rootObj["IL2tpManager"]["IsSubsystemReady"].asString();
...
grpc::Status L2tpServerImpl::SetConfig(ServerContext *context,
    ...
    if (data.status == telux::common::Status::SUCCESS
        ...
        data.stateRootObj[subsystem]["l2tpConfig"]["enable"]       = request->enable_config();
        data.stateRootObj[subsystem]["l2tpConfig"]["enableMtu"]    = request->enable_mtu();
        data.stateRootObj[subsystem]["l2tpConfig"]["enableTcpMss"] = request->enable_mss();
        data.stateRootObj[subsystem]["l2tpConfig"]["mtuSize"]      = request->mtu_size();
...
grpc::Status L2tpServerImpl::RequestConfig(ServerContext *context,
    const google::protobuf::Empty *request, dataStub::RequestConfigReply *response) {
    ...
    if (data.status == telux::common::Status::SUCCESS
        && data.error == telux::common::ErrorCode::SUCCESS) {
        response->set_enable_mtu(data.stateRootObj[subsystem]["l2tpConfig"]["enableMtu"].asBool());
        response->set_enable_tcp_mss(
            data.stateRootObj[subsystem]["l2tpConfig"]["enableTcpMss"].asBool());
        response->set_mtu_size(data.stateRootObj[subsystem]["l2tpConfig"]["mtuSize"].asInt());

        int currentConfigCount = data.stateRootObj[subsystem]["l2tpConfig"]["tunnelConfigs"].size();
        int configIdx          = 0;
        for (; configIdx < currentConfigCount; configIdx++) {
            Json::Value requestedConfig
                = data.stateRootObj[subsystem]["l2tpConfig"]["tunnelConfigs"][configIdx];
            ...
            tunnelConfig->mutable_ip_family_type()->set_ip_family_type(
                (DataUtilsStub::convertIpFamilyStringToEnum(requestedConfig["ipType"].asString())));
            int currentSessionCount = requestedConfig["sessionConfig"].size();
            for (auto sessionIdx = 0; sessionIdx < currentSessionCount; sessionIdx++) {
                Json::Value requestedSession = requestedConfig["sessionConfig"][sessionIdx];
                ::dataStub::L2tpSessionConfig *sessionConfig = tunnelConfig->add_session_config();
...
grpc::Status L2tpServerImpl::AddTunnel(ServerContext *context,
    ...
    if (data.status == telux::common::Status::SUCCESS
        ...
        int currentCount = data.stateRootObj[subsystem]["l2tpConfig"]["tunnelConfigs"].size();
        do {
            ...
            if (currentCount > MAX_TUNNEL) {
                ...
                LOG(DEBUG, __FUNCTION__, " adding tunnel ", request->l2tp_tunnel_config().loc_id());
...
grpc::Status L2tpServerImpl::AddSession(ServerContext *context,
    ...
    if (data.status == telux::common::Status::SUCCESS
        ...
        Json::Value &sessionConfig
            = data.stateRootObj[subsystem]["l2tpConfig"]["tunnelConfigs"][tunnelIdx]["sessionConfi"
                                                                                     "g"];
        ...
        if (MAX_SESSION == currentSessionCount) {
            ...
            data.error = telux::common::ErrorCode::NOT_SUPPORTED;
        } else if (configExists(sessionConfig, request->session_config().loc_id(), sessionIdx)) {
            LOG(DEBUG, __FUNCTION__, " session with Id ", request->session_config().loc_id(),
                " already exist");
            data.error = telux::common::ErrorCode::NO_EFFECT;
        } else {
            // adding sessionconfig
            ...
            LOG(DEBUG, __FUNCTION__, " added session with Id ", request->session_config().loc_id());
            JsonParser::writeToJsonFile(data.stateRootObj, stateJsonPath);
        }
    }
...
grpc::Status L2tpServerImpl::RemoveSession(ServerContext *context,
    ...
    if (data.status == telux::common::Status::SUCCESS
        ...
        Json::Value &sessionConfig
            = data.stateRootObj[subsystem]["l2tpConfig"]["tunnelConfigs"][tunnelIdx]["sessionConfi"
                                                                                     "g"];
        if (!configExists(sessionConfig, request->session_id(), sessionIdx)) {
            ...
            LOG(DEBUG, __FUNCTION__, " removing session with Id ", request->session_id());
            sessionConfig
                = newRoot[subsystem]["l2tpConfig"]["tunnelConfigs"][tunnelIdx]["sessionConfig"];
        }
...
grpc::Status L2tpServerImpl::BindSessionToBackhaul(ServerContext *context,
    ...
    if (data.status == telux::common::Status::SUCCESS
        && data.error == telux::common::ErrorCode::SUCCESS) {
        Json::Value &bindings = data.stateRootObj[subsystem]["sessionToBackhaulBindings"];
        ...
        if (configExists(bindings, request->loc_id(), bindingIdx)) {
            ...
            newbinding["backhaul"] = DataUtilsStub::convertEnumToBackhaulPrefString(
                static_cast<::dataStub::BackhaulPreference>(request->backhaul_type()));
    ...
    if (data.status == telux::common::Status::SUCCESS
        ...
        if (!bindingExists(bindings, request, bindingIdx)) {
            ...
            for (; index < currentCount; index++) {
                ...
                newRoot[subsystem]["sessionToBackhaulBindings"][newCount] = bindings[index];
                newCount++;
            }
            bindings = newRoot[subsystem]["sessionToBackhaulBindings"];
            JsonParser::writeToJsonFile(data.stateRootObj, stateJsonPath);
        }
    }
...
grpc::Status L2tpServerImpl::QueryBindSessionToBackhaul(ServerContext *context,
    const dataStub::QueryBindSessionRequest *request, dataStub::QueryBindSessionReply *response) {
    ...
    if (data.status == telux::common::Status::SUCCESS
        ...
        int currentCount = data.stateRootObj[subsystem]["sessionToBackhaulBindings"].size();
        int configIdx    = 0;
        for (; configIdx < currentCount; configIdx++) {
            Json::Value requestedBinding
                = data.stateRootObj[subsystem]["sessionToBackhaulBindings"][configIdx];
...
bool L2tpServerImpl::bindingExists(
    const Json::Value &bindings, const dataStub::SessionConfigRequest *request, int &idx) {
    bool isFound = false;
    ...
    for (idx = 0; idx < count; idx++) {
        ...
        if (request->backhaul_type()
                == static_cast<dataStub::BackhaulPreference>(telux::data::BackhaulType::WWAN)
        ...
        if (request->backhaul_type()
                == static_cast<dataStub::BackhaulPreference>(telux::data::BackhaulType::ETH)
