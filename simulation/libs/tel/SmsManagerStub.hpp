...
class SmsManagerStub : public ISmsManager,
 ...
 public:
    ...
    telux::common::Status sendRawSmsEx(
        const std::vector<PduBuffer> rawPdus, SmsResponseCbEx sentCallback = nullptr) override;

    telux::common::Status requestSmscAddress(
        std::shared_ptr<ISmscAddressCallback> callback = nullptr) override;

    telux::common::Status setSmscAddress(const std::string &smscAddress,
        telux::common::ResponseCallback callback = nullptr) override;

    telux::common::Status requestSmsMessageList(SmsTagType type, RequestSmsInfoListCb callback,
    ...
    telux::common::Status deleteMessage(
        DeleteInfo info, telux::common::ResponseCallback callback = nullptr) override;

    telux::common::Status requestPreferredStorage(RequestPreferredStorageCb callback) override;

    telux::common::Status setPreferredStorage(
        StorageType storageType, telux::common::ResponseCallback callback = nullptr) override;

    telux::common::Status setTag(uint32_t msgIndex, SmsTagType tagType,
        telux::common::ResponseCallback callback = nullptr,
        StorageType storageType                  = StorageType::SIM) override;

    telux::common::Status requestStorageDetails(
        RequestStorageDetailsCb callback, StorageType storageType = StorageType::SIM) override;
 ...
 private:
    ...
    void invokeReadSmsMessageCb(
        SmsMessage message, telux::common::ErrorCode error, ReadSmsMessageCb callback, int cbDelay);
    void invokeRequestPreferredStorageCb(telux::tel::StorageType storageType, int cbDelay,
        telux::common::ErrorCode error, RequestPreferredStorageCb callback);
    void invokeRequestStorageDetailsCb(int maxCount, int availableCount, int cbDelay,
    ...
    void invokeIncomingSmslisteners(int phoneId, std::shared_ptr<std::vector<SmsMessage>> messages);
