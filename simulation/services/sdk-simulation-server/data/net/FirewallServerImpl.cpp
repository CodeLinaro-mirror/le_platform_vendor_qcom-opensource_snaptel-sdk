...
grpc::Status FirewallServerImpl::InitService(ServerContext *context,
    ...
    std::string cbStatus = rootObj["IFirewallManager"]["IsSubsystemReady"].asString();
...
grpc::Status FirewallServerImpl::SetFirewall(ServerContext *context,
    ...
    if (data.status == telux::common::Status::SUCCESS
        ...
        if (isFound) {
            data.stateRootObj[subsystem]["firewallConfig"][idx]["enable"] = request->fw_enable();
        ...
        if (isFound) {
            ...
            for (const auto &protocol : protocols) {
                ...
                if (ipFamily == "IPV4") {
                    ...
                    newConfig["ipv4_srcSubnetMask"] = request->ipv4_params().ipv4_src_subnet_mask();
                ...
                if (ipFamily == "IPV6") {
                    ...
                    newConfig["ipv6_srcPrefixLen"] = request->ipv6_params().ipv6_src_prefix_len();
                    newConfig["ipv6_destAddr"]     = request->ipv6_params().ipv6_dest_address();
                    newConfig["ipv6_dstPrefixLen"] = request->ipv6_params().ipv6_dest_prefix_len();
...
grpc::Status FirewallServerImpl::RemoveFirewallEntry(ServerContext *context,
    ...
    if (data.status == telux::common::Status::SUCCESS
        ...
        if (isFound) {
            LOG(DEBUG, __FUNCTION__, " removing fw entry for handle::", request->entry_handle());
            int currentCount = data.stateRootObj[subsystem]["firewallEntry"].size();
            int newCount     = 0;
            int index        = 0;
            Json::Value newRoot;
            ...
            data.stateRootObj[subsystem]["firewallEntry"] = newRoot[subsystem]["firewallEntry"];
            JsonParser::writeToJsonFile(data.stateRootObj, stateJsonPath);
        } else {
...
grpc::Status FirewallServerImpl::RequestFirewallEntries(ServerContext *context,
    ...
    if (data.status == telux::common::Status::SUCCESS
        ...
        if (isFound) {
            int count = data.stateRootObj[subsystem]["firewallEntry"].size();
            int index = 0;
            for (; index < count; index++) {
                ...
                if ((requestedFirewallEntry["profileId"].asInt() == profile_id)
                    ...
                    fw_entry->set_firewall_handle(requestedFirewallEntry["handle"].asInt());
                    LOG(DEBUG, __FUNCTION__,
                        " found fw entry for handle::", requestedFirewallEntry["handle"].asInt());
                } else {
    ...
    if (data.status == telux::common::Status::SUCCESS
        ...
        if (isFound) {
            ...
            if (ipFamily == "IPV6") {
                ...
                int currentCount = data.stateRootObj[subsystem]["dmzConfig"].size();
                int newCount     = 0;
                int index        = 0;
                Json::Value newRoot;
                for (; index < currentCount; index++) {
                ...
                data.stateRootObj[subsystem]["dmzConfig"] = newRoot[subsystem]["dmzConfig"];
                JsonParser::writeToJsonFile(data.stateRootObj, stateJsonPath);
            }
        } else {
            data.error = telux::common::ErrorCode::INTERNAL;
...
grpc::Status FirewallServerImpl::RequestDMZEntry(ServerContext *context,
    const dataStub::DMZEntryRequest *request, dataStub::RequestDMZEntryReply *response) {
    LOG(DEBUG, __FUNCTION__);
    std::string apiJsonPath   = FIREWALL_MANAGER_API_LOCAL_JSON;
    std::string stateJsonPath = FIREWALL_MANAGER_STATE_JSON;
    std::string subsystem     = "IFirewallManager";
