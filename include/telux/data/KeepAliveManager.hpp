...
public:
     ...
     * The TCP client shall not send or acknowledge any packets after
     * the @ref startTCPKeepAliveOffload API is called, or else the keep-alive offload to the modem
     * will stop, and this will be reported via @ref onKeepAliveStatusChange.
     *
     * When the TCP keep-alive is offloaded to the modem, it does not support any TCP options in
     * the header, i.e., the keep-alive is sent without any options set. To ensure that keepalive
     * offload functions as expected, the SO_KEEPALIVE socket option must be set.
     *
     * On platforms with access control enabled, the caller needs to have the
     * TELUX_DATA_KA_OFFLOAD_OPS permission to successfully invoke this API.
     *
     ...
     * the @ref startTCPKeepAliveOffload API is called, or else the keep-alive offload to the modem
     * will stop, and this will be reported via @ref onKeepAliveStatusChange.
     * Once a monitor handle ( @ref MonitorHandleType) is used to invoke
     * @ref startTCPKeepAliveOffload, the same handle cannot be used again to start the TCP
     * keep-alive offload (via @ref startTCPKeepAliveOffload). A new handle should be created using
     * @ref enableTCPMonitor and used to invoke @ref startTCPKeepAliveOffload.
     *
     * When the TCP keep-alive is offloaded to the modem, it does not support any TCP options in
     * the header, i.e., the keep-alive is sent without any options set. To ensure that keepalive
     * offload functions as expected, the SO_KEEPALIVE socket option must be set.
    ...
    /**
     * Registers with the @ref IKeepAliveManager as a listener to receive TCP connection
...
public:
    ...
    IKeepAliveListener() {}
    /**
     * This function is called when there is an update in the keep-alive status. The modem can stop
     * sending keep-alive messages as a result of network failure.
     *
