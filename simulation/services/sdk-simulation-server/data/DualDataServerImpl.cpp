...
void DualDataServerImpl::handleDdsSwitchRecommendationChangeRequest(std::string event) {
    ...
    std::string token;
    while (!(token = EventParserUtil::getNextToken(currentEventString, DEFAULT_DELIMITER)).empty()) {
    ...
    JsonData data;
    telux::common::ErrorCode jsonReadError =
    ...
    bool enableTemporary = data.stateRootObj["IDualDataManager"]["ddsSwitchRecommendationConfig"]
                               ["enableTemporaryRecommendations"].asBool();
    bool enablePermanent = data.stateRootObj["IDualDataManager"]["ddsSwitchRecommendationConfig"]
                               ["enablePermanentRecommendations"].asBool();
...
::dataStub::DdsInfo::DdsType DualDataServerImpl::
    convertDdsTypeStringToEnum(const std::string &ddsTypeStr) {
    if (ddsTypeStr == "PERMANENT") return ::dataStub::DdsInfo::PERMANENT;
    if (ddsTypeStr == "TEMPORARY") return ::dataStub::DdsInfo::TEMPORARY;
    return ::dataStub::DdsInfo::UNKNOWN;
}

grpc::Status DualDataServerImpl::ConfigureDdsSwitchRecommendation(
    ...
    if (data.error == telux::common::ErrorCode::SUCCESS) {
        data.stateRootObj["IDualDataManager"]["ddsSwitchRecommendationConfig"]
            ["enableTemporaryRecommendations"] = enableTemporary;
        data.stateRootObj["IDualDataManager"]["ddsSwitchRecommendationConfig"]
            ["enablePermanentRecommendations"] = enablePermanent;
...
::dataStub::TemporaryRecommendationCauseCodes
    DualDataServerImpl::convertTemporaryCauseStringToEnum(const std::string &causeStr) {
    ...
    while (!(token = EventParserUtil::getNextToken(mutableCauseStr, COMMA_DELIMITER)).empty()) {
        if (token == "TEMP_CAUSE_CODE_UNKNOWN") {
            tempCause = static_cast<::dataStub::TemporaryRecommendationCauseCodes>(
                tempCause | ::dataStub::TemporaryRecommendationCauseCodes::
                    TEMP_CAUSE_CODE_UNKNOWN);
...
::dataStub::PermanentRecommendationCauseCodes
    DualDataServerImpl::convertPermanentCauseStringToEnum(const std::string &causeStr) {
    ...
    while (!(token = EventParserUtil::getNextToken(mutableCauseStr, COMMA_DELIMITER)).empty()) {
        if (token == "PERM_CAUSE_CODE_UNKNOWN") {
            permCause = static_cast<::dataStub::PermanentRecommendationCauseCodes>(
                permCause | ::dataStub::PermanentRecommendationCauseCodes::
                    PERM_CAUSE_CODE_UNKNOWN);
            ...
            permCause = static_cast<::dataStub::PermanentRecommendationCauseCodes>(
                permCause | ::dataStub::PermanentRecommendationCauseCodes::
                    PERM_CAUSE_CODE_INTERNET_NOT_AVAIL);
        }
    }
    return permCause;
}
...
grpc::Status DualDataServerImpl::GetDdsSwitchRecommendation(grpc::ServerContext* context,
    ...
    if (!enableTemporary && !enablePermanent) {
        ...
        response->mutable_dds_switch_recommendation()->mutable_recommended_dds_info()->
            set_dds_type(::dataStub::DdsInfo::UNKNOWN);
