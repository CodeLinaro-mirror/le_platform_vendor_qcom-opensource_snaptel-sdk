...
telux::common::Status CallManagerStub::makeCall(
    int phoneId, const std::string &dialNumber, std::shared_ptr<IMakeCallCallback> callback) {
    ...
    if (reqstatus.ok()) {
        ...
        if (status == telux::common::Status::SUCCESS) {
            if (inputApi == CallApi::makeAecsVoiceCall) {
                findMatchingCall(
                    callIndex, remotePartyNumber, phoneId, cbDelay, nullptr, cb, error);
            } else {
                // Update call details from server and invoke callback
                findMatchingCall(
                    callIndex, remotePartyNumber, phoneId, cbDelay, callback, nullptr, error);
            }
        } else {
            // Update call details from server
            findMatchingCall(
                callIndex, remotePartyNumber, phoneId, cbDelay, nullptr, nullptr, error);
        }
    }
    return status;
}
...
void CallManagerStub::findMatchingCall(int index, std::string remotePartyNumber, int phoneId,
    ...
    if (iter != std::end(calls_)) {
        ...
        if (iMakecallback) {
            auto f = std::async(
                std::launch::async, [this, error, iter, iMakecallback, cbDelay, value]() {
        ...
        if (callback) {
            auto f
                = std::async(std::launch::async, [this, error, iter, callback, cbDelay, value]() {
                      std::this_thread::sleep_for(std::chrono::milliseconds(cbDelay));
                      LOG(DEBUG, __FUNCTION__, " invoking callback");
...
bool CallManagerStub::find(
    int phoneId, std::shared_ptr<CallStub> call, std::string remotePartyNumber, int index) {
...
telux::common::Status CallManagerStub::makeECall(int phoneId, const std::string dialNumber,
    ...
    ::telStub::MakeECallRequest request = createRequest<::telStub::MakeECallRequest>(
        phoneId, dialNumber, true, makeTpsECallOverCSWithMsd);
...
telux::common::Status CallManagerStub::makeECall(int phoneId, const ECallMsdData &eCallMsdData,
    ...
    ::telStub::MakeECallRequest request
        = createRequest<::telStub::MakeECallRequest>(phoneId, "", true, makeECallWithMsd);
    ::telStub::MakeECallReply response;
    ClientContext context;
...
void CallManagerStub::logCallDetails(std::shared_ptr<ICall> info) {
    LOG(DEBUG, __FUNCTION__, " Call Info: remotePartyNumber = ", info->getRemotePartyNumber(),
...
void CallManagerStub::handleCallInfoChanged(::telStub::GetInProgressCallsData event) {
    int phoneId = event.phone_id();
    LOG(DEBUG, __FUNCTION__, " phoneId ", phoneId);
    std::vector<std::shared_ptr<CallStub>> calls;
    for (int i = 0; i < event.calls_size(); i++) {
        CallInfo callInfo;
        callInfo.callState = static_cast<telux::tel::CallState>(event.calls(i).call_state());
        ...
        callInfo.callDirection
            = static_cast<telux::tel::CallDirection>(event.calls(i).call_direction());
        callInfo.remotePartyNumber = static_cast<std::string>(event.calls(i).remote_party_number());
    ...
    AecsCallEndReason aecsReason = static_cast<telux::tel::AecsCallEndReason>(event.reason());
...
void CallManagerStub::refreshCachedCalls(int phoneId,
    std::vector<std::shared_ptr<CallStub>> &latestCalls, CallEndCause endCause,
...
void CallManagerStub::notifyAndRemoveDroppedCalls(
    CallEndCause causeCode, AecsCallEndReason reason) {
    std::vector<std::shared_ptr<CallStub>> callsToBeNotified;
    LOG(DEBUG, __FUNCTION__, " causeCode: ", static_cast<int>(causeCode),
...
telux::common::Status CallManagerStub::makeECall(int phoneId, const std::string dialNumber,
    ...
    ::telStub::MakeECallRequest request = createRequest<::telStub::MakeECallRequest>(
        phoneId, dialNumber, true, makeTpsECallOverIMS);
    ::telStub::MakeECallReply response;
    ClientContext context;
    ...
    if (reqstatus.ok()) {
        ...
        if (status == telux::common::Status::SUCCESS) {
            // Update call details from server and invoke callback
            findMatchingCall(
                callIndex, remotePartyNumber, phoneId, cbDelay, nullptr, callback, error);
        } else {
            // Update call details from server
            findMatchingCall(
                callIndex, remotePartyNumber, phoneId, cbDelay, nullptr, nullptr, error);
...
telux::common::Status CallManagerStub::makeECall(int phoneId, const std::vector<uint8_t> &msdPdu,
    int category, int variant, MakeCallCallback callback, TestECallConfig config) {
    LOG(DEBUG, "CallManager - ", __FUNCTION__);
    if (phoneId <= 0 || phoneId > noOfSlots_) {
        LOG(DEBUG, __FUNCTION__, " Invalid PhoneId");
        return telux::common::Status::INVALIDPARAM;
    ...
    ::telStub::MakeECallRequest request
        = createRequest<::telStub::MakeECallRequest>(phoneId, "", true, makeECallWithRawMsd);
    ::telStub::MakeECallReply response;
    ClientContext context;
    grpc::Status reqstatus       = stub_->MakeECall(&context, request, &response);
    telux::common::Status status = telux::common::Status::FAILED;
...
telux::common::Status CallManagerStub::makeECall(int phoneId, const std::string dialNumber,
    ...
    ::telStub::MakeECallRequest request = createRequest<::telStub::MakeECallRequest>(
        phoneId, dialNumber, true, makeTpsECallOverCSWithRawMsd);
    ::telStub::MakeECallReply response;
    ClientContext context;
...
telux::common::Status CallManagerStub::makeECall(
    int phoneId, int category, int variant, MakeCallCallback callback, TestECallConfig config) {
    LOG(DEBUG, "CallManager - ", __FUNCTION__);
    ...
    ::telStub::MakeECallRequest request
        = createRequest<::telStub::MakeECallRequest>(phoneId, "", false, makeECallWithoutMsd);
    ::telStub::MakeECallReply response;
    ClientContext context;
...
telux::common::Status CallManagerStub::makeECall(
    int phoneId, const std::string dialNumber, int category, MakeCallCallback callback) {
    LOG(DEBUG, "CallManager - ", __FUNCTION__);
    if (phoneId <= 0 || phoneId > noOfSlots_) {
        LOG(DEBUG, __FUNCTION__, " Invalid PhoneId");
        return telux::common::Status::INVALIDPARAM;
    ...
    ::telStub::MakeCallRequest request = createRequest<::telStub::MakeCallRequest>(
        phoneId, dialNumber, false, makeTpsECallOverCSWithoutMsd);
    ::telStub::MakeCallReply response;
    ClientContext context;
...
telux::common::Status CallManagerStub::requestECallHlapTimerStatus(
    ...
    if (reqstatus.ok()) {
        ...
        timersStatus.t2
            = static_cast<telux::tel::HlapTimerStatus>((response.hlap_timer_status()).t2());
        timersStatus.t5
            = static_cast<telux::tel::HlapTimerStatus>((response.hlap_timer_status()).t5());
        timersStatus.t6
            = static_cast<telux::tel::HlapTimerStatus>((response.hlap_timer_status()).t6());
        timersStatus.t7
            = static_cast<telux::tel::HlapTimerStatus>((response.hlap_timer_status()).t7());
        timersStatus.t9
            = static_cast<telux::tel::HlapTimerStatus>((response.hlap_timer_status()).t9());
        timersStatus.t10
            = static_cast<telux::tel::HlapTimerStatus>((response.hlap_timer_status()).t10());
        int cbDelay = static_cast<int>(response.delay());
        if (callback) {
            auto f = std::async(
                std::launch::async, [this, error, phoneId, timersStatus, callback, cbDelay]() {
                    std::this_thread::sleep_for(std::chrono::milliseconds(cbDelay));
...
telux::common::Status CallManagerStub::swap(std::shared_ptr<ICall> callToHold,
    ...
    if (reqstatus.ok()) {
        ...
        if ((status == telux::common::Status::SUCCESS) && (isCallbackNeeded)) {
            if (callback) {
                auto f = std::async(std::launch::async, [this, error, callback, delay]() {
...
telux::common::Status CallManagerStub::hangupWaitingOrBackground(
    int phoneId, common::ResponseCallback callback) {
    LOG(DEBUG, __FUNCTION__, " SlotId: ", phoneId);
    if (phoneId <= 0 || phoneId > noOfSlots_) {
        LOG(DEBUG, __FUNCTION__, " Invalid PhoneId");
...
telux::common::Status CallManagerStub::requestEcbm(int phoneId, EcbmStatusCallback callback) {
    ...
    if (reqstatus.ok()) {
        ...
        if ((status == telux::common::Status::SUCCESS) && (isCallbackNeeded)) {
            if (callback) {
                auto f
                    = std::async(std::launch::async, [this, error, callback, ecbMode, cbDelay]() {
                          std::this_thread::sleep_for(std::chrono::milliseconds(cbDelay));
...
telux::common::Status CallManagerStub::exitEcbm(int phoneId, common::ResponseCallback callback) {
    ...
    if (reqstatus.ok()) {
        ...
        bool isCallbackNeeded          = static_cast<bool>(response.iscallback());
        if ((status == telux::common::Status::SUCCESS) && (isCallbackNeeded)) {
            auto f = std::async(std::launch::async, [this, error, cbDelay, callback]() {
                std::this_thread::sleep_for(std::chrono::milliseconds(cbDelay));
                callback(error);
            }).share();
            taskQ_->add(f);
...
telux::common::Status CallManagerStub::updateEcallHlapTimer(
    int phoneId, HlapTimerType type, uint32_t timeDuration, common::ResponseCallback callback) {
    LOG(DEBUG, __FUNCTION__, " phoneId:", phoneId);
    if (phoneId <= 0 || phoneId > noOfSlots_) {
        LOG(ERROR, __FUNCTION__, " Invalid PhoneId");
        return telux::common::Status::INVALIDPARAM;
...
telux::common::Status CallManagerStub::requestEcallHlapTimer(
    int phoneId, HlapTimerType type, ECallHlapTimerCallback callback) {
    LOG(DEBUG, __FUNCTION__, " phoneId:", phoneId);
    if (phoneId <= 0 || phoneId > noOfSlots_) {
        LOG(ERROR, __FUNCTION__, " Invalid PhoneId");
        return telux::common::Status::INVALIDPARAM;
    ...
    if (type == HlapTimerType::T10_TIMER) {
        ...
        if (reqstatus.ok()) {
            ...
            if ((status == telux::common::Status::SUCCESS) && (isCallbackNeeded)) {
                if (callback) {
                    auto f = std::async(
                        std::launch::async, [this, error, callback, cbDelay, timeDuration]() {
                            std::this_thread::sleep_for(std::chrono::milliseconds(cbDelay));
...
telux::common::Status CallManagerStub::setECallConfig(EcallConfig config) {
    ...
    if (validityMask.test(ECALL_CONFIG_NUM_TYPE)) {
        ...
        if (config.numType == ECallNumType::DEFAULT) {
            request.set_num_type(
                static_cast<::telStub::ECallNumType>(telux::tel::ECallNumType::DEFAULT));
        } else {
            request.set_num_type(
                static_cast<::telStub::ECallNumType>(telux::tel::ECallNumType::OVERRIDDEN));
        }
    }
...
telux::common::ErrorCode CallManagerStub::encodeECallMsd(
    telux::tel::ECallMsdData eCallMsdData, std::vector<uint8_t> &data) {
    LOG(DEBUG, __FUNCTION__);
...
telux::common::Status CallManagerStub::makeRttCall(
    int phoneId, const std::string &dialNumber, std::shared_ptr<IMakeCallCallback> callback) {
    LOG(DEBUG, __FUNCTION__, " Phone Id ", phoneId, " dial number ", dialNumber);
...
telux::common::Status CallManagerStub::sendRtt(
    int phoneId, std::string message, common::ResponseCallback callback) {
...
telux::common::Status CallManagerStub::configureECallRedial(
    RedialConfigType config, const std::vector<int> &timeGap, common::ResponseCallback callback) {
    if (getServiceStatus() != telux::common::ServiceStatus::SERVICE_AVAILABLE) {
        LOG(ERROR, __FUNCTION__, " CallManager is not ready ");
        return telux::common::Status::NOTREADY;
    }
...
telux::common::Status CallManagerStub::restartECallHlapTimer(
    int phoneId, EcallHlapTimerId timerId, int duration, common::ResponseCallback callback) {
    if (getServiceStatus() != telux::common::ServiceStatus::SERVICE_AVAILABLE) {
        LOG(ERROR, __FUNCTION__, " CallManager is not ready ");
        return telux::common::Status::NOTREADY;
    }
    ...
    grpc::Status reqstatus = stub_->restartECallHlapTimer(&context, request, &response);
    if (reqstatus.ok()) {
        telux::common::ErrorCode error = static_cast<telux::common::ErrorCode>(response.error());
        status                         = static_cast<telux::common::Status>(response.status());
        int cbDelay                    = static_cast<int>(response.delay());
...
telux::common::Status CallManagerStub::updateECallPostTestRegistrationTimer(
    ...
    grpc::Status reqstatus = stub_->updateECallPostTestRegistration(&context, request, &response);
    if (reqstatus.ok()) {
        telux::common::ErrorCode error = static_cast<telux::common::ErrorCode>(response.error());
        status                         = static_cast<telux::common::Status>(response.status());
        int cbDelay                    = static_cast<int>(response.delay());
...
telux::common::ErrorCode CallManagerStub::getECallPostTestRegistrationTimer(
    int phoneId, uint32_t &timer) {
    if (getServiceStatus() != telux::common::ServiceStatus::SERVICE_AVAILABLE) {
        LOG(ERROR, __FUNCTION__, " CallManager is not ready ");
    ...
    request.set_phone_id(phoneId);
    grpc::Status reqstatus = stub_->getECallPostTestRegistrationTimer(&context, request, &response);
    if (reqstatus.ok()) {
...
telux::common::ErrorCode CallManagerStub::getECallRedialConfig(
    std::vector<int> &callOrigTimeGap, std::vector<int> &callDropTimeGap) {
    if (getServiceStatus() != telux::common::ServiceStatus::SERVICE_AVAILABLE) {
        LOG(ERROR, __FUNCTION__, " CallManager is not ready ");
        return telux::common::ErrorCode::INVALID_STATE;
    }
    ...
    ClientContext context;
    telux::common::ErrorCode errorCode = telux::common::ErrorCode::GENERIC_FAILURE;
    grpc::Status reqstatus             = stub_->getECallRedialConfig(&context, request, &response);
    if (reqstatus.ok()) {
        errorCode = static_cast<telux::common::ErrorCode>(response.error());
...
telux::common::Status CallManagerStub::makeAecsCall(
    int phoneId, const std::string &eCallIdentifier, MakeCallCallback callback) {
    LOG(DEBUG, __FUNCTION__, " Phone Id ", phoneId, " dial number ", eCallIdentifier);
    telux::common::Status status
        = dialCall(phoneId, eCallIdentifier, nullptr, callback, makeAecsVoiceCall);
    return status;
}
