    ...
    do {
        ...
        if (status != telux::common::Status::SUCCESS && status != telux::common::Status::ALREADY) {
            LOG(ERROR, __FUNCTION__,
                " Failed to register for default notifications, error: ", static_cast<int>(status));
            break;
        }
        ...
        if (firstReg.test(ServingSystemNotificationType::SYSTEM_INFO)) {
            status = clientEventManager.registerListener(
                shared_from_this(), {telux::tel::TEL_SERVING_SYSTEM_INFO});
            if ((status != telux::common::Status::SUCCESS)
                && (status != telux::common::Status::ALREADY)) {
        ...
        if (firstReg.test(ServingSystemNotificationType::RF_BAND_INFO)) {
            status = clientEventManager.registerListener(
                shared_from_this(), {telux::tel::TEL_SERVING_SYSTEM_RF_BAND_INFO});
            if ((status != telux::common::Status::SUCCESS)
                && (status != telux::common::Status::ALREADY)) {
        ...
        if (firstReg.test(ServingSystemNotificationType::NETWORK_REJ_INFO)) {
            status = clientEventManager.registerListener(
                shared_from_this(), {telux::tel::TEL_SERVING_SYSTEM_NETWORK_REJ_INFO});
            if ((status != telux::common::Status::SUCCESS)
                && (status != telux::common::Status::ALREADY)) {
        ...
        if (firstReg.test(ServingSystemNotificationType::ARFCN_INFO)) {
            status = clientEventManager.registerListener(
                shared_from_this(), {telux::tel::TEL_SERVING_SYSTEM_ARFCN_INFO});
            if ((status != telux::common::Status::SUCCESS)
                && (status != telux::common::Status::ALREADY)) {
...
telux::common::Status ServingSystemManagerStub::deregisterListener(
    ...
    do {
        ...
        auto &clientEventManager = telux::common::ClientEventManager::getInstance();
        status                   = clientEventManager.deregisterListener(
            shared_from_this(), {telux::tel::TEL_SERVING_SYSTEM_SELECTION_PREF,
...
telux::common::Status ServingSystemManagerStub::setRatPreference(
    RatPreference ratPref, common::ResponseCallback callback) {
    LOG(DEBUG, __FUNCTION__);
    if (getServiceStatus() != telux::common::ServiceStatus::SERVICE_AVAILABLE) {
        LOG(ERROR, __FUNCTION__, " Service Status is UNAVAILABLE");
...
telux::common::Status ServingSystemManagerStub::requestRatPreference(
    RatPreferenceCallback callback) {
    LOG(DEBUG, __FUNCTION__);
    if (getServiceStatus() != telux::common::ServiceStatus::SERVICE_AVAILABLE) {
        LOG(ERROR, __FUNCTION__, " Service Status is UNAVAILABLE");
        return telux::common::Status::NOTREADY;
    ...
    int cbDelay                    = static_cast<int>(response.delay());
    if ((status == telux::common::Status::SUCCESS) && (isCallbackNeeded)) {
        auto f = std::async(std::launch::async, [this, cbDelay, preference, error, callback]() {
            std::this_thread::sleep_for(std::chrono::milliseconds(cbDelay));
            if (callback) {
...
telux::common::Status ServingSystemManagerStub::getCallBarringInfo(
    ...
    for (int i = 0; i < response.barring_infos_size(); i++) {
        ...
        info.rat    = static_cast<telux::tel::RadioTechnology>(response.barring_infos(i).rat());
        info.domain = static_cast<telux::tel::ServiceDomain>(response.barring_infos(i).domain());
        info.callType
            = static_cast<telux::tel::CallsAllowedInCell>(response.barring_infos(i).call_type());
...
telux::common::Status ServingSystemManagerStub::getSmsCapabilityOverNetwork(
    ...
    smsCapability.domain         = static_cast<telux::tel::SmsDomain>(response.domain());
...
telux::common::Status ServingSystemManagerStub::getLteCsCapability(LteCsCapability &lteCapability) {
    LOG(DEBUG, __FUNCTION__);
    if (getServiceStatus() != telux::common::ServiceStatus::SERVICE_AVAILABLE) {
        LOG(ERROR, __FUNCTION__, " Service Status is UNAVAILABLE");
        return telux::common::Status::NOTREADY;
    ...
    lteCapability                = static_cast<telux::tel::LteCsCapability>(response.capability());
    telux::common::Status status = static_cast<telux::common::Status>(response.status());
    return status;
}
...
telux::common::Status ServingSystemManagerStub::requestRFBandPreferences(
    ...
    int cbDelay                    = static_cast<int>(response.delay());
    if ((status == telux::common::Status::SUCCESS) && (isCallbackNeeded)) {
        auto f = std::async(std::launch::async, [this, cbDelay, prefBands, error, callback]() {
            std::this_thread::sleep_for(std::chrono::milliseconds(cbDelay));
            if (callback) {
...
telux::common::Status ServingSystemManagerStub::requestRFBandCapability(
    ...
    if ((status == telux::common::Status::SUCCESS) && (isCallbackNeeded)) {
        auto f
            = std::async(std::launch::async, [this, cbDelay, capabilityBands, error, callback]() {
                  std::this_thread::sleep_for(std::chrono::milliseconds(cbDelay));
                  if (callback) {
...
telux::common::ErrorCode ServingSystemManagerStub::getHplmnSearchTime(uint32_t &time) {
    LOG(DEBUG, __FUNCTION__);
    if (getServiceStatus() != telux::common::ServiceStatus::SERVICE_AVAILABLE) {
        LOG(ERROR, __FUNCTION__, " Service Status is UNAVAILABLE");
        return telux::common::ErrorCode::INVALID_STATE;
...
NetworkMode ServingSystemManagerStub::updateNetworkMode(
...
telux::common::Status ServingSystemManagerStub::requestRrcState(RrcStateCallback callback) {
    ...
    int cbDelay                    = static_cast<int>(response.delay());
    if ((status == telux::common::Status::SUCCESS) && (isCallbackNeeded)) {
        auto f = std::async(std::launch::async, [this, cbDelay, rrcState, error, callback]() {
            std::this_thread::sleep_for(std::chrono::milliseconds(cbDelay));
            if (callback) {
...
void ServingSystemManagerStub::handleSystemInfoChanged(::telStub::SystemInfoEvent event) {
    ...
    for (int i = 0; i < event.barring_infos_size(); i++) {
        CallBarringInfo info;
        info.rat    = static_cast<telux::tel::RadioTechnology>(event.barring_infos(i).rat());
        info.domain = static_cast<telux::tel::ServiceDomain>(event.barring_infos(i).domain());
        info.callType
            = static_cast<telux::tel::CallsAllowedInCell>(event.barring_infos(i).call_type());
