...
class VlanServerImpl final : public dataStub::VlanManager::Service {
 public:
    VlanServerImpl();
    ~VlanServerImpl();
    ...
    grpc::Status CreateVlan(ServerContext *context, const dataStub::CreateVlanRequest *request,
        dataStub::CreateVlanReply *response) override;

    grpc::Status RemoveVlan(ServerContext *context, const dataStub::RemoveVlanRequest *request,
    ...
    grpc::Status BindToBackhaul(ServerContext *context,
        const dataStub::BindToBackhaulConfig *request, dataStub::DefaultReply *response) override;
 ...
 private:
    ...
    template <typename T>
    bool isBindingConfigAvailable(std::string subsystem, std::string method, const JsonData &data,
        const T *request, int &configIdx = 0) {
        const Json::Value &config = data.stateRootObj[subsystem][method];
        int count                 = config.size();
        bool isFound              = false;
