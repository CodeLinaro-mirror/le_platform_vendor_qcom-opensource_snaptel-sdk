    ...
    if (error == telux::common::ErrorCode::SUCCESS) {
        ...
        for (auto it = sp->eCallDataMap_.begin(); it != sp->eCallDataMap_.end(); it++) {
            if (it->second.triggerHighCapSwitch
                && (it->second.msdTransmissionStatus
                    == telux::tel::ECallMsdTransmissionStatus::SUCCESS)) {
                ...
                if (status == telux::common::Status::SUCCESS) {
                    ...
                    std::cout << CLIENT_NAME << "Failed to initiate an Ecall on slot: " << it->first
                              << "\n";
                }
            } else {
                std::cout << CLIENT_NAME << "Slot: " << it->first << " MSD transmission status:"
                          << static_cast<int>(it->second.msdTransmissionStatus)
                          << " isTriggerHighCapSwitch: " << it->second.triggerHighCapSwitch << "\n";
            }
        }
    } else {
        std::cout << CLIENT_NAME
                  << "Set high capability request failed, errorCode: " << static_cast<int>(error)
...
telux::common::Status TelClient::EcallScanFailHandler::setHighCapability(int phoneId) {
    ...
    auto setHighCapabilityCallback
        = [&](telux::common::ErrorCode error) { setHighCapabilityResponse(error); };
...
void TelClient::EcallScanFailHandler::requestHighCapabilityResponse(
    int highCapSlotId, telux::common::ErrorCode error) {
    auto sp = eCallTelClient_.lock();
    if (!sp) {
    ...
    if (error == telux::common::ErrorCode::SUCCESS) {
            ...
            if (highCapSlotId != eCallNWScanFailedOnSlot) {
                ...
                if (sp->isECallInProgress()) {
                    if (sp->eCall_) {
                        ...
                        if (status != telux::common::Status::SUCCESS) {
                            std::cout << CLIENT_NAME
                                      << "Failed to Hangup the eCall on slot: " << eCallOnSlot
                                      << "\n";
                            return;
                        }
                    }
        ...
        std::cout << CLIENT_NAME
                  << "High capability request failed with errorCode: " << static_cast<int>(error)
                  << ", description: " << Utils::getErrorCodeAsString(error) << "\n";
        if (sp->isECallInProgress()) {
            if (sp->eCall_) {
