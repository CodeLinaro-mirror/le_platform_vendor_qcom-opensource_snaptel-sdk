...
int generateRSAKey(std::shared_ptr<telux::sec::ICryptoManager> cryptMgr, std::vector<uint8_t> &kb) {
    ...
    cp = telux::sec::CryptoParamBuilder()
             ...
             .setDigest(telux::sec::Digest::DIGEST_SHA_2_256 | telux::sec::Digest::DIGEST_SHA_2_512)
    ...
    if (ec != telux::common::ErrorCode::SUCCESS) {
        std::cout << "Can't generate RSA asym key, err: " << static_cast<int>(ec) << std::endl;
        return -1;
    }
...
int exportRSAKey(std::shared_ptr<telux::sec::ICryptoManager> cryptMgr, std::vector<uint8_t> keyBlob,
