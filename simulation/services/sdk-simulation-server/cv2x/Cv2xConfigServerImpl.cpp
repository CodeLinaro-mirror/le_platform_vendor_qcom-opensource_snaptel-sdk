...
Cv2xConfigServerImpl::Cv2xConfigServerImpl() {
...
grpc::Status Cv2xConfigServerImpl::initService(ServerContext *context,
    ...
    Json::Value rootNode;
    telux::common::ErrorCode errorCode = JsonParser::readFromJsonFile(rootNode, CV2X_CFG_API_JSON);
    if (errorCode == ErrorCode::SUCCESS) {
        cbDelay              = rootNode[CV2X_CFG_ROOT]["IsSubsystemReadyDelay"].asInt();
        std::string cbStatus = rootNode["ICv2xConfig"]["IsSubsystemReady"].asString();
...
grpc::Status Cv2xConfigServerImpl::updateConfiguration(ServerContext *context,
    const cv2xStub::Cv2xConfigPath *request, ::cv2xStub::Cv2xCommandReply *res) {
    LOG(DEBUG, __FUNCTION__, " cv2x config path: ", request->path());

    Cv2xServerUtil::apiJsonReader(CV2X_CFG_API_JSON, CV2X_CFG_ROOT, "updateConfiguration", res);
...
grpc::Status Cv2xConfigServerImpl::retrieveConfiguration(ServerContext *context,
    const cv2xStub::Cv2xConfigPath *request, ::cv2xStub::Cv2xCommandReply *res) {
    LOG(DEBUG, __FUNCTION__, " cv2x config path: ", request->path());

    Cv2xServerUtil::apiJsonReader(CV2X_CFG_API_JSON, CV2X_CFG_ROOT, "retrieveConfiguration", res);
