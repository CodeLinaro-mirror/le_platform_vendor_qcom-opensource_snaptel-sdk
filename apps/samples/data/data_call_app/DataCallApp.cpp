...
class DataConnectionManager
 ...
 public:
    bool init(SlotId slotId) {
        ...
        if (dataConnMgr_->registerListener(shared_from_this()) != telux::common::Status::SUCCESS) {
            return false;
        }
    ...
    void startDataCall(telux::data::OperationType opType = telux::data::OperationType::DATA_LOCAL) {
        ...
        std::promise<telux::common::ErrorCode> prom = std::promise<telux::common::ErrorCode>();
        {
            ...
            if (++dataCallAttempt_ > MAX_START_DATA_CALL_RETRY) {
                ...
                std::cout << "failed to start data call attempted " << MAX_START_DATA_CALL_RETRY
                          << " times" << std::endl;
                exit(1);
            }
            ...
            status                       = dataConnMgr_->startDataCall(dataCallParams,
                                      [this, &prom](const std::shared_ptr<telux::data::IDataCall> &dataCall,
        ...
        if (isProfileThrottled()) {
            ...
            std::this_thread::sleep_for(std::chrono::seconds(DATA_CALL_RETRY_TIMER));
    ...
    bool isProfileThrottled() {
        ...
        telux::common::Status status = dataConnMgr_->requestThrottledApnInfo(
            [&](const std::vector<telux::data::APNThrottleInfo> &throttleInfoList,
                ...
                std::cout << "startCallResponse: errorCode: " << static_cast<int>(error)
                          << std::endl;
        ...
        if (status == telux::common::Status::SUCCESS) {
            ...
            std::cout << "Error: failed to trigger requestThrottledApnInfo; status: "
