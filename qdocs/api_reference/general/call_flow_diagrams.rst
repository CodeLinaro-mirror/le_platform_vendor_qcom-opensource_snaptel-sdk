...
``IWlanControlManager`` is currently unavailable. ``WlanFactory::getWlanControlManager()``
returns a valid object whose ``getServiceStatus()`` always reports ``SERVICE_FAILED``.
The initialization callback is invoked with ``SERVICE_FAILED`` and no API operations
are available at this time.

1. Application requests ``IWlanControlManager`` object from ``WlanFactory`` using
   ``getWlanControlManager(initCb)``.
2. ``WlanFactory`` returns a shared pointer to ``IWlanControlManager``. The initialization
   callback is invoked asynchronously with ``SERVICE_FAILED``.
3. Application calls ``IWlanControlManager::getServiceStatus()``, receives ``SERVICE_FAILED``,
