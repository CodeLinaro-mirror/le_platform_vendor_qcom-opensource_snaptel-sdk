...
DataConnectionManagerStub::DataConnectionManagerStub(SlotId slotId) {
    LOG(DEBUG, __FUNCTION__,
        " Initializing DataConnectionManagerStub for slot:", static_cast<int>(slotId));
...
telux::common::Status DataConnectionManagerStub::init(telux::common::InitResponseCb callback) {
...
void DataConnectionManagerStub::requestConnectedDataCallLists() {
    ...
    for (auto idx = 0; idx < response.datacalls_size(); idx++) {
        ...
        call          = std::make_shared<telux::data::DataCallStub>(datacall->iface_name());
        ...
        IpFamilyType ipFamilyType = static_cast<IpFamilyType>(
            DataUtilsStub::convertIpFamilyStringToEnum(datacall->ip_family_type()));
...
telux::common::Status DataConnectionManagerStub::setDefaultProfile(
    OperationType oprType, uint8_t profileId, telux::common::ResponseCallback callback) {
    LOG(DEBUG, __FUNCTION__);
...
telux::common::Status DataConnectionManagerStub::getDefaultProfile(
    ...
    if (status == telux::common::Status::SUCCESS) {
        ...
        if (callback && (delay != SKIP_CALLBACK)) {
            auto f = std::async(
                std::launch::async, [this, error, profileId, slotID, delay, callback]() {
...
telux::common::Status DataConnectionManagerStub::setRoamingMode(bool enable, uint8_t profileId,
...
telux::common::Status DataConnectionManagerStub::requestRoamingMode(
    ...
    if (status == telux::common::Status::SUCCESS) {
        ...
        if (callback && (delay != SKIP_CALLBACK)) {
            auto f = std::async(
                std::launch::async, [this, mode, profile_Id, error, delay, callback]() {
                    std::this_thread::sleep_for(std::chrono::milliseconds(delay));
...
void DataConnectionManagerStub::invokeDataConnectionListener(std::shared_ptr<IDataCall> call) {
    LOG(DEBUG, __FUNCTION__);
...
void DataConnectionManagerStub::handleStartDataCallEvent(
    ...
    if (dataCalls_.find(profileId) != dataCalls_.end()) {
        ...
        call = std::make_shared<telux::data::DataCallStub>(ifaceName);
        call->setProfileId(profileId);
        call->setSlotId(slotId_);
        call->setIpFamilyType(ipFamilyType);
        // setting to defaults.
    ...
    std::shared_ptr<IDataCall> baseCallPtr = std::static_pointer_cast<IDataCall>(call);
    ...
    if (ipv4Supported || ipv6Supported) {
        ...
        if (ipFamilyType == IpFamilyType::IPV4 || ipFamilyType == IpFamilyType::IPV4V6) {
            ...
            ipAddrList.emplace_back(ipv6Addr);
            call->setIpAddrList(ipAddrList);
            auto f = std::async(std::launch::async, [this, baseCallPtr]() {
...
telux::common::Status DataConnectionManagerStub::startDataCall(
    ...
    if (dataCallParams.profileId < MIN_PROFILE_ID && dataCallParams.profileId > MAX_PROFILE_ID) {
        LOG(ERROR, __FUNCTION__, " Invalid profile id");
        return telux::common::Status::INVALIDPARAM;
    }
    ...
    request.set_operation_type((::dataStub::OperationType)dataCallParams.operationType);
    request.set_client_id(getpid());
    ...
    do {
        if (status == telux::common::Status::SUCCESS
            ...
            if (dataCalls_.find(dataCallParams.profileId) != dataCalls_.end()) {
                ...
                baseCallPtr
                    = std::static_pointer_cast<IDataCall>(dataCalls_[dataCallParams.profileId]);
                break;
            } else if (cacheDataCalls_.find(dataCallParams.profileId) != cacheDataCalls_.end()) {
                LOG(DEBUG, __FUNCTION__, " datacall already exists");
                ...
                dataCalls_[dataCallParams.profileId] = cacheDataCalls_[dataCallParams.profileId];
                cacheDataCalls_.erase(dataCallParams.profileId);
                break;
            }

            LOG(DEBUG, __FUNCTION__,
                " creating new datacall for profile:", dataCallParams.profileId);
            call        = std::make_shared<telux::data::DataCallStub>("");
            baseCallPtr = std::static_pointer_cast<IDataCall>(call);
    ...
    if (callback && (delay != SKIP_CALLBACK)) {
        auto f = std::async(std::launch::async, [this, baseCallPtr, error, delay, callback]() {
            std::this_thread::sleep_for(std::chrono::milliseconds(delay));
...
void DataConnectionManagerStub::handleStopDataCallEvent(::dataStub::StopDataCallEvent stopEvent) {
    LOG(DEBUG, __FUNCTION__);
    ...
    do {
        ...
        if (status == telux::common::Status::SUCCESS) {
            ...
            if (call) {
                ...
                call->setDataBearerTechnology(DataBearerTechnology::UNKNOWN);
                call->setOperationType(operationType);
                if ((ipFamilyType == IpFamilyType::IPV4)
                    || (ipFamilyType == IpFamilyType::IPV4V6)) {
                    call->setDataCallStatus(DataCallStatus::NET_DISCONNECTING, IpFamilyType::IPV4);
                }

                if ((ipFamilyType == IpFamilyType::IPV6)
                    || (ipFamilyType == IpFamilyType::IPV4V6)) {
                    call->setDataCallStatus(DataCallStatus::NET_DISCONNECTING, IpFamilyType::IPV6);
                }
                call->setDataCallEndReason(emptyDataCallEndReason);
...
telux::common::Status DataConnectionManagerStub::requestDataCallList(
    ...
    if (status == telux::common::Status::SUCCESS) {
        ...
        auto f = std::async(std::launch::async, [this, dataCalls, error, delay, callback]() {
            if (callback && (delay != SKIP_CALLBACK)) {
                std::this_thread::sleep_for(std::chrono::milliseconds(delay));
...
telux::common::Status DataConnectionManagerStub::requestThrottledApnInfo(ThrottleInfoCb callback) {
    ...
    if (status == telux::common::Status::SUCCESS) {
        ...
        for (auto idx = 0; idx < size; idx++) {
            ...
            auto apn_throttle_info
                = response.mutable_apn_throttle_info_list()->mutable_rep_apn_throttle_info(idx);
        ...
        if (callback && (delay != SKIP_CALLBACK)) {
            auto f
                = std::async(std::launch::async, [this, apnThrottleInfo, error, delay, callback]() {
                      std::this_thread::sleep_for(std::chrono::milliseconds(delay));
...
void DataConnectionManagerStub::handleThrottledApnInfoChangedEvent(
    ...
    LOG(DEBUG, __FUNCTION__,
        " throttled_apn_info_list size: ", throttleInfoList.rep_apn_throttle_info_size());
