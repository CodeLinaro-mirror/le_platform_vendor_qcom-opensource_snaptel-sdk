...
void CardManagerServerImpl::getJsonForSystemData(
...
void CardManagerServerImpl::getJsonForApiResponseSlot(
    int phoneId, std::string &jsonfilename, Json::Value &rootObj) {
...
bool CardManagerServerImpl::findAppId(Json::Value rootObj, const char *appid, int &index) {
    ...
    for (index = 0; index < sizeofADF; index++) {
        const char *tmp
            = (rootObj["ICardManager"]["EFs"]["ADF"][index]["AppId"].asString()).c_str();
        std::string val = rootObj["ICardManager"]["EFs"]["ADF"][index]["AppId"].asString();
...
grpc::Status CardManagerServerImpl::InitService(ServerContext *context,
    ...
    if (readStatus.ok()) {
        ...
        std::string cbStatus = rootObj["ICardManager"]["IsSubsystemReady"].asString();
...
grpc::Status CardManagerServerImpl::CleanUpService(ServerContext *context,
    ...
    for (auto it = refreshVotingClients_.begin(); it != refreshVotingClients_.end();) {
    ...
    for (auto it = refreshRegisterClients_.begin(); it != refreshRegisterClients_.end();) {
        if (it->clientId == clientId) {
...
grpc::Status CardManagerServerImpl::GetCardState(ServerContext *context,
    const ::telStub::GetCardStateRequest *request, telStub::GetCardStateReply *response) {
    LOG(DEBUG, __FUNCTION__);
...
grpc::Status CardManagerServerImpl::ReadEFLinearFixed(ServerContext *context,
    const ::telStub::ReadEFLinearFixedRequest *request, telStub::ReadEFLinearFixedReply *response) {
    LOG(DEBUG, __FUNCTION__);
    ...
    if (readStatus.ok()) {
        ...
        CommonUtils::getValues(
            jsonObjApiResponse, "ICardManager", "readEFLinearFixed", status, error, cbDelay);
        int index;
        int i = 0;
        if (status == telux::common::Status::SUCCESS) {
            ...
            if (foundAppId) {
                int size
                    = rootObj["ICardManager"]["EFs"]["ADF"][index]["LinearFixedEFFiles"].size();
                LOG(DEBUG, __FUNCTION__, " LinearFixedEFfiles size ", size);
                tmp = findmatchingrecordADF<telStub::ReadEFLinearFixedReply *>(
                    rootObj, response, size, index, recordNum, fileId, i);
                if (tmp == commonStub::ErrorCode::ERROR_CODE_SUCCESS) {
                    ...
                    result.payload = rootObj["ICardManager"]["EFs"]["ADF"][index]
                                            ["LinearFixedEFFiles"][i + recordNum]["payload"]
                                                .asString();
                    std::string input = rootObj["ICardManager"]["EFs"]["ADF"][index]
                                               ["LinearFixedEFFiles"][i + recordNum]["data"]
                                                   .asString();
...
grpc::Status CardManagerServerImpl::WriteEFLinearFixed(ServerContext *context,
    ...
    if (readStatus.ok()) {
        ...
        int index        = 0;
        CommonUtils::getValues(
            jsonObjApiResponse, "ICardManager", "writeEFLinearFixed", status, error, cbDelay);
        if (status == telux::common::Status::SUCCESS) {
            bool foundAppId = findAppId(rootObj, appid, index);
            if (foundAppId) {
                int size
                    = rootObj["ICardManager"]["EFs"]["ADF"][index]["LinearFixedEFFiles"].size();
                LOG(DEBUG, __FUNCTION__, " LinearFixedEFfiles size ", size);
                tmp = findmatchingrecordADF<telStub::WriteEFLinearFixedReply *>(
                    rootObj, response, size, index, recordsize, fileId, i);
                if (tmp == commonStub::ErrorCode::ERROR_CODE_SUCCESS) {
                    str1 = CommonUtils::convertVectorToString(data, false);
                    LOG(DEBUG, __FUNCTION__, " String value is", str1);
                    rootObj["ICardManager"]["EFs"]["ADF"][index]["LinearFixedEFFiles"]
                           [i + recordsize]["data"]
                        = str1;
                ...
                if (tmp == commonStub::ErrorCode::ERROR_CODE_SUCCESS) {
                    str1 = CommonUtils::convertVectorToString(data, false);
                    rootObj["ICardManager"]["EFs"]["DFLinearFixedEFRecords"][i + recordsize]["data"]
                        = str1;
                    LOG(DEBUG, __FUNCTION__, " String is  ", str1);
                    str1 = CommonUtils::convertVectorToString(data, true);
                    rootObj["ICardManager"]["EFs"]["DFLinearFixedEFRecords"][i + recordsize]["paylo"
                                                                                             "ad"]
                        = str1;
                    JsonParser::writeToJsonFile(rootObj, jsonfilename);
                    jsonObjSystemStateSlot_[slotId] = rootObj;
...
grpc::Status CardManagerServerImpl::ReadEFLinearFixedAll(ServerContext *context,
    ...
    if (readStatus.ok()) {
        ...
        telux::common::Status status;
        telux::common::ErrorCode error;
        CommonUtils::getValues(jsonObjApiResponse, "ICardManager", apiname, status, error, cbDelay);
        ...
        if (status == telux::common::Status::SUCCESS) {
            ...
            if (foundAppId) {
                    ...
                    if (tmpfileId == fileId) {
                        ...
                        for (int j = 1; j <= num; j++) {
                            ...
                            result.sw1 = rootObj["ICardManager"]["EFs"]["ADF"][index]
                                                ["LinearFixedEFFiles"][i + j]["sw1"]
                                                    .asInt();
                            LOG(DEBUG, __FUNCTION__, " sw1 ", result.sw1);
                            result.sw2 = rootObj["ICardManager"]["EFs"]["ADF"][index]
                                                ["LinearFixedEFFiles"][i + j]["sw2"]
                                                    .asInt();
                            LOG(DEBUG, __FUNCTION__, " sw2 ", result.sw2);
                            result.payload = rootObj["ICardManager"]["EFs"]["ADF"][index]
                                                    ["LinearFixedEFFiles"][i + j]["payload"]
                                                        .asString();
                            LOG(DEBUG, __FUNCTION__, " payload ", result.payload);
                            std::string input = rootObj["ICardManager"]["EFs"]["ADF"][index]
                                                       ["LinearFixedEFFiles"][i + j]["data"]
                                                           .asString();
                            result.data = CommonUtils::convertStringToVector(input);
                ...
                tmp           = findmatchingrecordDF(rootObj, size, recordnum, fileId, i);
                if (tmp == commonStub::ErrorCode::ERROR_CODE_SUCCESS) {
                    int num = rootObj["ICardManager"]["EFs"]["DFLinearFixedEFRecords"][i]["numberOf"
                                                                                          "Records"]
                                  .asInt();
                    LOG(DEBUG, __FUNCTION__, " NumberOfRecords ", num);
                    for (int j = 1; j <= num; j++) {
                        telux::tel::IccResult result;
                        result.sw1
                            = rootObj["ICardManager"]["EFs"]["DFLinearFixedEFRecords"][j]["sw1"]
                                  .asInt();
                        result.sw2
                            = rootObj["ICardManager"]["EFs"]["DFLinearFixedEFRecords"][j]["sw2"]
                                  .asInt();
                        result.payload
                            = rootObj["ICardManager"]["EFs"]["DFLinearFixedEFRecords"][j]["payload"]
                                  .asString();
                        std::string input
                            = rootObj["ICardManager"]["EFs"]["DFLinearFixedEFRecords"][j]["data"]
                                  .asString();
                        result.data = CommonUtils::convertStringToVector(input);
                        records.emplace_back(result);
                    }
...
grpc::Status CardManagerServerImpl::ReadEFTransparent(ServerContext *context,
    const ::telStub::ReadEFTransparentRequest *request, telStub::ReadEFTransparentReply *response) {
    LOG(DEBUG, __FUNCTION__);
    int slotId                             = request->slot_id();
    std::string jsonfilename               = "";
    std::string jsonObjApiResponseFileName = "";
    ...
    if (readStatus.ok()) {
        ...
        if (status == telux::common::Status::SUCCESS) {
            ...
            if (foundAppId) {
                int i = 0;
                int size
                    = rootObj["ICardManager"]["EFs"]["ADF"][index]["TransparentEFFiles"].size();
                LOG(DEBUG, __FUNCTION__, " TransparentEFfiles size ", size);
                while (i < size) {
                    uint16_t tmpfileId = rootObj["ICardManager"]["EFs"]["ADF"][index]
                                                ["TransparentEFFiles"][i]["fileId"]
                                                    .asInt();
                    if (tmpfileId == fileId) {
                        if (recordsize >= 0) {
                            result.sw1 = rootObj["ICardManager"]["EFs"]["ADF"][index]
                                                ["TransparentEFFiles"][i]["sw1"]
                                                    .asInt();
                            LOG(DEBUG, __FUNCTION__, " sw1 ", result.sw1);
                            result.sw2 = rootObj["ICardManager"]["EFs"]["ADF"][index]
                                                ["TransparentEFFiles"][i]["sw2"]
                                                    .asInt();
                            LOG(DEBUG, __FUNCTION__, " sw2 ", result.sw2);
                            result.payload = rootObj["ICardManager"]["EFs"]["ADF"][index]
                                                    ["TransparentEFFiles"][i]["payload"]
                                                        .asString();
                            LOG(DEBUG, __FUNCTION__, " payload ", result.payload);
                            std::string input = rootObj["ICardManager"]["EFs"]["ADF"][index]
                                                       ["TransparentEFFiles"][i]["data"]
                                                           .asString();
                            result.data = CommonUtils::convertStringToVector(input);
                            break;
                        } else {
                ...
                while (i < size) {
                    uint16_t tmpfileId
                        = rootObj["ICardManager"]["EFs"]["DFTransparentEFRecords"][i]["fileId"]
                              .asInt();
                    if (tmpfileId == fileId) {
                        if (recordsize >= 0) {
                            result.sw1
                                = rootObj["ICardManager"]["EFs"]["DFTransparentEFRecords"][i]["sw1"]
                                      .asInt();
                            LOG(DEBUG, __FUNCTION__, " sw1 ", result.sw1);
                            result.sw2
                                = rootObj["ICardManager"]["EFs"]["DFTransparentEFRecords"][i]["sw2"]
                                      .asInt();
                            LOG(DEBUG, __FUNCTION__, " sw2 ", result.sw2);
                            result.payload = rootObj["ICardManager"]["EFs"]["DFTransparentEFRecord"
                                                                            "s"][i]["payload"]
                                                 .asString();
                            LOG(DEBUG, __FUNCTION__, " payload ", result.payload);
                            std::string input = rootObj["ICardManager"]["EFs"]["DFTransparentEFReco"
                                                                               "rds"][i]["data"]
                                                    .asString();
                            result.data = CommonUtils::convertStringToVector(input);
                            break;
        ...
        if (status == telux::common::Status::SUCCESS) {
            ...
            if (foundAppId) {
                ...
                while (i < size) {
                    ...
                    if (tmpfileId == fileId) {
                        str1 = CommonUtils::convertVectorToString(data, false);
                        rootObj["ICardManager"]["EFs"]["ADF"][index]["TransparentEFFiles"][i]["dat"
                                                                                              "a"]
                            = str1;
                        LOG(DEBUG, __FUNCTION__, " String is  ", str1);
                        str1 = CommonUtils::convertVectorToString(data, true);
                        rootObj["ICardManager"]["EFs"]["ADF"][index]["TransparentEFFiles"][i]["payl"
                                                                                              "oad"]
                            = str1;
                        JsonParser::writeToJsonFile(rootObj, jsonfilename);
                        jsonObjSystemStateSlot_[slotId] = rootObj;
                        result.sw1 = rootObj["ICardManager"]["EFs"]["ADF"][index]["TransparentEFFil"
                                                                                  "es"][i]["sw1"]
                                         .asInt();
                        LOG(DEBUG, __FUNCTION__, " sw1 ", result.sw1);
                        result.sw2 = rootObj["ICardManager"]["EFs"]["ADF"][index]["TransparentEFFil"
                                                                                  "es"][i]["sw2"]
                                         .asInt();
                        LOG(DEBUG, __FUNCTION__, " sw2 ", result.sw2);
                        break;
...
grpc::Status CardManagerServerImpl::RequestEFAttributes(ServerContext *context,
    const ::telStub::EFAttributesRequest *request, ::telStub::RequestEFAttributesReply *response) {
            ...
            if (foundAppId) {
                if (ef_type == telux::tel::EfType::TRANSPARENT) {
                    ...
                    if (tmp == commonStub::ErrorCode::ERROR_CODE_SUCCESS) {
                        ...
                        result.sw1 = rootObj["ICardManager"]["EFs"]["ADF"][index]["LinearFixedEFFil"
                                                                                  "es"][i]["sw1"]
                                         .asInt();
                        LOG(DEBUG, __FUNCTION__, " sw1 ", result.sw1);
                        result.sw2 = rootObj["ICardManager"]["EFs"]["ADF"][index]["LinearFixedEFFil"
                                                                                  "es"][i]["sw2"]
                                         .asInt();
                        LOG(DEBUG, __FUNCTION__, " sw2 ", result.sw2);
                        result.payload = rootObj["ICardManager"]["EFs"]["ADF"][index]
                                                ["LinearFixedEFFiles"][i]["payload"]
                                                    .asString();
                        LOG(DEBUG, __FUNCTION__, " payload ", result.payload);
                    } else {
                        error = telux::common::ErrorCode::GENERIC_FAILURE;
                    ...
                    if (tmp == commonStub::ErrorCode::ERROR_CODE_SUCCESS) {
                        ...
                        LOG(DEBUG, __FUNCTION__, " sw2 ", result.sw2);
                        result.payload = rootObj["ICardManager"]["EFs"]["ADF"][index]
                                                ["LinearFixedEFFiles"][i + 1]["payload"]
                                                    .asString();
                        LOG(DEBUG, __FUNCTION__, " payload ", result.payload);
                        std::string input = rootObj["ICardManager"]["EFs"]["ADF"][index]
                                                   ["LinearFixedEFFiles"][i + 1]["data"]
                                                       .asString();
                        result.data = CommonUtils::convertStringToVector(input);
                    } else {
                        error = telux::common::ErrorCode::GENERIC_FAILURE;
...
commonStub::ErrorCode CardManagerServerImpl::getTransparentFileAttributes(Json::Value rootObj,
    ...
    while (i < size) {
        uint16_t tmpfileId
            = rootObj["ICardManager"]["EFs"]["ADF"][index]["TransparentEFFiles"][i]["fileId"]
                  .asInt();
        if (tmpfileId == fileId) {
            std::string data
                = rootObj["ICardManager"]["EFs"]["ADF"][index]["TransparentEFFiles"][i]["data"]
                      .asString();
            std::vector<int> tmp  = CommonUtils::convertStringToVector(data);
...
commonStub::ErrorCode CardManagerServerImpl::getLinearfixedFileAttributes(Json::Value rootObj,
    ...
    while (i < size) {
        uint16_t tmpfileId
            = rootObj["ICardManager"]["EFs"]["ADF"][index]["LinearFixedEFFiles"][i]["fileId"]
                  .asInt();
        if (tmpfileId == fileId) {
            attributes.recordCount
                = rootObj["ICardManager"]["EFs"]["ADF"][index]["LinearFixedEFFiles"][i]["numberOfRe"
                                                                                        "cords"]
                      .asInt();  //
            std::string data
                = rootObj["ICardManager"]["EFs"]["ADF"][index]["LinearFixedEFFiles"][i + 1]["data"]
                      .asString();
            std::vector<int> tmp  = CommonUtils::convertStringToVector(data);
            attributes.recordSize = tmp.size();
...
grpc::Status CardManagerServerImpl::OpenLogicalChannel(ServerContext *context,
    ...
    if (readStatus.ok()) {
        ...
        CommonUtils::getValues(
            jsonObjApiResponse, "ICardManager", "openLogicalChannel", status, error, cbDelay);
        ...
        if (status == telux::common::Status::SUCCESS) {
            ...
            if (isChannelOpen) {
                ...
                result.sw1 = rootObj["ICardManager"]["transmitApduLogicalChannel"][0]["onChannelRes"
                                                                                      "ponseSw1"]
                                 .asInt();
                LOG(DEBUG, __FUNCTION__, " sw1 ", result.sw1);
                result.sw2 = rootObj["ICardManager"]["transmitApduLogicalChannel"][0]["onChannelRes"
                                                                                      "ponseSw2"]
                                 .asInt();
                LOG(DEBUG, __FUNCTION__, " sw2 ", result.sw2);
                result.payload
                    = rootObj["ICardManager"]["transmitApduLogicalChannel"][0]["onChannelResponsePa"
                                                                               "yload"]
                          .asString();
                LOG(DEBUG, __FUNCTION__, " payload ", result.payload);
                std::string tmp
                    = rootObj["ICardManager"]["transmitApduLogicalChannel"][0]["onChannelResponseDa"
                                                                               "ta"]
                          .asString();
                std::vector<int> data = CommonUtils::convertStringToVector(tmp);
                result.data           = data;
                channelId
                    = rootObj["ICardManager"]["openLogicalChannel"]["onChannelResponseChannel"]
                          .asInt();
...
grpc::Status CardManagerServerImpl::CloseLogicalChannel(ServerContext *context,
    ...
    if (readStatus.ok()) {
        ...
        int cbDelay;
        CommonUtils::getValues(
            jsonObjApiResponse, "ICardManager", "closeLogicalChannel", status, error, cbDelay);

        if (status == telux::common::Status::SUCCESS) {
            int inputchannel
                = rootObj["ICardManager"]["openLogicalChannel"]["onChannelResponseChannel"].asInt();
            if (inputchannel == channel) {
                bool isChannelOpen
                    = rootObj["ICardManager"]["openLogicalChannel"]["isOpen"].asBool();
                if (isChannelOpen) {
...
grpc::Status CardManagerServerImpl::TransmitAPDU(ServerContext *context,
    const ::telStub::TransmitAPDURequest *request, telStub::TransmitAPDUReply *response) {
    ...
    if (readStatus.ok()) {
        ...
        std::string str1 = "";
        telux::tel::IccResult result;
        CommonUtils::getValues(jsonObjApiResponse, "ICardManager", "transmitApduLogicalChannel",
        ...
        if (status == telux::common::Status::SUCCESS) {
            ...
            if (!isApSimProfileRequest_) {
                ...
                if (startsWith(str1, "191 45") || str1.empty()) {
                    result.sw1
                        = rootObj["ICardManager"]["transmitApduLogicalChannel"][1]["onChannelRespon"
                                                                                   "seSw1"]
                              .asInt();
                    result.sw2
                        = rootObj["ICardManager"]["transmitApduLogicalChannel"][1]["onChannelRespon"
                    ...
                    if (result.sw1 == 97) {
                        result.payload = rootObj["ICardManager"]["transmitApduLogicalChannel"][1]
                                                ["onChannelResponsePayload"]
                                                    .asString();
                        std::string tmp
                            = rootObj["ICardManager"]["transmitApduLogicalChannel"][1]["onChannelRe"
                                                                                       "sponseData"]
                                  .asString();
                        result.data = CommonUtils::convertStringToVector(tmp);
                        rootObj["ICardManager"]["transmitApduLogicalChannel"][1]["onChannelResponse"
                                                                                 "Sw1"]
                            = 144;
                        rootObj["ICardManager"]["transmitApduLogicalChannel"][1]["onChannelResponse"
                                                                                 "Sw2"]
                        ...
                        result.payload = rootObj["ICardManager"]["transmitApduLogicalChannel"][2]
                                                ["onChannelResponsePayload"]
                                                    .asString();
                        std::string tmp
                            = rootObj["ICardManager"]["transmitApduLogicalChannel"][2]["onChannelRe"
                                                                                       "sponseData"]
                                  .asString();
                        result.data = CommonUtils::convertStringToVector(tmp);
                        rootObj["ICardManager"]["transmitApduLogicalChannel"][1]["onChannelResponse"
                    ...
                    result.sw1
                        = rootObj["ICardManager"]["transmitApduLogicalChannel"][3]["onChannelRespon"
                                                                                   "seSw1"]
                              .asInt();
                    result.sw2
                        = rootObj["ICardManager"]["transmitApduLogicalChannel"][3]["onChannelRespon"
                    ...
                    result.sw1
                        = rootObj["ICardManager"]["transmitApduLogicalChannel"][4]["onChannelRespon"
                                                                                   "seSw1"]
                              .asInt();
                    result.sw2
                        = rootObj["ICardManager"]["transmitApduLogicalChannel"][4]["onChannelRespon"
...
grpc::Status CardManagerServerImpl::exchangeSimIO(ServerContext *context,
    const ::telStub::exchangeSimIORequest *request, telStub::exchangeSimIOReply *response) {
    ...
    if (readStatus.ok()) {
        ...
        if (status == telux::common::Status::SUCCESS) {
            ...
            JsonParser::writeToJsonFile(rootObj, jsonfilename);
            jsonObjSystemStateSlot_[phoneId] = rootObj;
            result.sw1 = rootObj["ICardManager"]["exchangeSimIO"]["onChannelResponseSw1"].asInt();
            result.sw2 = rootObj["ICardManager"]["exchangeSimIO"]["onChannelResponseSw2"].asInt();
            result.payload
                = rootObj["ICardManager"]["exchangeSimIO"]["onChannelResponsePayload"].asString();
            std::string tmp
                = rootObj["ICardManager"]["exchangeSimIO"]["onChannelResponseData"].asString();
            result.data = CommonUtils::convertStringToVector(tmp);
        }
...
grpc::Status CardManagerServerImpl::TransmitBasicAPDU(ServerContext *context,
    const ::telStub::TransmitBasicAPDURequest *request, telStub::TransmitBasicAPDUReply *response) {
    ...
    if (readStatus.ok()) {
        ...
        if (status == telux::common::Status::SUCCESS) {
            ...
            JsonParser::writeToJsonFile(rootObj, jsonfilename);
            jsonObjSystemStateSlot_[phoneId] = rootObj;
            result.sw1 = rootObj["ICardManager"]["transmitApduBasicChannel"]["onChannelResponseSw1"]
                             .asInt();
            result.sw2 = rootObj["ICardManager"]["transmitApduBasicChannel"]["onChannelResponseSw2"]
                             .asInt();
            result.payload
                = rootObj["ICardManager"]["transmitApduBasicChannel"]["onChannelResponsePayload"]
                      .asString();
            std::string tmp
                = rootObj["ICardManager"]["transmitApduBasicChannel"]["onChannelResponseData"]
                      .asString();
            result.data = CommonUtils::convertStringToVector(tmp);
        }
...
grpc::Status CardManagerServerImpl::requestEid(ServerContext *context,
    const ::telStub::requestEidRequest *request, telStub::requestEidReply *response) {
    ...
    if (readStatus.ok()) {
        ...
        std::string apiname = "transmitApduBasicChannel";
        CommonUtils::getValues(
            jsonObjApiResponse, "ICardManager", apiname, status, errorCodefromUser, cbDelay);
...
grpc::Status CardManagerServerImpl::updateSimStatus(ServerContext *context,
    const ::telStub::updateSimStatusRequest *request, telStub::updateSimStatusReply *response) {
    LOG(DEBUG, __FUNCTION__);
    int phoneId              = request->phone_id();
    std::string jsonfilename = "";
    Json::Value rootObj;
    ...
    if (readStatus.ok()) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            telStub::AppType apptype = static_cast<telStub::AppType>(
                rootObj["ICardManager"]["getApplications"][i]["appType"].asInt());
            LOG(DEBUG, __FUNCTION__, " apptype is  ", static_cast<int>(apptype));
            apps->set_app_type(apptype);
            telStub::AppState appstate = static_cast<telStub::AppState>(
                rootObj["ICardManager"]["getApplications"][i]["appState"].asInt());
    ...
    if (readStatus.ok()) {
        ...
        if (status == telux::common::Status::SUCCESS) {
            if (locktype == ::telStub::CardLockType::PIN1) {
                ...
                retrycount
                    = rootObj["ICardManager"]["changeCardPassword"]["retryCountPin1"].asInt();
                if ((oldPwd == password) && (retrycount != -1)) {
                    ...
                    if (retrycount < 0) {
                        ...
                        for (int i = 0; i < size; i++) {
                            std::string id
                                = rootObj["ICardManager"]["getApplications"][i]["appId"].asString();
                            if (id == appId) {
                                rootObj["ICardManager"]["getApplications"][i]["appState"]
                                    = 3;  // puk state
                                JsonParser::writeToJsonFile(rootObj, jsonfilename);
                                jsonObjSystemStateSlot_[phoneId] = rootObj;
                        ...
                        if (retrycount >= -1) {
                            ...
                            rootObj["ICardManager"]["changeCardPassword"]["retryCountPin1"]
                                = retrycount;
                            JsonParser::writeToJsonFile(rootObj, jsonfilename);
                            jsonObjSystemStateSlot_[phoneId] = rootObj;
                            IsCardInfoChanged                = true;
                ...
                retrycount
                    = rootObj["ICardManager"]["changeCardPassword"]["retryCountPin2"].asInt();
                if (oldPwd == password && (retrycount != -1)) {
                    ...
                    if (retrycount < 0) {
                        ...
                        rootObj["ICardManager"]["changeCardPassword"]["retryCountPin2"]
                            = retrycount;
                        JsonParser::writeToJsonFile(rootObj, jsonfilename);
                        jsonObjSystemStateSlot_[phoneId] = rootObj;
                    }
...
grpc::Status CardManagerServerImpl::UnlockByPin(ServerContext *context,
    const ::telStub::UnlockByPinRequest *request, telStub::UnlockByPinReply *response) {
    LOG(DEBUG, __FUNCTION__);
    int phoneId              = request->phone_id();
    std::string jsonfilename = "";
    Json::Value rootObj;
...
grpc::Status CardManagerServerImpl::UnlockByPuk(ServerContext *context,
    const ::telStub::UnlockByPukRequest *request, telStub::UnlockByPukReply *response) {
    LOG(DEBUG, __FUNCTION__);
    int phoneId              = request->phone_id();
    std::string jsonfilename = "";
    Json::Value rootObj;
        ...
        if (status == telux::common::Status::SUCCESS) {
            if (locktype == ::telStub::CardLockType::PUK1) {
                ...
                if ((puk == password) && (retrycount != -1)) {
                    ...
                    jsonObjSystemStateSlot_[phoneId] = rootObj;
                    retrycount
                        = rootObj["ICardManager"]["unlockCardByPuk"]["retryCountPin1"].asInt();
                } else {
                    retrycount
                        = rootObj["ICardManager"]["unlockCardByPuk"]["retryCountPin1"].asInt();
                    if (retrycount < 0) {
                ...
                if ((puk == password) && (retrycount != -1)) {
                    ...
                    jsonObjSystemStateSlot_[phoneId] = rootObj;
                    retrycount
                        = rootObj["ICardManager"]["unlockCardByPuk"]["retryCountPin2"].asInt();
                } else {
                    retrycount
                        = rootObj["ICardManager"]["unlockCardByPuk"]["retryCountPin2"].asInt();
                    if (retrycount < 0) {
...
grpc::Status CardManagerServerImpl::CardPower(ServerContext *context,
    const ::telStub::CardPowerRequest *request, telStub::CardPowerResponse *response) {
    LOG(DEBUG, __FUNCTION__);
    int phoneId              = request->phone_id();
    std::string jsonfilename = "";
    std::string apiname;
...
commonStub::ErrorCode CardManagerServerImpl::findmatchingrecordDF(
    Json::Value rootObj, int &size, int &recordNum, uint16_t &fileId, int &i) {
    commonStub::ErrorCode error = commonStub::ErrorCode::ERROR_CODE_SUCCESS;
    while (i < size) {
        uint16_t tmpfileId
            = rootObj["ICardManager"]["EFs"]["DFLinearFixedEFRecords"][i]["fileId"].asInt();
        if (tmpfileId == fileId) {
            int num = rootObj["ICardManager"]["EFs"]["DFLinearFixedEFRecords"][i]["numberOfRecords"]
                          .asInt();
            LOG(DEBUG, __FUNCTION__, " NumberOfRecords ", num);
            if (recordNum <= num) {
                error = commonStub::ErrorCode::ERROR_CODE_SUCCESS;
...
bool CardManagerServerImpl::handleCardInfoChanged(
    std::string eventParams, ::eventService::EventResponse &notification) {
...
bool CardManagerServerImpl::handleSimRefreshInjector(
    std::string eventParams, ::eventService::EventResponse &notification) {
    ...
    LOG(DEBUG, __FUNCTION__, " slotId ", slotId, ", stage ", stage, ", mode ", mode, ", fileId ",
        fileId, ", filePath ", filePath, ", sessionId ", sessionId, ", aid ", aid);
    ...
    if (not newRefreshEvt.pendingAllow) {
        ...
        updateSimRefreshStage(
            slotId, ENDED_WITH_FAILURE, REFRESH_USER_ALLOW_TIMEOUT_MS, true, false);
    }
    ...
    if (stage == STARTING) {
        ...
        if (newRefreshEvt.pendingComplete == 0
            ...
            updateSimRefreshStage(
                slotId, ENDED_WITH_FAILURE, REFRESH_USER_COMPLETE_TIMEOUT_MS, false, true);
        }
    }
    return true;
}
...
::grpc::Status CardManagerServerImpl::setupRefreshConfig(ServerContext *context,
    ...
    for (auto file : clientInfo.files) {
        LOG(DEBUG, __FUNCTION__, " File ID : ", file.fileId, ", File path : ", file.filePath);
    }

    LOG(DEBUG, __FUNCTION__, " phoneId ", clientInfo.phoneId, " isRegister ",
        static_cast<int>(request->isregister()), ", doVoting ",
        static_cast<int>(request->dovoting()));
    ...
    do {
        ...
        regError
            = updateClientSimRefresh(refreshRegisterClients_, clientInfo, request->isregister());
...
::grpc::Status CardManagerServerImpl::confirmRefreshHandlingCompleted(ServerContext *context,
    ...
    getApiConfigureFromJson(
        clientInfo.phoneId, "confirmRefreshHandlingCompleted", status, error, cbDelay);
    do {
        if (status != telux::common::Status::SUCCESS) {
            LOG(INFO, __FUNCTION__, " user prefer non-SUCCESS in json.");
            break;
        ...
        if (isCompleted) {
            ...
            if (pendComplete == 0) {
                updateSimRefreshStage(
                    clientInfo.phoneId, ENDED_WITH_SUCCESS, DEFAULT_DELAY, false, false);
            }
        } else {
...
void CardManagerServerImpl::updateSimRefreshStage(int slotId, CardRefreshStage newStage,
    ...
    auto f = std::async(std::launch::async,
        [this, slotId, newStage, delayMs, checkPendingUserAllow, checkPendingUserComplete]() {
            if (delayMs > 0) {
            ...
            if (newStage == STARTING) {
                if (refreshEvtMap_[slotId].pendingComplete > 0
                    && requireConfirmComplete(newStage,
                        static_cast<telux::tel::RefreshMode>(
                            refreshEvtMap_[slotId].refreshEvent.mode()),
...
bool CardManagerServerImpl::requireConfirmComplete(const CardRefreshStage stage,
    ...
    if (((mode == telux::tel::RefreshMode::INIT_FCN || mode == telux::tel::RefreshMode::FCN)
        ...
        LOG(DEBUG, __FUNCTION__, " yes, mode ", static_cast<int>(mode), ", st ",
            static_cast<int>(st));
        return true;
    }
...
int CardManagerServerImpl::findMatchedClients(
    ...
    client.sessionAid.sessionType
        = static_cast<telux::tel::SessionType>(event.refreshs().sessiontype());
    ...
    for (auto it = entry.begin(); it != entry.end(); ++it) {
        if (it->phoneId == client.phoneId
            ...
            if (it->files.size() == 0 && client.files.size() == 0) {
                found++;
                LOG(DEBUG, __FUNCTION__, " Found a client with the empty file ");
                ...
                if (client.files.size() == efMatch) {
                    LOG(DEBUG, __FUNCTION__, " Found a client containing ", client.files.size(),
                        " files ");
                    found++;
                }
            }
...
void CardManagerServerImpl::getClientInfoFromRpc(const T *rpcMsg, ClientSimRefreshPref &client) {
    ...
    client.sessionAid.sessionType
        = static_cast<telux::tel::SessionType>(rpcMsg->refreshs().sessiontype());
    client.sessionAid.aid = rpcMsg->refreshs().aid();
    LOG(DEBUG, __FUNCTION__, " phoneId ", client.phoneId, ", sessionType ",
        static_cast<uint32_t>(client.sessionAid.sessionType), ", aid ", client.sessionAid.aid);
}
...
void CardManagerServerImpl::updateApduResultToJson(int phoneId, uint32_t sw1, uint32_t sw2) {
    ...
    std::string jsonfilename
        = (phoneId == SLOT_1) ? AP_SIM_PROFILE_JSON_STATE_PATH1 : AP_SIM_PROFILE_JSON_STATE_PATH2;
