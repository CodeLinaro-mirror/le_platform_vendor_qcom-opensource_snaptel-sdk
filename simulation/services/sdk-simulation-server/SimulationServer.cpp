...
void SimulationServer::startGrpcServer() {
    ...
    std::shared_ptr<Cv2xRadioServer> cv2xRadioServer = std::make_shared<Cv2xRadioServer>();
    if (cv2xRadioServer) {
    ...
    std::shared_ptr<SocksServerImpl> socksService = std::make_shared<SocksServerImpl>();
    builder.RegisterService(socksService.get());

    std::shared_ptr<NatServerImpl> natService = std::make_shared<NatServerImpl>();
    builder.RegisterService(natService.get());

    std::shared_ptr<L2tpServerImpl> l2tpService = std::make_shared<L2tpServerImpl>();
    builder.RegisterService(l2tpService.get());

    std::shared_ptr<FirewallServerImpl> firewallService = std::make_shared<FirewallServerImpl>();
    builder.RegisterService(firewallService.get());

    std::shared_ptr<BridgeServerImpl> bridgeService = std::make_shared<BridgeServerImpl>();
    ...
    std::shared_ptr<DataLinkServerImpl> dataLinkService = std::make_shared<DataLinkServerImpl>();
    ...
    std::shared_ptr<CallManagerServerImpl> callService = std::make_shared<CallManagerServerImpl>();
    ...
    std::shared_ptr<FsManagerServerImpl> FsManagerService = std::make_shared<FsManagerServerImpl>();
