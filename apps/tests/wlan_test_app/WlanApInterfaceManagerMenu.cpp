...
bool WlanApInterfaceManagerMenu::init() {
    if (wlanApInterfaceManager_ == nullptr) {
        ...
        if (wlanApInterfaceManager_ == nullptr) {
            // Return immediately
            std::cout << "\nError encountered in initializing Wlan Ap Interface Manager"
                      << std::endl;
            return false;
        }
    }
...
void WlanApInterfaceManagerMenu::showMenu() {
    if (menuOptionsAdded_ == false) {
        ...
        int stepID                                   = 1;
        std::shared_ptr<ConsoleAppCommand> setConfig = std::make_shared<ConsoleAppCommand>(
            ConsoleAppCommand(std::to_string(stepID++), "set_config", {},
                std::bind(&WlanApInterfaceManagerMenu::setConfig, this, std::placeholders::_1)));
        std::shared_ptr<ConsoleAppCommand> setSecurityConfig = std::make_shared<ConsoleAppCommand>(
            ConsoleAppCommand(std::to_string(stepID++), "set_security_config", {},
                std::bind(
                    &WlanApInterfaceManagerMenu::setSecurityConfig, this, std::placeholders::_1)));
        std::shared_ptr<ConsoleAppCommand> setSsid = std::make_shared<ConsoleAppCommand>(
            ConsoleAppCommand(std::to_string(stepID++), "set_ssid", {},
                std::bind(&WlanApInterfaceManagerMenu::setSsid, this, std::placeholders::_1)));
        std::shared_ptr<ConsoleAppCommand> setVisibility = std::make_shared<ConsoleAppCommand>(
            ConsoleAppCommand(std::to_string(stepID++), "set_visibility", {},
                std::bind(
                    &WlanApInterfaceManagerMenu::setVisibility, this, std::placeholders::_1)));
        std::shared_ptr<ConsoleAppCommand> configureElementInfo
            = std::make_shared<ConsoleAppCommand>(
                ConsoleAppCommand(std::to_string(stepID++), "configure_elementInfo", {},
                    std::bind(&WlanApInterfaceManagerMenu::configureElementInfo, this,
                        std::placeholders::_1)));
        std::shared_ptr<ConsoleAppCommand> setPassPhrase = std::make_shared<ConsoleAppCommand>(
            ConsoleAppCommand(std::to_string(stepID++), "set_passphrase", {},
                std::bind(
                    &WlanApInterfaceManagerMenu::setPassPhrase, this, std::placeholders::_1)));
        ...
        std::shared_ptr<ConsoleAppCommand> getConnectedDevices
            = std::make_shared<ConsoleAppCommand>(
                ConsoleAppCommand(std::to_string(stepID++), "get_connected_devices", {},
                    std::bind(&WlanApInterfaceManagerMenu::getConnectedDevices, this,
                        std::placeholders::_1)));
        std::shared_ptr<ConsoleAppCommand> manageApService = std::make_shared<ConsoleAppCommand>(
            ConsoleAppCommand(std::to_string(stepID++), "manage_service", {},
                std::bind(
                    &WlanApInterfaceManagerMenu::manageApService, this, std::placeholders::_1)));
        std::vector<std::shared_ptr<ConsoleAppCommand>> commandsList
            = {setConfig, setSecurityConfig, setSsid, setVisibility, configureElementInfo,
                setPassPhrase, getConfig, getStatus, getConnectedDevices, manageApService};
    ...
    if (retCode == telux::common::ErrorCode::SUCCESS) {
        for (auto &cfg : config) {
            ...
            for (auto &netCfg : cfg.network) {
                std::cout << "AP Type: " << WlanUtils::getWlanApType(netCfg.info.apType)
                          << std::endl;
                std::cout << "AP Radio: " << WlanUtils::RadioTypeToString(netCfg.info.apRadio)
                          << std::endl;
                std::cout << "AP SSID: " << netCfg.ssid << std::endl;
                std::cout << "AP is Visible: " << ((netCfg.isVisible) ? "Yes" : "No") << std::endl;
                WlanUtils::printApElementInfo(netCfg.elementInfoConfig);
                std::cout << "AP Interworking: " << WlanUtils::apAccessToString(netCfg.interworking)
                          << std::endl;
                std::cout << "AP Security: " << std::endl;
                std::cout
                    << "    Mode: " << WlanUtils::apSecurityModeToString(netCfg.apSecurity.mode)
                    << std::endl;
                std::cout << "    Authorization: "
                          << WlanUtils::apSecurityAuthToString(netCfg.apSecurity.auth) << std::endl;
                std::cout << "    Encryption: "
                          << WlanUtils::apSecurityEncryptToString(netCfg.apSecurity.encrypt)
                          << std::endl;
...
void WlanApInterfaceManagerMenu::getConnectedDevices(std::vector<std::string> userInput) {
    ...
    telux::common::ErrorCode retCode = wlanApInterfaceManager_->getConnectedDevices(clientsInfo);
...
void WlanApInterfaceManagerMenu::manageApService(std::vector<std::string> userInput) {
    ...
    telux::common::ErrorCode retCode = wlanApInterfaceManager_->manageApService(
        static_cast<telux::wlan::Id>(apId), static_cast<telux::wlan::ServiceOperation>(opr));
...
void WlanApInterfaceManagerMenu::setSecurityConfig(std::vector<std::string> userInput) {
    ...
    telux::common::ErrorCode retCode
        = wlanApInterfaceManager_->setSecurityConfig(static_cast<telux::wlan::Id>(id), security);
...
void WlanApInterfaceManagerMenu::setSsid(std::vector<std::string> userInput) {
    ...
    telux::common::ErrorCode retCode
        = wlanApInterfaceManager_->setSsid(static_cast<telux::wlan::Id>(id), input);
...
void WlanApInterfaceManagerMenu::setPassPhrase(std::vector<std::string> userInput) {
    ...
    telux::common::ErrorCode retCode
        = wlanApInterfaceManager_->setPassPhrase(static_cast<telux::wlan::Id>(id), input);
