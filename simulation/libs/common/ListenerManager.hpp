...
class ListenerManager {
 public:
    ...
    void getAvailableListeners(std::vector<std::weak_ptr<T>> &availableListeners) {
    ...
    telux::common::Status registerListener(
        std::weak_ptr<T> listener, U indications, U &firstRegistration) {
        auto sp = listener.lock();
        ...
        // When no indications are provided, simply return success without storing the listener for
        // any indication
        if (indications.none()) {
        ...
        for (size_t itr = 0; itr < indications.size(); itr++) {
            if (indications.test(itr)) {
                ...
                if (registrationMap_[itr].empty()) {
                    ...
                    auto temp = std::find_if(std::begin(registrationMap_[itr]),
                        std::end(registrationMap_[itr]), [=](std::weak_ptr<T> listenerExisted) {
                            return (listenerExisted.lock() == sp);
                        });
    ...
    telux::common::Status deRegisterListener(
        std::weak_ptr<T> listener, U indications, U &lastDeregistration) {
        auto sp = listener.lock();
        if (sp == nullptr) {
            LOG(ERROR, __FUNCTION__, " Null listener");
            return telux::common::Status::INVALIDPARAM;
