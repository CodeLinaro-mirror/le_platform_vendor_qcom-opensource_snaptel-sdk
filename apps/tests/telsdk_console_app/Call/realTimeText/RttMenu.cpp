    ...
    if (menuOptionsAdded_ == false) {
        ...
        std::shared_ptr<ConsoleAppCommand> respondToModifyRequestCommand
            = std::make_shared<ConsoleAppCommand>(
                ConsoleAppCommand("5", "Respond_To_Modify_Request", {"callIndex"},
        ...
        std::shared_ptr<ConsoleAppCommand> hangupWithCallIndexCommand
            = std::make_shared<ConsoleAppCommand>(ConsoleAppCommand("7", "Hangup", {"index"},
                std::bind(&RttMenu::hangupWithCallIndex, this, std::placeholders::_1)));
        std::shared_ptr<ConsoleAppCommand> hangupDialingOrAlertingCommand
            = std::make_shared<ConsoleAppCommand>(ConsoleAppCommand("8", "Hangup", {},
                std::bind(&RttMenu::hangupDialingOrAlerting, this, std::placeholders::_1)));
        std::shared_ptr<ConsoleAppCommand> holdCallCommand
            = std::make_shared<ConsoleAppCommand>(ConsoleAppCommand(
                "9", "Hold_call", {}, std::bind(&RttMenu::holdCall, this, std::placeholders::_1)));
        ...
        std::shared_ptr<ConsoleAppCommand> swapCommand
            = std::make_shared<ConsoleAppCommand>(ConsoleAppCommand(
                "11", "Swap", {}, std::bind(&RttMenu::swap, this, std::placeholders::_1)));
        ...
        std::vector<std::shared_ptr<ConsoleAppCommand>> commandsList = {dialRttCommand,
            acceptCallCommand, rejectCallCommand, modifyCommand, respondToModifyRequestCommand,
            sendRttMessageCommand, hangupWithCallIndexCommand, hangupDialingOrAlertingCommand,
            holdCallCommand, resumeCallCommand, swapCommand, getCallsCommand};
...
void RttMenu::acceptCall(std::vector<std::string> userInput) {
    ...
    try {
        ...
        std::cout << "ERROR: invalid input, please enter a valid value. INPUT: " << mode
                  << std::endl;
        return;
    }
    if (telux::common::DeviceConfig::isMultiSimSupported()) {
...
void RttMenu::respondToModifyRequest(std::vector<std::string> userInput) {
    ...
    try {
        ...
        std::cout << "ERROR: invalid input, please enter a valid value. INPUT: " << request
                  << std::endl;
        return;
    }
