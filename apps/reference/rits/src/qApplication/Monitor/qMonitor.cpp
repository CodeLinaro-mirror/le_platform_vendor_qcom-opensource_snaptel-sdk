...
QMonitor::QMonitor(const Configuration conf) {
    ...
    // Creating Socket
    serverSock = socket(config.sockDomain, config.sockType, config.sockProtocol);
    ret        = serverSock;
    if (errorCheck(AlertInfo<int>{"Error Creating qMonitor Server Socket", ret, SOCK_ERROR})
        == SOCK_ERROR) {
        close(serverSock);
        return;
    }
    ...
    // Adding Options
    ret = setsockopt(
        serverSock, config.sockLevel, config.sockOptName, &config.sockOpt, sizeof(config.sockOpt));

    if (errorCheck(
            AlertInfo<int>{"Error Setting Option for qMonitor Server Socket", ret, SOCK_ERROR})
        == SOCK_ERROR) {
        close(serverSock);
        return;
    }
    ...
    ret = bind(serverSock, (struct sockaddr *)&config.sockAddress, sizeof(config.sockAddress));
    // cout << "Return from bind with value: " << ret << endl;
    if (errorCheck(AlertInfo<int>{"Error Binding qMonitor Server Socket", ret, SOCK_ERROR})
        == SOCK_ERROR) {
        close(serverSock);
        return;
    }
    ...
    if (errorCheck(AlertInfo<int>{"Error Listening on qMonitor Server Socket", ret, SOCK_ERROR})) {
        close(serverSock);
        return;
    }
...
void QMonitor::connectionHandler() {
    while (isMonitoring) {
        ...
        const int clientSock  = accept(serverSock, (struct sockaddr *)&clientAddress, &addrSize);

        if (errorCheck(
                AlertInfo<int>{"Error accepting client with Address:", clientSock, SOCK_ERROR})
            != SOCK_ERROR) {
...
void QMonitor::clientHandler(int clientSock) {
    ...
    while (cData->handling) {
        ...
        ret = write(
            cData->sock, json_object_to_json_string_length(cData->res, 0, &resSize), resSize);
...
int QMonitor::createResponse(int client, json_object *res) {
    ...
    using namespace QMonitorJson;
    if (valOpts->totalTx) {
        json_object_object_add(res, kStr[TOTAL_TX], json_object_new_int64(tempData.totalTx));
    }
    if (valOpts->totalRx) {
        json_object_object_add(res, kStr[TOTAL_RX], json_object_new_int64(tempData.totalRx));
    }
    if (valOpts->totalRSUs) {
        json_object_object_add(res, kStr[TOTAL_RSUS], json_object_new_int64(tempData.totalRSUs));
    }
    if (valOpts->totalRVs) {
        json_object_object_add(res, kStr[TOTAL_RVS], json_object_new_int64(tempData.totalRVs));
    }
    if (valOpts->rxFails) {
        json_object_object_add(res, kStr[RX_FAILS], json_object_new_int64(tempData.rxFails));
    }
    if (valOpts->decodeFails) {
        json_object_object_add(
            res, kStr[DECODE_FAILS], json_object_new_int64(tempData.decodeFails));
    }
    if (valOpts->securityFails) {
        json_object_object_add(res, kStr[SEC_FAILS], json_object_new_int64(tempData.secFails));
    }
    if (valOpts->mbdAlerts) {
        json_object_object_add(res, kStr[MBD_ALERTS], json_object_new_int64(tempData.mbdAlerts));
    }
    if (valOpts->txBSMs) {
        json_object_object_add(res, kStr[TX_BSMS], json_object_new_int64(tempData.txBSMs));
    }
    if (valOpts->txSignedBSMs) {
        json_object_object_add(
            res, kStr[TX_SIGNED_BSMS], json_object_new_int64(tempData.txSignedBSMs));
    }
    if (valOpts->rxBSMs) {
        json_object_object_add(res, kStr[RX_BSMS], json_object_new_int64(tempData.rxBSMs));
    }
    if (valOpts->rxSignedBSMs) {
        json_object_object_add(
            res, kStr[RX_SIGNED_BSMS], json_object_new_int64(tempData.rxSignedBSMs));
    }
    ...
    if (metaOpts->timestamp) {
        ...
        json_object_object_add(res, kStr[TIMESTAMP], json_object_new_int64(nanoTime));
    }
    if (metaOpts->jsonVersion) {
        json_object_object_add(res, kStr[JSON_VER], json_object_new_string(JSON_VERSION));
    }

    if (metaOpts->qitsVersion) {
        json_object_object_add(res, kStr[QITS_VER], json_object_new_string(QITS_VERSION));
    }

    if (metaOpts->telsdkVersion) {
        json_object_object_add(res, kStr[TELSDK_VER], json_object_new_string(TELSDK_VERSION));
    }
    if (metaOpts->qMonVersion) {
        json_object_object_add(res, kStr[QMON_VER], json_object_new_string(QMON_VERSION));
    }
    ...
    if (metaOpts->blob) {
        ...
        json_object_object_add(res, kStr[BLOB], json_object_new_string(tempStr.c_str()));
    }
...
int QMonitor::processReq(int client) {
    ...
    string info;
    if (cData->req == NULL || json_object_get_type(cData->req) != json_type_object) {
    ...
    if (config.debugLevel > 1) {
        cout << "Processing the following json: " << json_object_to_json_string(cData->req) << endl;
    }
    ...
    for (auto key : kStr) {
        if (json_object_object_get_ex(cData->req, key, &jsonTmp)) {
            info = string("Error adding ") + key + " to options from client " + to_string(client);
            errorCheck(AlertInfo<int>{info.c_str(), changeOption(client, key, jsonTmp), 0});
        } else {
...
int QMonitor::changeOption(int client, const char *key, json_object *obj) {
    ...
    if (config.debugLevel > LOW_ALERT) {
        std::cout << "Changing the following option " << key << " to: " << json_object_get_int(obj)
                  << std::endl;
    }
    try {
        switch (kMap.at(key)) {
...
int QMonitor::alert(AlertInfo<T> info) {
    ...
    switch (config.debugLevel) {
        case LOW_ALERT:
            ...
            break;
        case MED_ALERT:
            cout << info.data << " Value: " << info.value << " Error: " << info.errorValue << endl;
            break;
        case HIGH_ALERT:
            cout << info.data << " Value: " << info.value << " Error: " << info.errorValue
                 << " Errno: " << errno << " Errno Info: " << strerror(errno) << endl;
            break;
        case NO_ALERT:
        default:
            break;
...
void QMonitor::printConfig() {
    if (config.debugLevel) {
        cout << "Port net: " << config.sockAddress.sin_port
             << " port host:" << ntohs(config.sockAddress.sin_port) << endl;
