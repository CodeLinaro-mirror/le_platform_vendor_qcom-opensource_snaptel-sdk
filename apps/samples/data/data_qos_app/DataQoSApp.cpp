...
DataQoSApp::DataQoSApp()
   : ConsoleApp("Data QoS App Menu", "data-qos-app> ") {
    ...
    if (!dataQoSManager_) {
        ...
        dataQoSManager_ = telux::data::DataFactory::getInstance().getQoSManager(
            [&qosProm](telux::common::ServiceStatus status) { qosProm.set_value(status); });
...
void DataQoSApp::onDataCallInfoChanged(const std::shared_ptr<telux::data::IDataCall> &dataCall) {
...
bool DataQoSApp::initDataConnectionManager() {
    ...
    if (!dataConnectionManager_) {
        ...
        if (subSystemStatus == telux::common::ServiceStatus::SERVICE_AVAILABLE) {
            ...
            telux::common::Status status = dataConnectionManager_->registerListener(dcmListener);
...
bool DataQoSApp::initVlanManager() {
    if (!vlanManager_) {
        ...
        vlanManager_ = telux::data::DataFactory::getInstance().getVlanManager(
            ...
            [&vlanProm](telux::common::ServiceStatus status) { vlanProm.set_value(status); });
...
bool DataQoSApp::loadDefaultProfile() {
    if (initDataConnectionManager()) {
        ...
        telux::common::Status status
            = dataConnectionManager_->getDefaultProfile(telux::data::OperationType::DATA_LOCAL,
                [&defaultProfileProm, this](
...
bool DataQoSApp::startAndWaitForDataCall() {
    if (initDataConnectionManager()) {
        ...
        if (loadDefaultProfile()) {
            telux::common::Status status = dataConnectionManager_->startDataCall(
                profileId_, ipFamilyType,
...
bool DataQoSApp::createAndWaitForVlan(int vlanId, bool isAccelerated, int pcp) {
    if (initVlanManager()) {
        ...
        auto respCb = [&vlanProm, &vlanId, &pcp](
                          bool isAccelerated, telux::common::ErrorCode error) {
            std::cout << std::endl << std::endl;
            std::cout << "VLAN ID: " << vlanId << ", PCP: " << pcp
                      << ", IPA HW acceleration: " << (isAccelerated ? " enabled" : " not enabled")
                      << std::endl;
            std::cout << "CALLBACK: "
                      << "createVlan Response"
...
void DataQoSApp::stopDataCall() {
    if (dataConnectionManager_) {
        ...
        dataConnectionManager_->stopDataCall(
            profileId_, telux::data::IpFamilyType::IPV4,
            [&conProm](const std::shared_ptr<telux::data::IDataCall> &dataCall,
                telux::common::ErrorCode errorCode) {
                std::cout << "stopCallResponse: errorCode: " << static_cast<int>(errorCode)
                          << std::endl;
    ...
    if (initVlanManager()) {
        ...
        telux::common::Status status = vlanManager_->removeVlan(vlanId,
            telux::data::InterfaceType::ETH, [&vlanProm](telux::common::ErrorCode errorCode) {
                std::cout << "removeVlan: errorCode: " << static_cast<int>(errorCode) << std::endl;
...
bool DataQoSApp::createDownLinkTrafficClass(
...
void DataQoSApp::showAllConfigs() {
    if (initVlanManager()) {
        ...
        auto respBindCb = [this, &vlanBindProm](
            ...
            std::cout << "CALLBACK: "
                      ...
                      << ". ErrorCode: " << static_cast<int>(error) << std::endl;
            for (auto c : bindings) {
                std::cout << "Backhaul: " << backhaulToString(c.bhInfo.backhaul);
        ...
        if (vlanManager_->queryVlanToBackhaulBindings(
                telux::data::BackhaulType::WWAN, respBindCb, slotId_)
...
void DataQoSApp::consoleInit() {
    ...
    std::shared_ptr<ConsoleAppCommand> createTCAndAddQoSFilterForDLTetheredToAppsSWPath
        = std::make_shared<ConsoleAppCommand>(ConsoleAppCommand("3",
            "Use case 1 Create_TC_And_Add_QoS_Filter_For_DL_Tethered_To_Apps_SW_Path", {},
    ...
    std::shared_ptr<ConsoleAppCommand> createTCAndAddQoSFilterForULTetheredToAppsSWPath
        = std::make_shared<ConsoleAppCommand>(ConsoleAppCommand("4",
            "Use case 2 Create_TC_And_Add_QoS_Filter_For_UL_Tethered_To_Apps_SW_Path", {},
    ...
    std::shared_ptr<ConsoleAppCommand> createTCAndAddQoSFilterForDLTetheredToWanHWAccPath
        = std::make_shared<ConsoleAppCommand>(ConsoleAppCommand("5",
            "Use case 3 Create_TC_And_Add_QoS_Filter_For_DL_Tethered_To_Wan_HW_Acc_Path", {},
    ...
    std::shared_ptr<ConsoleAppCommand> createTCAndAddQoSFilterForULTetheredToWanHWAccPath
        = std::make_shared<ConsoleAppCommand>(ConsoleAppCommand("6",
            "Use case 4 Create_TC_And_Add_QoS_Filter_For_UL_Tethered_To_Wan_HW_Acc_Path", {},
    ...
    std::shared_ptr<ConsoleAppCommand> createTCAndAddQoSFilterForULAppsToWanPath
        = std::make_shared<ConsoleAppCommand>(ConsoleAppCommand("7",
            "Use case 5 Create_TC_And_Add_QoS_Filter_For_UL_Apps_To_Wan_Path", {},
            std::bind(&DataQoSApp::createTCAndAddQoSFilterForULAppsToWanPath, this)));

    std::shared_ptr<ConsoleAppCommand> cleanupCmd = std::make_shared<ConsoleAppCommand>(
        ConsoleAppCommand("8", "Clean_all_traffic_class_and_qos_filters", {},
            std::bind(&DataQoSApp::cleanup, this, SIGINT)));

    std::shared_ptr<ConsoleAppCommand> showAllConfigsCmd
        = std::make_shared<ConsoleAppCommand>(ConsoleAppCommand(
            "9", "show_all_configs", {}, std::bind(&DataQoSApp::showAllConfigs, this)));

    std::vector<std::shared_ptr<ConsoleAppCommand>> commandsListPowerMenu
        = {configureVLANs, clearVlanConfigs, createTCAndAddQoSFilterForDLTetheredToAppsSWPath,
...
bool DataQoSApp::parseArguments(int argc, char **argv) {
    ...
    static struct option long_options[]
        = {{"Console app mode", no_argument, 0, 'c'}, {"Configure VLANs", no_argument, 0, 'v'},
            {"Clear VLAN configurations", no_argument, 0, 'x'},
            {"List use case", no_argument, 0, 'l'}, {"Run use case", required_argument, 0, 'u'},
            {"Delete all QoS TC and filter configurations", no_argument, 0, 'd'},
            {"Show all configurations", no_argument, 0, 's'}, {"help", no_argument, 0, 'h'},
            {0, 0, 0, 0}};
