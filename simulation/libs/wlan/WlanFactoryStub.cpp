...
WlanFactoryStub::WlanFactoryStub() { LOG(DEBUG, __FUNCTION__); }

WlanFactoryStub::~WlanFactoryStub() { LOG(DEBUG, __FUNCTION__); }

WlanFactory::WlanFactory() { LOG(DEBUG, __FUNCTION__); }
...
std::shared_ptr<IWlanDeviceManager> WlanFactoryStub::getWlanDeviceManager(
  ...
  std::function<std::shared_ptr<telux::wlan::IWlanDeviceManager>(
    ...
    if (manager && telux::common::Status::SUCCESS != manager->init(initCb)) {
      LOG(ERROR, __FUNCTION__,
          " WLAN Factory unable to initialize Wlan Device Manager");
      return nullptr;
    }
    return manager;
  };
  ...
  auto manager = getManager<telux::wlan::IWlanDeviceManager>(
      type, wlanDeviceManager_, wlanDeviceManagerCallbacks_, clientCallback,
      createAndInit);
  return manager;
}

std::shared_ptr<IApInterfaceManager> WlanFactoryStub::getApInterfaceManager() {
  std::function<std::shared_ptr<telux::wlan::IApInterfaceManager>()>
      createAndInit =
    ...
    if (manager && telux::common::Status::SUCCESS != manager->init()) {
      LOG(ERROR, __FUNCTION__,
          " WLAN Factory unable to initialize Ap Interface Manager");
      return nullptr;
    }
    return manager;
  };
  ...
  auto manager = getManager<telux::wlan::IApInterfaceManager>(
      type, apInterfaceManager_, createAndInit);
  return manager;
}

std::shared_ptr<IStaInterfaceManager>
WlanFactoryStub::getStaInterfaceManager() {
  std::function<std::shared_ptr<telux::wlan::IStaInterfaceManager>()>
      createAndInit =
    ...
    if (manager && telux::common::Status::SUCCESS != manager->init()) {
      LOG(ERROR, __FUNCTION__,
          " WLAN Factory unable to initialize Sta Interface Manager");
      return nullptr;
    }
    return manager;
  };
  ...
  LOG(DEBUG, __FUNCTION__, ": Requesting ", type.c_str());
  // Use FactoryHelper to manage singleton instance.
  auto manager = getManager<telux::wlan::IStaInterfaceManager>(
      type, staInterfaceManager_, createAndInit);
  return manager;
}
