...
void Cv2xRadioServer::init(std::weak_ptr<telux::cv2x::ICv2xListener> self) {
    LOG(DEBUG, __FUNCTION__);
    if (evtListener_) {
        std::vector<std::string> filters = {CV2X_EVENT_RADIO_FILTER};
...
void Cv2xRadioServer::onStatusChanged(telux::cv2x::Cv2xStatus status) {
    LOG(DEBUG, __FUNCTION__, static_cast<int>(status.rxStatus), static_cast<int>(status.txStatus));
...
grpc::Status Cv2xRadioServer::requestCv2xStatus(ServerContext *context,
    const google::protobuf::Empty *request, ::cv2xStub::Cv2xRequestStatusReply *res) {
    LOG(DEBUG, __FUNCTION__);
    Cv2xServerUtil::apiJsonReader(RADIO_API_JSON, RADIO_ROOT, "requestCv2xStatus", res);
...
grpc::Status Cv2xRadioServer::addRxSubscription(ServerContext *context,
    const cv2xStub::RxSubscription *request, cv2xStub::Cv2xCommandReply *res) {
    bool conflict = false;

    Cv2xServerUtil::apiJsonReader(RADIO_API_JSON, RADIO_ROOT, "addRxSubscription", res);
    ...
    if (not rxSubsVec_.empty()) {
        for (cv2xStub::RxSubscription itr : rxSubsVec_) {
            if (itr.iptype() == request->iptype()) {
                if (0 == request->ids_size() || (request->ids_size() > 0 && 0 == itr.ids_size())) {
                    ...
                    conflict = true;
                    LOG(ERROR, __FUNCTION__, " port number conflict ", request->portnum());
                    break;
                } else {
    ...
    if (not conflict) {
        ...
        LOG(DEBUG, __FUNCTION__, " iptype ", request->iptype(), ", port ", request->portnum(),
            " success.");
    } else {
        res->set_status(::commonStub::Status::FAILED);
        res->set_error(::commonStub::ErrorCode::V2X_ERR_SRV_ID_L2_ADDRS_NOT_COMPATIBLE);
    }
    return grpc::Status::OK;
}

grpc::Status Cv2xRadioServer::delRxSubscription(ServerContext *context,
    const cv2xStub::RxSubscription *request, cv2xStub::Cv2xCommandReply *res) {
    LOG(ERROR, __FUNCTION__);
    bool success = false;

    Cv2xServerUtil::apiJsonReader(RADIO_API_JSON, RADIO_ROOT, "delRxSubscription", res);
    if (::commonStub::Status::SUCCESS != res->status()) {
        return grpc::Status::OK;
    }
    ...
    LOG(DEBUG, __FUNCTION__, " iptype ", request->iptype(), ", port ", request->portnum());
    {
        ...
        for (auto it = rxSubsVec_.begin(); it != rxSubsVec_.end();) {
            if (it->iptype() == request->iptype() && it->portnum() == request->portnum()) {
...
grpc::Status Cv2xRadioServer::enableRxMetaDataReport(ServerContext *context,
    const cv2xStub::RxSubscription *request, cv2xStub::Cv2xCommandReply *res) {
    Cv2xServerUtil::apiJsonReader(RADIO_API_JSON, RADIO_ROOT, "enableRxMetaDataReport", res);
    return grpc::Status::OK;
}

::commonStub::Status Cv2xRadioServer::saveFlowInfo(std::map<uint32_t, cv2xStub::FlowInfo> &flows,
...
grpc::Status Cv2xRadioServer::registerFlow(
    ServerContext *context, const cv2xStub::FlowInfo *request, cv2xStub::Cv2xRadioFlowReply *res) {
    int32_t id = -1;
    LOG(DEBUG, __FUNCTION__);
    Cv2xServerUtil::apiJsonReader(RADIO_API_JSON, RADIO_ROOT, "registerFlow", res);
    if (::commonStub::Status::SUCCESS != res->status()) {
        return grpc::Status::OK;
    }
    ...
    cv2xStub::FlowInfo flow     = *request;
    {
        ...
        if (request->spsport() > 0) {
            status = saveFlowInfo(spsFlows_, flow, 0, SIMULATION_SPS_MAX_NUM_FLOWS, id);
        } else if (request->eventport() > 0) {
            status = saveFlowInfo(
                evtFlows_, flow, SIMULATION_EVT_FLOW_BASE, SIMULATION_NON_SPS_MAX_NUM_FLOWS, id);
        }
    }
...
grpc::Status Cv2xRadioServer::deregisterFlow(
    ServerContext *context, const cv2xStub::FlowInfo *request, cv2xStub::Cv2xRadioFlowReply *res) {
    int32_t id = request->flowid();
    LOG(DEBUG, __FUNCTION__, " with flow id ", id);
    Cv2xServerUtil::apiJsonReader(RADIO_API_JSON, RADIO_ROOT, "deregisterFlow", res);
    if (::commonStub::Status::SUCCESS != res->status()) {
        return grpc::Status::OK;
    }
...
grpc::Status Cv2xRadioServer::updateSrcL2Info(ServerContext *context,
    const google::protobuf::Empty *request, cv2xStub::Cv2xCommandReply *res) {
    LOG(DEBUG, __FUNCTION__);
    Cv2xServerUtil::apiJsonReader(RADIO_API_JSON, RADIO_ROOT, "updateSrcL2Info", res);
    if (::commonStub::Status::SUCCESS == res->status()) {
        if (evtListener_) {
...
grpc::Status Cv2xRadioServer::updateTrustedUEList(ServerContext *context,
    const google::protobuf::Empty *request, cv2xStub::Cv2xCommandReply *res) {
    LOG(DEBUG, __FUNCTION__);
    Cv2xServerUtil::apiJsonReader(RADIO_API_JSON, RADIO_ROOT, "updateTrustedUEList", res);

    return grpc::Status::OK;
}

grpc::Status Cv2xRadioServer::getIfaceNameFromIpType(
    ServerContext *context, const cv2xStub::IpType *request, cv2xStub::IfaceNameReply *res) {
    LOG(DEBUG, __FUNCTION__);

    Cv2xServerUtil::apiJsonReader(RADIO_API_JSON, RADIO_ROOT, "getIfaceNameFromIpType", res);
    if (::commonStub::Status::SUCCESS != res->status()) {
        return grpc::Status::OK;
    }
...
grpc::Status Cv2xRadioServer::enableTxStatusReport(
    ServerContext *context, const cv2xStub::UintNum *request, cv2xStub::Cv2xCommandReply *res) {
    LOG(DEBUG, __FUNCTION__);

    Cv2xServerUtil::apiJsonReader(RADIO_API_JSON, RADIO_ROOT, "enableTxStatusReport", res);
    if (::commonStub::Status::SUCCESS != res->status()) {
        return grpc::Status::OK;
    }
...
grpc::Status Cv2xRadioServer::disableTxStatusReport(
    ...
    Cv2xServerUtil::apiJsonReader(RADIO_API_JSON, RADIO_ROOT, "disableTxStatusReport", res);
    if (::commonStub::Status::SUCCESS != res->status()) {
        return grpc::Status::OK;
    }
    {
...
grpc::Status Cv2xRadioServer::setGlobalIPInfo(ServerContext *context,
    const google::protobuf::Empty *request, ::cv2xStub::Cv2xCommandReply *res) {
    LOG(DEBUG, __FUNCTION__);
    Cv2xServerUtil::apiJsonReader(RADIO_API_JSON, RADIO_ROOT, "setGlobalIPInfo", res);
...
grpc::Status Cv2xRadioServer::setGlobalIPUnicastRoutingInfo(ServerContext *context,
    const google::protobuf::Empty *request, ::cv2xStub::Cv2xCommandReply *res) {
    LOG(DEBUG, __FUNCTION__);
    Cv2xServerUtil::apiJsonReader(RADIO_API_JSON, RADIO_ROOT, "setGlobalIPUnicastRoutingInfo", res);
...
grpc::Status Cv2xRadioServer::requestDataSessionSettings(ServerContext *context,
    const google::protobuf::Empty *request, ::cv2xStub::Cv2xCommandReply *res) {
    LOG(DEBUG, __FUNCTION__);
    Cv2xServerUtil::apiJsonReader(RADIO_API_JSON, RADIO_ROOT, "requestDataSessionSettings", res);

    return grpc::Status::OK;
}

grpc::Status Cv2xRadioServer::injectVehicleSpeed(
    ServerContext *context, const cv2xStub::UintNum *request, cv2xStub::Cv2xCommandReply *res) {
    uint32_t speed = request->num();
    LOG(DEBUG, __FUNCTION__, " speed ", speed);

    Cv2xServerUtil::apiJsonReader(RADIO_API_JSON, RADIO_ROOT, "injectVehicleSpeed", res);
