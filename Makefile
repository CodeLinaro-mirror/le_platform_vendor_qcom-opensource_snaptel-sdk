...
apps: sim
	...
	mkdir -p ${ROOTFS}/etc/telux/ && mkdir -p ${ROOTFS}/data/telux/json/ && cp -r simulation/json/ ${ROOTFS}/data/telux/ && cp simulation/conf/tel.conf ${ROOTFS}/etc/telux/ && cp simulation/setup_simualtion.sh ${ROOTFS}/bin/ && mkdir -p ${ROOTFS}/etc/init.d && cp simulation/conf/telsdk_simulation_server ${ROOTFS}/etc/init.d/
