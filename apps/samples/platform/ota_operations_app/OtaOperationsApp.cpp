 ...
 public:
    ...
    int prepareForOTA() {
        ...
        status = fsManager_->prepareForOta(telux::platform::OtaOperation::START,
            [&p](telux::common::ErrorCode errorCode) { p.set_value(errorCode); });
        ...
        if (ec != telux::common::ErrorCode::SUCCESS) {
            std::cout << "Failed to prepare for OTA, err " << static_cast<int>(ec) << std::endl;
            return -EIO;
        }
    ...
    int startPostOTAOperations() {
        ...
        if (status != telux::common::Status::SUCCESS) {
            std::cout << "Can't start post OTA operation, err " << static_cast<int>(status)
                      << std::endl;
            return -EIO;
        }
        ...
        ec = p.get_future().get();
        if (ec != telux::common::ErrorCode::SUCCESS) {
            std::cout << "Failed to start post OTA operation, err " << static_cast<int>(ec)
                      << std::endl;
            return -EIO;
        }
    ...
    int syncABPartitions() {
        ...
        /* Step - 8 */
        status = fsManager_->startAbSync(
            [&p](telux::common::ErrorCode errorCode) { p.set_value(errorCode); });

        if (status != telux::common::Status::SUCCESS) {
            std::cout << "Can't sync partition, err " << static_cast<int>(status) << std::endl;
            return -EIO;
        }

        ec = p.get_future().get();
        if (ec != telux::common::ErrorCode::SUCCESS) {
            std::cout << "Failed to sync partition, err " << static_cast<int>(ec) << std::endl;
            return -EIO;
        }
