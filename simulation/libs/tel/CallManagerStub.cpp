...
CallManagerStub::CallManagerStub() {
  ...
  if (telux::common::DeviceConfig::isMultiSimSupported()) { // For DSDA slot
                                                            // count is 2.
    noOfSlots_ = 2;
  }
...
telux::common::Status
CallManagerStub::init(telux::common::InitResponseCb callback) {
  LOG(DEBUG, __FUNCTION__);
  listenerMgr_ =
      std::make_shared<telux::common::ListenerManager<ICallListener>>();
  if (!listenerMgr_) {
...
void CallManagerStub::initSync() {
  ...
  telux::common::Status status = getInProgressCallsFromServer();
  if (status == telux::common::Status::FAILED) {
    cbStatus = telux::common::ServiceStatus::SERVICE_FAILED;
    LOG(ERROR, __FUNCTION__, " status ", static_cast<int>(cbStatus));
  }
...
void CallManagerStub::setServiceStatus(telux::common::ServiceStatus status) {
  ...
  if (initCb_) {
    auto f1 =
        std::async(std::launch::async, [this, status]() {
...
telux::common::Status
CallManagerStub::registerListener(std::shared_ptr<ICallListener> listener) {
  LOG(DEBUG, __FUNCTION__);
  ...
  if (listenerMgr_) {
    ...
    if (applisteners.size() == 1) {
      auto &clientEventManager =
          telux::common::ClientEventManager::getInstance();
...
telux::common::Status
CallManagerStub::removeListener(std::shared_ptr<ICallListener> listener) {
  LOG(DEBUG, __FUNCTION__);
  telux::common::Status status = telux::common::Status::FAILED;
  if (listenerMgr_) {
    std::vector<std::weak_ptr<ICallListener>> applisteners;
...
telux::common::Status CallManagerStub::dialCall(int phoneId, const std::string &dialNumber,
  ...
  // If a CS conference call is present, callsInProgress will treat as one call
  // even though we have the calls as two seperate in inProgressCalls.
 ...
 * callback and same call object cached by client.
 */

void CallManagerStub::findMatchingCall(
    int index, std::string remotePartyNumber, int phoneId, int cbDelay,
    std::shared_ptr<IMakeCallCallback> iMakecallback, MakeCallCallback callback,
    telux::common::ErrorCode error) {
  LOG(DEBUG, __FUNCTION__, " phoneId:: ", phoneId, "errorcode is ",
      static_cast<int>(error));
  std::vector<std::shared_ptr<CallStub>>::iterator iter;
  std::lock_guard<std::mutex> lock(callManagerMutex_);
  iter = std::find_if(std::begin(calls_), std::end(calls_),
                      [=](std::shared_ptr<CallStub> call) {
  ...
  if (iter != std::end(calls_)) {
    ...
    if (iMakecallback) {
      auto f =
          std::async(std::launch::async, [this, error, iter, iMakecallback,
                                          cbDelay, value]() {
    ...
    if (callback) {
      auto f =
          std::async(std::launch::async, [this, error, iter, callback, cbDelay,
                                          value]() {
            std::this_thread::sleep_for(std::chrono::milliseconds(cbDelay));
            LOG(DEBUG, __FUNCTION__, " invoking callback");
...
bool CallManagerStub::find(int phoneId, std::shared_ptr<CallStub> call,
  ...
  if (call->getCallIndex() != INVALID_CALL_INDEX) {
    // Remote party number is known by client for a custom number ecall over
    // PS/CS or voice call when the call is dialed
    if ((call->getRemotePartyNumber() == remotePartyNumber) &&
        (call->getPhoneId() == phoneId) && (call->getCallIndex() == index)) {
      return true;
    } else if ((call->getRemotePartyNumber() == "") &&
               (call->getPhoneId() == phoneId) &&
               (call->getCallIndex() == index)) {
      // Remote party number is not known by client for a standard ecall when
      // the call is dialed.
      return true;
    } else {
      return false;
...
telux::common::Status
CallManagerStub::makeECall(int phoneId, const std::string dialNumber,
...
void CallManagerStub::onEventUpdate(google::protobuf::Any event) {
  ...
  } else if (event.Is<::telStub::GetInProgressCallsData>()) {
    ::telStub::GetInProgressCallsData callevent;
    event.UnpackTo(&callevent);
...
void CallManagerStub::handleModifyCallRequest(
    ::telStub::ModifyCallRequestEvent event) {
  int phoneId = event.phone_id();
  ...
  if (listenerMgr_) {
    ...
    for (auto &wp : applisteners) {
      if (auto sp = wp.lock()) {
        // RttMode is hardcoded to FULL because modem invokes notification only
        // during upgrade of the call.
...
void CallManagerStub::handleMsdUpdateRequest(
    ::telStub::MsdPullRequestEvent event) {
  int phoneId = event.phone_id();
  std::vector<std::weak_ptr<ICallListener>> applisteners;
  if (listenerMgr_) {
    listenerMgr_->getAvailableListeners(applisteners);
...
void CallManagerStub::handleCallInfoChanged(
    ::telStub::GetInProgressCallsData event) {
  int phoneId = event.phone_id();
  ...
  for (int i = 0; i < event.calls_size(); i++) {
    CallInfo callInfo;
    callInfo.callState =
        static_cast<telux::tel::CallState>(event.calls(i).call_state());
    ...
    callInfo.networkMode =
        static_cast<telux::tel::NetworkMode>(event.calls(i).network_mode());
    LOG(DEBUG, __FUNCTION__,
        " CallState: ", static_cast<int>(callInfo.callState),
...
telux::common::Status CallManagerStub::getInProgressCallsFromServer() {
  telux::common::Status status = telux::common::Status::FAILED;
  for (int i = 1; i <= noOfSlots_; i++) {
    ::telStub::GetInProgressCallsRequest request;
    ...
    grpc::Status reqstatus =
        stub_->getInProgressCalls(&context, request, &response);
    if (reqstatus.ok()) {
      status = telux::common::Status::SUCCESS;
      handleCallInfoChanged(response);
    } else {
...
void CallManagerStub::updateCurrentCalls() {
  for (int i = 1; i <= noOfSlots_; i++) {
    ...
    if (reqstatus.ok()) {
      LOG(DEBUG, __FUNCTION__,
          " Requested calls information for slot is successful ");
    } else {
      LOG(ERROR, __FUNCTION__, " Requested calls information for slot failed ");
    }
  }
...
void CallManagerStub::refreshCachedCalls(
  ...
  std::vector<std::shared_ptr<CallStub>> callsToBeNotified;
  {
    ...
    // we are taking each cached call and checking whether it matches any of the
    // calls in the latest call list. Id it is found then we update the cached
    // call info. If it is not found we assume that the modem has dropped the
    // call. So we add it to the dropped call list.
    for (auto cachedCall = std::begin(calls_);
         cachedCall != std::end(calls_);) {
      if ((*cachedCall)->getPhoneId() != phoneId) {
        // ignore if this function was called for a different slot.
        cachedCall++;
        continue;
      }
      auto iter = std::find_if(std::begin(latestCalls), std::end(latestCalls),
                               [=](std::shared_ptr<CallStub> latestCallInfo) {
                                 return (*cachedCall)->match(latestCallInfo);
                               });

      if (iter != std::end(latestCalls)) { // cached call found, update the call
                                           // with latest call info from ril
        ...
        LOG(DEBUG,
            "dropped call found, adding it to droppedCalls_ list and removing "
            "from "
            "calls_ list., Call pointer address ",
            *cachedCall);
        (*cachedCall)->logCallDetails(); // Logging call details for debugging
                                         // purposes only
  ...
  for (auto callIter = std::begin(callsToBeNotified);
       callIter != std::end(callsToBeNotified); ++callIter) {
    if ((*callIter)->getCallState() != CallState::CALL_ENDED) {
      /* Call ended notification is informed to application only after server
       * drops the call.
       */
...
void CallManagerStub::addLatestCalls(
  ...
  for (std::shared_ptr<CallStub> ci : latestCalls) {
    ...
    if (ci->getCallState() == CallState::CALL_INCOMING ||
      ...
      LOG(DEBUG, __FUNCTION__,
          " CallManager: notifying listeners about the new call");
...
void CallManagerStub::handleECallEvent(::telStub::ECallInfoEvent event) {
  ...
  if (input == "T2Timer") {
    ...
    invokeECallMsdTransmissionStatuslisteners(
        slotId, ECallMsdTransmissionStatus::START_RECEIVED);
  } else if (input == "MSD_TRANSMISSION_STARTED") {
    invokeECallMsdTransmissionStatuslisteners(
        slotId, ECallMsdTransmissionStatus::MSD_TRANSMISSION_STARTED);
  } else if (input == "LL_ACK_RECEIVED") {
    invokeECallMsdTransmissionStatuslisteners(
        slotId, ECallMsdTransmissionStatus::LL_ACK_RECEIVED);
  } else if (input == "MSD_TRANSMISSION_SUCCESS") {
    invokeECallMsdTransmissionStatuslisteners(
        slotId, ECallMsdTransmissionStatus::SUCCESS);
    invokeECallMsdTransmissionStatuslisteners(
        slotId, telux::common::ErrorCode::SUCCESS);
  } else if (input == "MSD_TRANSMISSION_FAILURE") {
    invokeECallMsdTransmissionStatuslisteners(
        slotId, ECallMsdTransmissionStatus::FAILURE);
    invokeECallMsdTransmissionStatuslisteners(
    ...
    invokeECallMsdTransmissionStatuslisteners(
        slotId, ECallMsdTransmissionStatus::MSD_AL_ACK_CLEARDOWN);
  } else {
...
void CallManagerStub::invokeECallHlapTimerEventlisteners(
    int phoneId, ECallHlapTimerEvents timersStatus) {
  LOG(DEBUG, __FUNCTION__);
  std::vector<std::weak_ptr<ICallListener>> applisteners;
  if (listenerMgr_) {
    listenerMgr_->getAvailableListeners(applisteners);
...
CallManagerStub::makeECall(int phoneId, const std::string dialNumber,
                           ...
                           CustomSipHeader header, MakeCallCallback callback) {
  LOG(DEBUG, "CallManager - ", __FUNCTION__);
  if (phoneId <= 0 || phoneId > noOfSlots_) {
    LOG(DEBUG, __FUNCTION__, " Invalid PhoneId");
    return telux::common::Status::INVALIDPARAM;
...
telux::common::Status CallManagerStub::makeECall(int phoneId,
...
telux::common::Status CallManagerStub::updateECallMsd(
    int phoneId, const ECallMsdData &eCallMsd,
  ...
  if (reqstatus.ok()) {
    ...
    if (callback) {
      auto f =
          std::async(std::launch::async, [this, error, callback, cbDelay]() {
            std::this_thread::sleep_for(std::chrono::milliseconds(cbDelay));
...
telux::common::Status
CallManagerStub::updateECallMsd(int phoneId, const std::vector<uint8_t> &msdPdu,
...
telux::common::Status CallManagerStub::requestECallHlapTimerStatus(
    int phoneId, ECallHlapTimerStatusCallback callback) {
  LOG(DEBUG, "CallManager - ", __FUNCTION__);
  ...
  grpc::Status reqstatus =
      stub_->RequestECallHlapTimerStatus(&context, request, &response);
  telux::common::Status status = telux::common::Status::FAILED;
  if (reqstatus.ok()) {
    telux::common::ErrorCode error =
        static_cast<telux::common::ErrorCode>(response.error());
    ...
    if (callback) {
      auto f =
          std::async(std::launch::async, [this, error, phoneId, timersStatus,
                                          callback, cbDelay]() {
            std::this_thread::sleep_for(std::chrono::milliseconds(cbDelay));
...
telux::common::Status CallManagerStub::conference(
    std::shared_ptr<ICall> call1, std::shared_ptr<ICall> call2,
    std::shared_ptr<telux::common::ICommandResponseCallback> callback) {
  return telux::common::Status::NOTSUPPORTED;
}

telux::common::Status CallManagerStub::swap(
    std::shared_ptr<ICall> callToHold, std::shared_ptr<ICall> callToActivate,
    std::shared_ptr<telux::common::ICommandResponseCallback> callback) {
  LOG(DEBUG, "CallMgr - ", __FUNCTION__);
  if ((callToHold == nullptr) || (callToActivate == nullptr)) {
    LOG(ERROR,
        "Unable to initiate swap operation as null call objects are passed");
    return telux::common::Status::INVALIDPARAM;
  }
  if (telux::common::ServiceStatus::SERVICE_AVAILABLE != getServiceStatus()) {
    LOG(ERROR, __FUNCTION__, " Call Manager is not ready");
  ...
  if (callToHold->getPhoneId() != callToActivate->getPhoneId()) {
    LOG(ERROR, "Unable to initiate swap operation as phoneId for both calls "
               "are different");
    return telux::common::Status::INVALIDPARAM;
  }
  ...
  if (reqstatus.ok()) {
    ...
    if ((status == telux::common::Status::SUCCESS) && (isCallbackNeeded)) {
      if (callback) {
        auto f =
            std::async(std::launch::async, [this, error, callback, delay]() {
...
telux::common::Status CallManagerStub::hangupForegroundResumeBackground(
    int phoneId, common::ResponseCallback callback) {
  ...
  request.set_phone_id(phoneId);
  grpc::Status reqstatus =
      stub_->HangupForegroundResumeBackground(&context, request, &response);
  telux::common::Status status = telux::common::Status::FAILED;
  if (reqstatus.ok()) {
    telux::common::ErrorCode error =
        static_cast<telux::common::ErrorCode>(response.error());
...
telux::common::Status
CallManagerStub::hangupWaitingOrBackground(int phoneId,
  ...
  request.set_phone_id(phoneId);
  grpc::Status reqstatus =
      stub_->HangupWaitingOrBackground(&context, request, &response);
  telux::common::Status status = telux::common::Status::FAILED;
  if (reqstatus.ok()) {
    telux::common::ErrorCode error =
        static_cast<telux::common::ErrorCode>(response.error());
...
telux::common::Status
CallManagerStub::requestEcbm(int phoneId, EcbmStatusCallback callback) {
  LOG(DEBUG, __FUNCTION__, " phoneId:", phoneId);
  if (phoneId <= 0 || phoneId > noOfSlots_) {
  ...
  if (reqstatus.ok()) {
    ...
    status = static_cast<telux::common::Status>(response.status());
    int cbDelay = static_cast<int>(response.delay());
    telux::tel::EcbMode ecbMode =
        static_cast<telux::tel::EcbMode>(response.ecbmode());
    bool isCallbackNeeded = static_cast<bool>(response.iscallback());
    if ((status == telux::common::Status::SUCCESS) && (isCallbackNeeded)) {
      if (callback) {
        auto f =
            std::async(std::launch::async, [this, error, callback, ecbMode,
                                            cbDelay]() {
              std::this_thread::sleep_for(std::chrono::milliseconds(cbDelay));
...
telux::common::Status
CallManagerStub::exitEcbm(int phoneId, common::ResponseCallback callback) {
  LOG(DEBUG, __FUNCTION__, " phoneId:", phoneId);
  if (phoneId <= 0 || phoneId > noOfSlots_) {
    LOG(ERROR, __FUNCTION__, " Invalid PhoneId");
    return telux::common::Status::INVALIDPARAM;
  ...
  if (reqstatus.ok()) {
    ...
    if ((status == telux::common::Status::SUCCESS) && (isCallbackNeeded)) {
      auto f =
          std::async(std::launch::async, [this, error, cbDelay, callback]() {
            std::this_thread::sleep_for(std::chrono::milliseconds(cbDelay));
            callback(error);
          }).share();
      taskQ_->add(f);
...
telux::common::Status CallManagerStub::requestNetworkDeregistration(
    int phoneId, common::ResponseCallback callback) {
  LOG(DEBUG, __FUNCTION__, " phoneId:", phoneId);
  if (phoneId <= 0 || phoneId > noOfSlots_) {
    LOG(ERROR, __FUNCTION__, " Invalid PhoneId");
  ...
  request.set_phone_id(phoneId);
  grpc::Status reqstatus =
      stub_->RequestNetworkDeregistration(&context, request, &response);
  telux::common::Status status = telux::common::Status::FAILED;
  if (reqstatus.ok()) {
    telux::common::ErrorCode error =
        static_cast<telux::common::ErrorCode>(response.error());
...
telux::common::Status
CallManagerStub::updateEcallHlapTimer(int phoneId, HlapTimerType type,
  ...
  if (type == HlapTimerType::T10_TIMER) {
    ...
    grpc::Status reqstatus =
        stub_->UpdateEcallHlapTimer(&context, request, &response);
    if (reqstatus.ok()) {
      telux::common::ErrorCode error =
          static_cast<telux::common::ErrorCode>(response.error());
      status = static_cast<telux::common::Status>(response.status());
...
telux::common::Status
CallManagerStub::requestEcallHlapTimer(int phoneId, HlapTimerType type,
  ...
  if (type == HlapTimerType::T10_TIMER) {
    ...
    request.set_type(static_cast<::telStub::HlapTimerType>(type));
    grpc::Status reqstatus =
        stub_->RequestEcallHlapTimer(&context, request, &response);
    if (reqstatus.ok()) {
      telux::common::ErrorCode error =
          static_cast<telux::common::ErrorCode>(response.error());
      status = static_cast<telux::common::Status>(response.status());
...
telux::common::Status CallManagerStub::getECallConfig(EcallConfig &config) {
  ...
  if (reqstatus.ok()) {
    ...
    if (response.is_num_type_valid() == true) {
      ...
      config.numType =
          static_cast<telux::tel::ECallNumType>(response.num_type());
    }
...
telux::common::Status CallManagerStub::encodeEuroNcapOptionalAdditionalData(
  ...
  auto status = eCallMsd.encodeEuroNcapOptionalAdditionalDataContent(
      optionalEuroNcapData, data);
  return status;
}

telux::common::ErrorCode
CallManagerStub::encodeECallMsd(telux::tel::ECallMsdData eCallMsdData,
                                std::vector<uint8_t> &data) {
  LOG(DEBUG, __FUNCTION__);
...
telux::common::Status
CallManagerStub::makeRttCall(int phoneId, const std::string &dialNumber,
                             std::shared_ptr<IMakeCallCallback> callback) {
  LOG(DEBUG, __FUNCTION__, " Phone Id ", phoneId, " dial number ", dialNumber);
...
telux::common::Status
CallManagerStub::sendRtt(int phoneId, std::string message,
                         common::ResponseCallback callback) {
  ...
  ClientContext context;
  // Text message obtained from the user is not sent to server as there is no
  // need to store or manipulate messages at server.
  request.set_phone_id(phoneId);
  telux::common::Status status = telux::common::Status::FAILED;
...
telux::common::Status
CallManagerStub::configureECallRedial(RedialConfigType config,
  ...
  grpc::Status reqstatus =
      stub_->ConfigureECallRedial(&context, request, &response);
  if (reqstatus.ok()) {
    telux::common::ErrorCode error =
        static_cast<telux::common::ErrorCode>(response.error());
    status = static_cast<telux::common::Status>(response.status());
...
telux::common::Status
CallManagerStub::restartECallHlapTimer(int phoneId, EcallHlapTimerId timerId,
                                       int duration,
                                       common::ResponseCallback callback) {
  if (getServiceStatus() != telux::common::ServiceStatus::SERVICE_AVAILABLE) {
    LOG(ERROR, __FUNCTION__, " CallManager is not ready ");
  ...
  ::telStub::RestartECallHlapTimerRequest request;
  ::telStub::RestartECallHlapTimerResponse response;
  ClientContext context;
  telux::common::Status status = telux::common::Status::FAILED;

  request.set_phone_id(phoneId);
  request.set_timer_id(static_cast<telStub::EcallHlapTimerId>(timerId));
  request.set_timer_duration(duration);
  grpc::Status reqstatus =
      stub_->restartECallHlapTimer(&context, request, &response);
  if (reqstatus.ok()) {
    telux::common::ErrorCode error =
        static_cast<telux::common::ErrorCode>(response.error());
    status = static_cast<telux::common::Status>(response.status());
...
CallManagerStub::makeECall(int phoneId, const std::string dialNumber,
  ...
  ::telStub::MakeECallRequest request =
      createRequest<::telStub::MakeECallRequest>(
          phoneId, dialNumber, true, makeSelfTestERAGLONASSECallWithRawMsd);
  ::telStub::MakeECallReply response;
  ClientContext context;
...
telux::common::Status CallManagerStub::updateECallPostTestRegistrationTimer(
    int phoneId, uint32_t timer, common::ResponseCallback callback) {
  if (getServiceStatus() != telux::common::ServiceStatus::SERVICE_AVAILABLE) {
    LOG(ERROR, __FUNCTION__, " CallManager is not ready ");
    return telux::common::Status::NOTREADY;
  }
  ::telStub::UpdateECallPostTestRegistrationTimerRequest request;
  ...
  grpc::Status reqstatus =
      stub_->updateECallPostTestRegistration(&context, request, &response);
  if (reqstatus.ok()) {
    telux::common::ErrorCode error =
        static_cast<telux::common::ErrorCode>(response.error());
    status = static_cast<telux::common::Status>(response.status());
...
telux::common::ErrorCode
CallManagerStub::getECallPostTestRegistrationTimer(int phoneId,
                                                   uint32_t &timer) {
  if (getServiceStatus() != telux::common::ServiceStatus::SERVICE_AVAILABLE) {
    LOG(ERROR, __FUNCTION__, " CallManager is not ready ");
    return telux::common::ErrorCode::INVALID_STATE;
  }
  ::telStub::GetECallPostTestRegistrationTimerRequest request;
  ...
  request.set_phone_id(phoneId);
  grpc::Status reqstatus =
      stub_->getECallPostTestRegistrationTimer(&context, request, &response);
  if (reqstatus.ok()) {
    errorCode = static_cast<telux::common::ErrorCode>(response.error());
    timer = static_cast<int>(response.timer_duration());
  }
  return errorCode;
}

telux::common::ErrorCode
CallManagerStub::getECallRedialConfig(std::vector<int> &callOrigTimeGap,
  ...
  telux::common::ErrorCode errorCode =
      telux::common::ErrorCode::GENERIC_FAILURE;
  grpc::Status reqstatus =
      stub_->getECallRedialConfig(&context, request, &response);
  if (reqstatus.ok()) {
    errorCode = static_cast<telux::common::ErrorCode>(response.error());
    for (auto &r : (response.call_orig_time_gap())) {
      int tmp = static_cast<int>(r);
      LOG(DEBUG, __FUNCTION__, " call orig time gap ", tmp);
      callOrigTimeGap.emplace_back(tmp);
    }
    for (auto &r : (response.call_drop_time_gap())) {
      int tmp = static_cast<int>(r);
      LOG(DEBUG, __FUNCTION__, " call drop time gap ", tmp);
      callDropTimeGap.emplace_back(tmp);
    }
  }
  return errorCode;
}
...
telux::common::Status
CallManagerStub::setEmergencyMode(int phoneId, bool emergencyModeEnabled,
  ...
  grpc::Status reqstatus =
      stub_->setEmergencyMode(&context, request, &response);
  if (reqstatus.ok()) {
    telux::common::ErrorCode error =
        static_cast<telux::common::ErrorCode>(response.error());
    status = static_cast<telux::common::Status>(response.status());
