...
telux::common::Status EventInjector::init() {
    telux::common::ErrorCode readError = JsonParser::readFromJsonFile(eventObj_, EVENT_JSON);
...
telux::common::Status EventInjector::parseAndHandleArguments(int argc, char **argv) {
    ...
    static struct option long_options[]
        = {{"help", optional_argument, 0, 'h'}, {"filter", required_argument, 0, 'f'},
            {"event", required_argument, 0, 'e'}, {NULL, 0, 0, '\0'}};
...
telux::common::Status EventInjector::sendMessage(std::string filter, std::string event) {
    ...
    std::string reformattedString
        = FILTER_FLAG + " " + filter + " " + EVENT_FLAG + " " + event + "\n";
