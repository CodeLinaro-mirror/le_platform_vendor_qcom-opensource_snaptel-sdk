...
grpc::Status SuppServicesManagerServerImpl::InitService(ServerContext *context,
    ...
    std::string cbStatus = rootObj[TEL_SUPP_SERVICES_MANAGER]["IsSubsystemReady"].asString();
    ...
    if (status != telux::common::ServiceStatus::SERVICE_AVAILABLE) {
        ...
        return grpc::Status(
            grpc::StatusCode::INTERNAL, " Json not found or service not available or failed");
    }
...
grpc::Status SuppServicesManagerServerImpl::SetCallWaitingPref(ServerContext *context,
    ...
    if (data.status == telux::common::Status::SUCCESS) {
        ::telStub::SuppServicesStatus::Status suppServicesStatus = request->supp_services_status();
    ...
    std::string failureCause
        = data.apiRootObj[TEL_SUPP_SERVICES_MANAGER]["failureCause"].asString();
    LOG(DEBUG, __FUNCTION__, " failureCause : ", failureCause);
    long value = CommonUtils::convertHexToInt(failureCause);

    response->set_failure_cause(static_cast<telStub::SuppServicesFailureCause_FailureCause>(value));
    ...
    if (data.status == telux::common::Status::SUCCESS) {
        ::telStub::SuppServicesStatus_Status suppServicesStatus
            = static_cast<::telStub::SuppServicesStatus_Status>(
                data.stateRootObj[TEL_SUPP_SERVICES_MANAGER]["CallWaitingPref"]["SuppServicesStatu"
                                                                                "s"]
                    .asInt());
...
grpc::Status SuppServicesManagerServerImpl::SetForwardingPref(ServerContext *context,
    const ::telStub::SetForwardingPrefRequest *request, telStub::SetForwardingPrefReply *response) {
    LOG(DEBUG, __FUNCTION__);
    int slotId                = request->slot_id();
    std::string apiJsonPath   = (slotId == DEFAULT_SLOT_ID) ? JSON_PATH1 : JSON_PATH2;
    std::string stateJsonPath = (slotId == DEFAULT_SLOT_ID) ? JSON_PATH3 : JSON_PATH4;
    ...
    if (data.status == telux::common::Status::SUCCESS) {
        ...
        LOG(DEBUG, __FUNCTION__, " forwardOperation : ", forwardOperation,
            " forwardReason : ", forwardReason);
        int currentCount
            = data.stateRootObj[TEL_SUPP_SERVICES_MANAGER]["CallForwardingPref"]["ForwardInfoList"]
                  .size();
        ...
        for (int j = 0; j < currentCount; j++) {
            ...
            if ((data.stateRootObj[TEL_SUPP_SERVICES_MANAGER]["CallForwardingPref"]
                                  ["ForwardInfoList"][j]["ForwardReason"])
                == newconfig["ForwardReason"]) {
                ...
                if (newconfig["Number"] == ""
                    ...
                    LOG(ERROR, __FUNCTION__,
                        " Before activating/deactivating supplementary"
                        " services register it first.");
                    newconfig["SuppServicesStatus"]
                        = static_cast<int>(telux::tel::SuppServicesStatus::DISABLED);
                    data.error = telux::common::ErrorCode::SUPS_FAILURE_CAUSE;
                }
                data.stateRootObj[TEL_SUPP_SERVICES_MANAGER]["CallForwardingPref"]["ForwardInfoLis"
                                                                                   "t"][j]
                    = newconfig;
                reasonFound = true;
                break;
        ...
        for (int j = 0; j < count; j++) {
            if ((data.stateRootObj[TEL_SUPP_SERVICES_MANAGER]["CallForwardingPref"]
                ...
                ::telStub::SuppServicesStatus_Status suppServicesStatus
                    = static_cast<telStub::SuppServicesStatus_Status>(
                        data.stateRootObj[TEL_SUPP_SERVICES_MANAGER]["CallForwardingPref"]
                                         ["ForwardInfoList"][j]["SuppServicesStatus"]
                                             .asInt());
                result->set_status(suppServicesStatus);
                int serviceClassSize
                    = data.stateRootObj[TEL_SUPP_SERVICES_MANAGER]["CallForwardingPref"]
                                       ["ForwardInfoList"][j]["ServiceClass"]
                                           .size();
                for (int idx = 0; idx < serviceClassSize; idx++) {
                    result->add_service_class(static_cast<telStub::ServiceClassType_Type>(idx));
                }
                string number = data.stateRootObj[TEL_SUPP_SERVICES_MANAGER]["CallForwardingPref"]
                                                 ["ForwardInfoList"][j]["Number"]
                                                     .asString();
                result->set_number(number);
                uint8_t noReplyTimer
                    = data.stateRootObj[TEL_SUPP_SERVICES_MANAGER]["CallForwardingPref"]
                                       ["ForwardInfoList"][j]["NoReplyTimer"]
                                           .asInt();
...
grpc::Status SuppServicesManagerServerImpl::SetOirPref(ServerContext *context,
    const ::telStub::SetOirPrefRequest *request, telStub::SetOirPrefReply *response) {
    LOG(DEBUG, __FUNCTION__);
    int slotId                = request->slot_id();
    std::string apiJsonPath   = (slotId == DEFAULT_SLOT_ID) ? JSON_PATH1 : JSON_PATH2;
    std::string stateJsonPath = (slotId == DEFAULT_SLOT_ID) ? JSON_PATH3 : JSON_PATH4;
    ...
    if (data.status == telux::common::Status::SUCCESS) {
        ::telStub::SuppServicesStatus_Status suppServicesStatus = request->supp_services_status();
        ...
        data.stateRootObj[TEL_SUPP_SERVICES_MANAGER]["CallOirPref"]["ServiceClass"] = value;
        JsonParser::writeToJsonFile(data.stateRootObj, stateJsonPath);
    }
    ...
    if (data.status == telux::common::Status::SUCCESS) {
        ...
        ::telStub::SuppSvcProvisionStatus_Status provisionStatus
            = static_cast<telStub::SuppSvcProvisionStatus_Status>(
                data.apiRootObj[TEL_SUPP_SERVICES_MANAGER]["requestOirPref"]["suppSvcProvisionStatu"
                                                                             "s"]
                    .asInt());
