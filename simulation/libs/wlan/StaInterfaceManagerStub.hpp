 ...
 */
class StaInterfaceManagerStub
    : public IStaInterfaceManager,
      public IStaListener,
...
public:
  ...
  registerListener(std::weak_ptr<IStaListener> listener) override;
  telux::common::ErrorCode
  deregisterListener(std::weak_ptr<IStaListener> listener) override;

  telux::common::ErrorCode
  setIpConfig(Id staId, StaIpConfig ipConfig,
              StaStaticIpConfig staticIpConfig) override;
  telux::common::ErrorCode setBridgeMode(Id staId,
                                         StaBridgeMode bridgeMode) override;
  ...
  telux::common::ErrorCode
  addNetworkConfig(Id staId, const StaNetworkConfigEntry &network) override;
  telux::common::ErrorCode removeNetworkConfig(Id staId,
                                               NetworkId networkId) override;
  telux::common::ErrorCode
  getNetworkConfigs(Id staId,
                    std::vector<StaNetworkConfigInfo> &network) override;
  telux::common::ErrorCode connect(Id staId, NetworkId networkId) override;
  telux::common::ErrorCode disconnect(Id staId) override;
  telux::common::ErrorCode manageStaService(Id staId,
...
private:
  ...
  std::unique_ptr<::wlanStub::StaInterfaceService::Stub> stub_;
  std::shared_ptr<telux::common::AsyncTaskQueue<void>> taskQ_;
  std::shared_ptr<telux::common::ListenerManager<IStaListener>> listenerMgr_;

  void handleStaStatusChangedEvent(
      ::wlanStub::OnStaStatusChanged stationStatusChangedEvent);
  void handleStaScanResultUpdatedEvent(
      ::wlanStub::OnStaScanResultUpdated scanResultUpdatedEvent);
  void handleStaBandChangedEvent(
      ::wlanStub::OnStaBandChanged stationBandChangedEvent);
};
