...
class TransmitApduCallback : public telux::tel::ICardCommandCallback, public ResponseData {
 public:
...
class OpenLogicalChannelCallback : public telux::tel::ICardChannelCallback, public ResponseData {
 public:
...
void ApSimProfileClient::MyApCardListener::onRefreshEvent(int slotId,
    ...
    std::cout << std::endl << std::endl;
    PRINT_NOTIFICATION
        << "onRefreshEvent on slot" << slotId << " ,Refresh Stage is "
        << refreshStageToString(stage) << " ,Refresh Mode is " << refreshModeToString(mode)
        << " ,Session Type is " << sessionTypeToString(config.sessionType)
        << ((!config.aid.empty()) ? " ,AID is " : "") << ((!config.aid.empty()) ? config.aid : "")
        << "\n";
...
telux::common::Status ApSimProfileClient::init() {
    ...
    apSimProfileManager_ = phoneFactory.getApSimProfileManager(
        [&](telux::common::ServiceStatus status) { apSimProfileMgrProm.set_value(status); });
...
telux::common::Status ApSimProfileClient::registerRefresh(SlotId slotId) {
    ...
    if (cardManager_) {
        status = cardManager_->setupRefreshConfig(
            slotId, true, false, efFiles, config, CardRefreshResponseCallback::commandResponse);
    }
    return status;
}
...
std::string ApSimProfileClient::getSwappedIccidString(const std::string &data) {
    ...
    if (!reverseString.empty()) {
        reverseString.erase(
            std::remove(reverseString.begin(), reverseString.end(), ' '), reverseString.end());
    }
...
int ApSimProfileClient::transmitApdu(
    SlotId slotId, int channel, std::vector<uint8_t> data, bool isGetProfile) {
    std::cout << std::endl;
    int result = -1;
    if (data.empty()) {
    ...
    if (card) {
        status
            = card->transmitApduLogicalChannel(channel, cla, ins, p1, p2, p3, data, apduLogicalCb);
        if (status == telux::common::Status::SUCCESS) {
            ...
            if (apduLogicalCb->getFuture().get() == telux::common::ErrorCode::SUCCESS) {
                ...
                if ((apduData->getResult().sw1 == SW1_NO_ERROR)
                    ...
                    while (sw1 == SW1_MORE_RESPONSE) {
                        ...
                        status = card->transmitApduLogicalChannel(channel, 0, INS_GET_MORE_RESPONSE,
                            0, 0, remainingBytes, data, apduMoreLogicalCb);
                        if (status == telux::common::Status::SUCCESS) {
                            ...
                            if (apduMoreLogicalCb->getFuture().get()
                                ...
                                auto apduMoreData
                                    = std::static_pointer_cast<ResponseData>(apduMoreLogicalCb)
                                          .get();
...
telux::common::Status ApSimProfileClient::enableProfile() {
    ...
    if (apSimProfileManager_) {
        ...
        status = apSimProfileManager_->sendProfileOperationResponse(
            slotId, apduResult, referenceId, MyApSimProfileCallback::onResponseCallback);
    } else {
        std::cout << "ERROR - ApSimProfileManger is null" << std::endl;
    }
    return status;
