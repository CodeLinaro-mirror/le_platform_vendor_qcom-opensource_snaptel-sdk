...
bool AecsCall::init() {
    ...
    telux::common::Status status =
        callMgr_->registerListener(callListener_);
    ...
    if (menuOptionsAdded_ == false) {
        ...
        std::shared_ptr<ConsoleAppCommand> setEmergencyModeCommand =
            std::make_shared<ConsoleAppCommand>(
                ConsoleAppCommand("7", "Set_Emergency_Mode", {},
...
void AecsCall::sendAecsMessage(std::vector<std::string> userInput) {
    ...
    auto cb = [weakBaseListener, phoneId](std::vector<int> msgRefs, telux::common::ErrorCode error)
    {
