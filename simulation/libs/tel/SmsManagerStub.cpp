...
telux::common::Status SmsManagerStub::removeListener(std::weak_ptr<ISmsListener> listener) {
    LOG(DEBUG, __FUNCTION__);
...
void SmsManagerStub::invokeDeliveryReportListener(
    ...
    for (int i = 0; i < noofdeliveryreport; i++) {
        ...
        if (listenerMgr_) {
            ...
            for (auto &wp : applisteners) {
                if (auto sp = wp.lock()) {
                    sp->onDeliveryReport(
                        phoneId_, infos[i].msgRef, receiverAddress, infos[i].errorCode);
                }
            }
        } else {
...
telux::common::Status SmsManagerStub::sendRawSmsEx(
    const std::vector<PduBuffer> rawPdus, SmsResponseCbEx sentCallback) {
    LOG(DEBUG, __FUNCTION__);
    if (rawPdus.empty()) {
...
telux::common::Status SmsManagerStub::sendRawSms(
    const std::vector<PduBuffer> rawPdus, SmsResponseCb sentCallback) {
    LOG(DEBUG, __FUNCTION__);
    if (rawPdus.empty()) {
        LOG(ERROR, __FUNCTION__, " Raw PDU is empty");
        return telux::common::Status::INVALIDPARAM;
...
void SmsManagerStub::invokeCallback(
    int cbDelay, ErrorCode error, std::vector<int> msgRefs, SmsResponseCb sentCallback) {
    LOG(DEBUG, __FUNCTION__);
...
telux::common::Status SmsManagerStub::requestSmscAddress(
    ...
    if ((status == telux::common::Status::SUCCESS) && (isCallbackNeeded)) {
        auto f = std::async(std::launch::async, [this, cbDelay, callback, smscAddress, error]() {
...
telux::common::Status SmsManagerStub::setSmscAddress(
    const std::string &smscAddress, telux::common::ResponseCallback callback) {
...
void SmsManagerStub::invokeResponseCallback(
    int cbDelay, telux::common::ErrorCode error, telux::common::ResponseCallback callback) {
    LOG(DEBUG, __FUNCTION__);
    std::this_thread::sleep_for(std::chrono::milliseconds(cbDelay));
...
telux::common::Status SmsManagerStub::requestSmsMessageList(
    SmsTagType type, RequestSmsInfoListCb callback, StorageType storageType) {
    LOG(DEBUG, __FUNCTION__);
    ...
    for (int i = 0; i < response.meta_info_size(); i++) {
        ...
        info.tagType
            = static_cast<telux::tel::SmsTagType>(response.mutable_meta_info(i)->tag_type());
        LOG(DEBUG, __FUNCTION__, "msgIndex ", info.msgIndex, "tagType ",
    ...
    if ((status == telux::common::Status::SUCCESS) && (isCallbackNeeded)) {
        auto f = std::async(std::launch::async, [this, infos, error, callback, cbDelay]() {
...
telux::common::Status SmsManagerStub::readMessage(
    uint32_t messageIndex, ReadSmsMessageCb callback, StorageType storageType) {
    LOG(DEBUG, __FUNCTION__);
    ...
    int numberOfSegments = static_cast<int>((response.sms_message()).messageinfono_of_segments());
    ...
    if ((status == telux::common::Status::SUCCESS) && (isCallbackNeeded)) {
        auto f1 = std::async(std::launch::async, [this, msg, error, callback, cbDelay]() {
...
void SmsManagerStub::invokeReadSmsMessageCb(
    SmsMessage message, telux::common::ErrorCode error, ReadSmsMessageCb callback, int cbDelay) {
    LOG(DEBUG, __FUNCTION__);
    std::this_thread::sleep_for(std::chrono::milliseconds(cbDelay));
...
telux::common::Status SmsManagerStub::deleteMessage(
    DeleteInfo info, telux::common::ResponseCallback callback) {
    LOG(DEBUG, __FUNCTION__, " PhoneId: ", phoneId_, " MessageIndex: ", info.msgIndex,
        " Delete Type: ", static_cast<int>(info.delType),
        " SMS Tag Type: ", static_cast<int>(info.tagType),
        "Storage Type: ", static_cast<int>(info.storageType));
    ...
    ::telStub::DelType_DeleteType deltype
        = static_cast<::telStub::DelType_DeleteType>(info.delType);
...
telux::common::Status SmsManagerStub::requestPreferredStorage(RequestPreferredStorageCb callback) {
    ...
    int cbDelay           = static_cast<int>(response.delay());
    if ((status == telux::common::Status::SUCCESS) && (isCallbackNeeded)) {
        auto f = std::async(std::launch::async, [this, callback, storageType, cbDelay, error]() {
...
telux::common::Status SmsManagerStub::setPreferredStorage(
    StorageType storageType, telux::common::ResponseCallback callback) {
    LOG(DEBUG, __FUNCTION__, " PhoneId : ", phoneId_);
...
telux::common::Status SmsManagerStub::requestStorageDetails(
    RequestStorageDetailsCb callback, StorageType storageType) {
    ...
    int cbDelay                    = static_cast<int>(response.delay());
    if ((status == telux::common::Status::SUCCESS) && (isCallbackNeeded)) {
        auto f = std::async(std::launch::async, [this, maxCount, availableCount, cbDelay, error,
                                                    callback]() {
            this->invokeRequestStorageDetailsCb(maxCount, availableCount, cbDelay, error, callback);
        }).share();
        taskQ_->add(f);
    }
    return status;
...
void SmsManagerStub::invokeRequestStorageDetailsCb(int maxCount, int availableCount, int cbDelay,
    telux::common::ErrorCode error, RequestStorageDetailsCb callback) {
    LOG(DEBUG, __FUNCTION__);
    std::this_thread::sleep_for(std::chrono::milliseconds(cbDelay));
...
MessageAttributes SmsManagerStub::calculateMessageAttributes(const std::string &message) {
    ...
    msg.encoding = static_cast<telux::tel::SmsEncoding>((response.message_attribute()).encoding());
...
void SmsManagerStub::handleIncomingSms(::telStub::SmsMessage event) {
    ...
    if ((msg.getMessagePartInfo())->numberOfSegments > 1) {
        ...
        LOG(ERROR, __FUNCTION__, " Invalid input for current segment ",
            (msg.getMessagePartInfo())->segmentNumber, " and total number of segments ",
            (msg.getMessagePartInfo())->numberOfSegments);
        return;
    }
}
...
void SmsManagerStub::invokeIncomingSmslisteners(
    int phoneId, std::shared_ptr<std::vector<SmsMessage>> messages) {
    LOG(DEBUG, __FUNCTION__);
    std::vector<std::weak_ptr<ISmsListener>> applisteners;
    if (listenerMgr_) {
        listenerMgr_->getAvailableListeners(applisteners);
...
void SmsManagerStub::parseAndConcatenateSmsMessage(int phoneId, SmsMessage &message) {
    ...
    for (auto &smsInfo : smsMessageMap_) {
        if (smsInfo.first.refNumber == ((message.getMessagePartInfo())->refNumber)
            ...
            auto comp = [](SmsMessage &lhs, SmsMessage &rhs) {
