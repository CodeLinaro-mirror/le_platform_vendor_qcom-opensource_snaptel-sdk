...
std::shared_ptr<telux::common::ICommandCallback> CommandCallbackManager::findAndRemoveCallback(
    int cmdId) {
    std::lock_guard<std::mutex> m(mutex_);
    if (cmdId < 0) {
    ...
    if (entry != cmdCallbackMap_.end()) {
        ...
        LOG(DEBUG, __FUNCTION__, " Removing callback for cmdId : ", cmdId,
            " cmdCallbackMap_ size: ", cmdCallbackMap_.size());
        return sp.lock();
    }
    ...
    if (it != funCallbackMap_.end()) {
        ...
        LOG(DEBUG, __FUNCTION__, " Removing callback for cmdId : ", cmdId,
            " funCallbackMap_ size: ", funCallbackMap_.size());
        return sp;
    }
    return nullptr;
}
