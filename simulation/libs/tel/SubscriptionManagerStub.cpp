...
telux::common::Status SubscriptionManagerStub::init(telux::common::InitResponseCb callback) {
    LOG(DEBUG, __FUNCTION__);
...
void SubscriptionManagerStub::initSync() {
            ...
            if (cardMgrStatus == telux::common::ServiceStatus::SERVICE_AVAILABLE) {
                ...
                for (int id = FIRST_SIM_SLOT_ID; id < (FIRST_SIM_SLOT_ID + numSlots); id++) {
                    ...
                    if (status != telux::common::Status::SUCCESS) {
                        LOG(ERROR, __FUNCTION__, " unable to update subscription", " map on slot ",
                            id);
                ...
                LOG(ERROR, __FUNCTION__, " Card Manager subsystem is not ready,",
    ...
    LOG(DEBUG, __FUNCTION__, " cbDelay:: ", cbDelay_,
        " cbStatus:: ", static_cast<int>(servicestatus));
    bool isSubsystemReady
        = (servicestatus == telux::common::ServiceStatus::SERVICE_AVAILABLE) ? true : false;
...
telux::common::Status SubscriptionManagerStub::getAppInfo(
    std::vector<CardAppStatus> &apps, int phoneId) {
    LOG(DEBUG, __FUNCTION__);
...
telux::common::Status SubscriptionManagerStub::createSubscriptionAndNotify(int slotId) {
    ...
    if (status == telux::common::Status::SUCCESS) {
        switch (cardState) {
            ...
            default: {
                LOG(DEBUG, __FUNCTION__,
                    " card state is unknown or invalid: ", static_cast<int>(cardState));
            }
        }
    } else {
...
telux::common::Status SubscriptionManagerStub::fetchSubscription(int slotId,
    ...
    *gid2        = static_cast<std::string>(response.gid_2());
    ...
    LOG(DEBUG, __FUNCTION__, " Carrier name is ", *carrierName, " Phone number is ", *number,
        " iccid is ", *iccId, " mcc is ", *mcc, " mnc is ", *mnc, " imsi is ", *imsi, " gid1 is ",
        *gid1, " gid2 is ", *gid2);
...
std::shared_ptr<ISubscription> SubscriptionManagerStub::getSubscription(
    int slotId, telux::common::Status *status) {
    LOG(DEBUG, __FUNCTION__, " slotId: ", slotId);
...
std::vector<std::shared_ptr<ISubscription>> SubscriptionManagerStub::getAllSubscriptions(
    telux::common::Status *status) {
    LOG(DEBUG, __FUNCTION__);
