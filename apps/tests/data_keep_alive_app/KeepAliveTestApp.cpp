...
void KeepAliveTestApp::onServiceStatusChange(
...
void KeepAliveTestApp::onKeepAliveStatusChange(ErrorCode error,
  ...
  if (error == ErrorCode::SUCCESS) {
    ...
    std::cout << "TCP keep-alive offloading error : " << static_cast<int>(error)
              << std::endl;
  }
}
...
void KeepAliveTestApp::startServerThread() {
  serverThread_ = std::thread([this] {
    serverExited_ = false;
    server_->startServer();
    std::lock_guard<std::mutex> lock(serverExitMutex_);
    serverExited_ = true;
    serverExitCondition_.notify_one();
  });
}
...
void KeepAliveTestApp::startTCPServer(std::vector<std::string> inputCommand) {
  ...
  if (userChoice) {
    ...
    Utils::validateInput(serverInterface);
  }
...
void KeepAliveTestApp::stopTCPServer(std::vector<std::string> inputCommand) {
  ...
  std::unique_lock<std::mutex> lock(serverExitMutex_);
  serverExitCondition_.wait(lock, [&] { return (bool)serverExited_; });
...
void KeepAliveTestApp::sendMessage(std::vector<std::string> inputCommand) {
  if (inputCommand.size() > 1 && (!inputCommand[1].empty())) {
    kaproto msg;
    memset(&msg, 0, sizeof(msg));
    std::string messageStr = inputCommand[1] + "\n";
...
void KeepAliveTestApp::startTCPClient(std::vector<std::string> inputCommand) {
  ...
  Utils::validateInput(serverIpAddr);
  std::cout << "Enter server port number: ";
  std::cin >> serverPort;
  Utils::validateInput(serverPort);
  ...
  if (userChoice) {
    ...
    Utils::validateInput(clientIpAddr);
    std::cout << "Enter client port number: ";
    std::cin >> clientPort;
    Utils::validateInput(clientPort);
  }
  ...
  if (userChoice) {
    ...
    Utils::validateInput(clientInterface);
  }

  clientWorker_ = std::make_shared<TCPClientWorker<kaproto>>();
  client_ = std::make_shared<TCPClient<kaproto>>(clientWorker_, serverPort,
                                                 serverIpAddr, clientPort,
                                                 clientIpAddr, clientInterface);
  startClientThread();
}
...
void KeepAliveTestApp::stopTCPClient(std::vector<std::string> inputCommand) {
  ...
  clientExitCondition_.wait(lock, [&] { return (bool)clientExited_; });
...
void KeepAliveTestApp::enableTCPMonitor(std::vector<std::string> inputCommand) {
  ...
  Utils::validateInput(params.srcIp);
  std::cout << "Enter destination IPv4/IPv6 address: ";
  std::cin >> params.dstIp;
  Utils::validateInput(params.dstIp);
  std::cout << "Enter source port: ";
  std::cin >> params.srcPort;
  Utils::validateInput(params.srcPort);
  std::cout << "Enter destination port: ";
  std::cin >> params.dstPort;
  Utils::validateInput(params.dstPort);

  MonitorHandleType monHandle;
  auto err = keepAliveMgr_->enableTCPMonitor(params, monHandle);
  if (err != ErrorCode::SUCCESS) {
    std::cout << "Operation failed with errorcode: " << static_cast<int>(err)
              << std::endl;
  } else {
    std::cout << "Operation completed. Monitor handle : " << monHandle
              << std::endl;
  }
}

void KeepAliveTestApp::disableTCPMonitor(
...
void KeepAliveTestApp::startTCPKeepAliveOffload(
    std::vector<std::string> inputCommand) {
  if (!keepAliveMgr_) {
    std::cout << "KeepAlive Manager not ready !\n";
    return;
  ...
  int mode = 0;
  std::cout << "Enter type of startTCPKeepAliveOffload API (0: default mode, "
               "1: monitor mode) : ";
  std::cin >> mode;
  Utils::validateInput(mode, {0, 1});
  TCPKAOffloadHandle handle;
  ...
  case 0:
    ...
    Utils::validateInput(session.recvNext);
    std::cout << "Enter recvWindow: ";
    std::cin >> session.recvWindow;
    Utils::validateInput(session.recvWindow);
    std::cout << "Enter sendNext: ";
    std::cin >> session.sendNext;
    Utils::validateInput(session.sendNext);
    std::cout << "Enter sendWindow: ";
    std::cin >> session.sendWindow;
    Utils::validateInput(session.sendWindow);
    std::cout << "Enter interval: ";
    std::cin >> interval;
    Utils::validateInput(interval);

    err = keepAliveMgr_->startTCPKeepAliveOffload(params, session, interval,
                                                  handle);
    break;
  ...
  if (err != ErrorCode::SUCCESS) {
    ...
    std::cout << "Operation completed. TCPKAOffload handle : " << handle
              << std::endl;
  }
}

void KeepAliveTestApp::stopTCPKeepAliveOffload(
    std::vector<std::string> inputCommand) {
  if (!keepAliveMgr_) {
    std::cout << "KeepAlive Manager not ready !\n";
    return;
...
void KeepAliveTestApp::consoleInit(bool isServer) {
  ...
  std::shared_ptr<ConsoleAppCommand> startTCPServerCommand =
      std::make_shared<ConsoleAppCommand>(
          ConsoleAppCommand("1", "startTCPServer", {},
                            std::bind(&KeepAliveTestApp::startTCPServer, this,
                                      std::placeholders::_1)));

  std::shared_ptr<ConsoleAppCommand> stopTCPServerCommand =
      std::make_shared<ConsoleAppCommand>(
          ConsoleAppCommand("2", "stopTCPServer", {},
                            std::bind(&KeepAliveTestApp::stopTCPServer, this,
                                      std::placeholders::_1)));

  std::shared_ptr<ConsoleAppCommand> startTCPClientCommand =
      std::make_shared<ConsoleAppCommand>(
          ConsoleAppCommand("1", "startTCPClient", {},
                            std::bind(&KeepAliveTestApp::startTCPClient, this,
                                      std::placeholders::_1)));

  std::shared_ptr<ConsoleAppCommand> stopTCPClientCommand =
      std::make_shared<ConsoleAppCommand>(
          ConsoleAppCommand("2", "stopTCPClient", {},
                            std::bind(&KeepAliveTestApp::stopTCPClient, this,
                                      std::placeholders::_1)));

  std::shared_ptr<ConsoleAppCommand> sendMessageCommand =
      std::make_shared<ConsoleAppCommand>(
          ConsoleAppCommand("3", "sendMessage", {"message"},
                            std::bind(&KeepAliveTestApp::sendMessage, this,
                                      std::placeholders::_1)));

  std::shared_ptr<ConsoleAppCommand> enableTCPMonitorCommand =
      std::make_shared<ConsoleAppCommand>(
          ConsoleAppCommand("4", "enableTCPMonitor", {},
                            std::bind(&KeepAliveTestApp::enableTCPMonitor, this,
                                      std::placeholders::_1)));

  std::shared_ptr<ConsoleAppCommand> disableTCPMonitorCommand =
      std::make_shared<ConsoleAppCommand>(
          ConsoleAppCommand("5", "disableTCPMonitor", {},
                            std::bind(&KeepAliveTestApp::disableTCPMonitor,
                                      this, std::placeholders::_1)));

  std::shared_ptr<ConsoleAppCommand> startTCPKeepAliveOffloadCommand =
      std::make_shared<ConsoleAppCommand>(ConsoleAppCommand(
          "6", "startTCPKeepAliveOffload", {},
          std::bind(&KeepAliveTestApp::startTCPKeepAliveOffload, this,
                    std::placeholders::_1)));

  std::shared_ptr<ConsoleAppCommand> stopTCPKeepAliveOffloadCommand =
      std::make_shared<ConsoleAppCommand>(ConsoleAppCommand(
          "7", "stopTCPKeepAliveOffload", {},
          std::bind(&KeepAliveTestApp::stopTCPKeepAliveOffload, this,
                    std::placeholders::_1)));

  if (isServer) {
    std::vector<std::shared_ptr<ConsoleAppCommand>> commandsListTCPKAMenu = {
        startTCPServerCommand,
        stopTCPServerCommand,
        sendMessageCommand,
        enableTCPMonitorCommand,
        disableTCPMonitorCommand,
        startTCPKeepAliveOffloadCommand,
        stopTCPKeepAliveOffloadCommand};
    ConsoleApp::addCommands(commandsListTCPKAMenu);
  } else {
    std::vector<std::shared_ptr<ConsoleAppCommand>> commandsListTCPKAMenu = {
        startTCPClientCommand,
        stopTCPClientCommand,
        sendMessageCommand,
        enableTCPMonitorCommand,
        disableTCPMonitorCommand,
        startTCPKeepAliveOffloadCommand,
...
void KeepAliveTestApp::registerForUpdates() {
  ...
  Status status = keepAliveMgr_->registerListener(self_);
  if (status != Status::SUCCESS) {
    std::cout << APP_NAME
              << " ERROR - Failed to register for keep-alive notification"
              << std::endl;
  } else {
    std::cout << APP_NAME << " Registered Listener for keep-alive notification"
              << std::endl;
  }
}

void KeepAliveTestApp::deregisterForUpdates() {
  Status status = keepAliveMgr_->deregisterListener(self_);
  if (status != Status::SUCCESS) {
    std::cout << APP_NAME
              << " ERROR - Failed to de-register for keep-alive notification"
              << std::endl;
  } else {
...
std::shared_ptr<KeepAliveTestApp> init() {
  std::shared_ptr<KeepAliveTestApp> keepAliveApp =
      std::make_shared<KeepAliveTestApp>();
  if (!keepAliveApp) {
  ...
  if (keepAliveApp->keepAliveMgr_->getServiceStatus() ==
    ...
    std::cout << " *** ERROR - Unable to initialize keep-alive subsystem *** "
              << std::endl;
    return nullptr;
  }
...
KeepAliveTestApp::KeepAliveTestApp()
    : ConsoleApp("TCP KeepAlive Test app Menu", "tcpka-test> "),
      keepAliveMgr_(nullptr), server_(nullptr) {}
