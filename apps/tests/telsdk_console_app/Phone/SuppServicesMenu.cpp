...
bool SuppServicesMenu::init() {
    ...
    for (auto index = 1; index <= slotCount; index++) {
        ...
        std::cout << " Waiting for supplementary service manager to be ready on slot id " << index
                  << std::endl;
    ...
    std::shared_ptr<ConsoleAppCommand> setCallWaitingPrefCmd
        = std::make_shared<ConsoleAppCommand>(ConsoleAppCommand("1", "Set_call_waiting_pref", {},
    ...
    std::shared_ptr<ConsoleAppCommand> getCallWaitingPrefCmd
        = std::make_shared<ConsoleAppCommand>(ConsoleAppCommand("2", "Get_call_waiting_pref", {},
    ...
    std::shared_ptr<ConsoleAppCommand> setCallForwardingPrefCmd
        = std::make_shared<ConsoleAppCommand>(ConsoleAppCommand("3", "Set_call_forwarding_pref", {},
    ...
    std::vector<std::shared_ptr<ConsoleAppCommand>> commandsListSuppServicesMenu
        = {setCallWaitingPrefCmd, getCallWaitingPrefCmd, setCallForwardingPrefCmd,
            getCallForwardingPrefCmd, setOirPrefCmd, getOirPrefCmd};
    ...
    if (pref == 1 || pref == 2) {
        if (suppServicesManager) {
            auto ret = suppServicesManager->setCallWaitingPref(
                static_cast<SuppServicesStatus>(pref), SetSuppSvcResponseCallback::setSuppSvcResp);
...
void SuppServicesMenu::setCallForwardingPref(std::vector<std::string> userInput) {
    ...
    std::cin >> command;
    Utils::validateInput(command);
    if (command == INPUT_ACTIVATE || command == INPUT_DEACTIVATE || command == INPUT_REGISTER
        || command == INPUT_ERASE) {
                    ...
                    if (Utils::validateDigitString(noReplyTimer)) {
                        if (noReplyTimer.size() <= MAX_INPUT_NO_REPLY) {
                            ...
                            if ((tmp >= MIN_NO_REPLY_TIMER) && (tmp <= MAX_NO_REPLY_TIMER)) {
                                ...
                                std::cout << "No reply timer value not in range (0-255)"
                                          << std::endl;
                                invalidTimer = true;
                            }
                        } else {
        ...
        if (suppServicesManager) {
            ...
            auto ret         = suppServicesManager->setForwardingPref(
                req, SetSuppSvcResponseCallback::setSuppSvcResp);
            if (ret == telux::common::Status::SUCCESS) {
...
void SuppServicesMenu::getCallForwardingPref(std::vector<std::string> userInput) {
    ...
    if (suppServicesManager) {
        ...
        auto ret = suppServicesManager->requestForwardingPref(
            serviceClass, reason, GetSuppSvcResponseCallback::getForwardingPrefResp);
        if (ret == telux::common::Status::SUCCESS) {
...
void SuppServicesMenu::getOirPref(std::vector<std::string> userInput) {
    ...
    if (suppServicesManager) {
        serviceClass.set(static_cast<uint8_t>(ServiceClassType::VOICE));
        auto ret = suppServicesManager->requestOirPref(
            serviceClass, GetSuppSvcResponseCallback::getOirStatusResp);
        if (ret == telux::common::Status::SUCCESS) {
