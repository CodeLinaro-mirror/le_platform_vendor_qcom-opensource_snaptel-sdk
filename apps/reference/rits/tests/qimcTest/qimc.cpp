...
json_object *Qimc::sendReq(json_object *req) {
    ...
    ret = write(clientSock, json_object_to_json_string_length(req, 0, &jsonLength), jsonLength);
    if (ret == CLIENT_ERROR) {
...
int Qimc::connectServer() {
    auto ret       = 0;
    clientSock     = socket(config.sockDomain, config.sockType, config.sockProtocol);
