...
bool DataSettingsMenu::init() {
    ...
    if (menuOptionsAdded_ == false) {
        ...
        std::vector<std::pair<std::string, std::function<void(std::vector<std::string> &)>>>
            ...
            = {std::make_pair("Set_Backhaul_Preference",
                ...
                std::make_pair("Set_Band_Interference_Configuration",
                    std::bind(
                        &DataSettingsMenu::setBandInterferenceConfig, this, std::placeholders::_1)),
                std::make_pair("Request_Band_Interference_Configuration",
                    std::bind(&DataSettingsMenu::requestBandInterferenceConfig, this,
                        std::placeholders::_1)),
                std::make_pair("Configure_Backhaul_Connectivity",
                    std::bind(
                        &DataSettingsMenu::setWwanConnectivityConfig, this, std::placeholders::_1)),
                std::make_pair("Request_Backhaul_Connectivity",
                    std::bind(&DataSettingsMenu::requestWwanConnectivityConfig, this,
                        std::placeholders::_1)),
                ...
                std::make_pair(
                    "Restore_Factory_Settings", std::bind(&DataSettingsMenu::restoreFactorySettings,
                                                    this, std::placeholders::_1)),
                std::make_pair("Is_Device_Data_Usage_Monitoring_Enabled",
                    std::bind(&DataSettingsMenu::isDeviceDataUsageMonitoringEnabled, this,
                        std::placeholders::_1)),
                std::make_pair("Get_IP_Passthrough_Configuration",
                    std::bind(
                        &DataSettingsMenu::getIpPassthroughConfig, this, std::placeholders::_1)),
                std::make_pair("Set_IP_Passthrough_Configuration",
                    std::bind(
                        &DataSettingsMenu::setIpPassthroughConfig, this, std::placeholders::_1)),
        ...
        for (auto &menuItem : settingsCommandPairList) {
            settingsMenuCommandList.push_back(std::make_shared<ConsoleAppCommand>(
                ConsoleAppCommand(std::to_string(commandId), menuItem.first, {}, menuItem.second)));
            commandId++;
        }
...
void DataSettingsMenu::setBackhaulPref(std::vector<std::string> inputCommand) {
    ...
    for (int i = 0; i < static_cast<int>(BackhaulType::MAX_SUPPORTED); ++i) {
        do {
            inputIsValid = true;
            std::cout << "Enter Backhaul " << i + 1 << " (0-ETH, 1-USB, 2-WLAN, 3-WWAN, 4-BLE): ";
            std::cin >> backhaul;
            std::cout << endl;
            Utils::validateInput(backhaul,
                {static_cast<int>(BackhaulType::ETH), static_cast<int>(BackhaulType::USB),
                    static_cast<int>(BackhaulType::WLAN), static_cast<int>(BackhaulType::WWAN),
    ...
    auto respCb = [](bool isEnabled, std::shared_ptr<telux::data::BandInterferenceConfig> config,
        ...
        if (error == telux::common::ErrorCode::SUCCESS) {
            if (isEnabled) {
                ...
                std::cout << "  High Prioroty: "
                          << ((config->priority == telux::data::BandPriority::WLAN) ? "Wlan 5GHz"
                                                                                    : "N79 5G")
                          << std::endl;
...
void DataSettingsMenu::requestCurrentDds(std::vector<std::string> inputCommand) {
    ...
    auto respCb = [](DdsInfo currentState, telux::common::ErrorCode error) {
        ...
        if (error == telux::common::ErrorCode::SUCCESS) {
            ...
            std::string type
                = (currentState.type == DdsType::PERMANENT) ? "Permanent" : "Temporary";
...
void DataSettingsMenu::getIpPassthroughConfig(std::vector<std::string> inputCommand) {
    ...
    PRINT_RESPONSE_DATA
        << "ip passthrough operation:\t\t"
        << (config.ipptOpr == telux::data::Operation::ENABLE
                   ? "ENABLE"
                   : (config.ipptOpr == telux::data::Operation::DISABLE ? "DISABLE" : "UNKNOWN"))
        << std::endl;
    PRINT_RESPONSE_DATA << "nework interface:\t\t"
                        << DataUtils::vlanInterfaceToString(
                               config.devConfig.nwInterface, telux::data::OperationType::DATA_LOCAL)
                        << std::endl;
...
void DataSettingsMenu::setIpPassthroughConfig(std::vector<std::string> inputCommand) {
    ...
    std::cin >> ipptOpr;
    config.ipptOpr
        = (ipptOpr == 1 ? telux::data::Operation::ENABLE : telux::data::Operation::DISABLE);
...
void DataSettingsMenu::getIpConfig(std::vector<std::string> inputCommand) {
    ...
    std::cin >> ipFamilyType;
    DataUtils::validateInput(ipFamilyType, {1, 2});
    ipConfigParams.ipFamilyType
        = (ipFamilyType == 1 ? telux::data::IpFamilyType::IPV4
    ...
    PRINT_RESPONSE_DATA
        << "interface type:\t\t"
        << (ipConfigParams.ifType == telux::data::InterfaceType::ETH ? "ETH" : "UNKNOWN")
        << std::endl;
    ...
    if (ipConfig.ipType == telux::data::IpAssignType::STATIC_IP) {
        ...
        PRINT_RESPONSE_DATA << "primary dns:\t\t" << ipConfig.ipAddr.primaryDnsAddress << std::endl;
...
void DataSettingsMenu::onDdsChange(DdsInfo currentState) {
    ...
    std::cout << "DDS Info : "
              << "Slot_Id: " << currentState.slotId << std::endl;
    std::string type = (currentState.type == DdsType::PERMANENT) ? "Permanent" : "Temporary";
    std::cout << "Switch Type: " << type << std::endl;
...
void DataSettingsMenu::isDeviceDataUsageMonitoringEnabled(std::vector<std::string> inputCommand) {
    ...
    std::cout << "RESPONSE: isDeviceDataUsageMonitoringEnabled "
              << ", Device data usage monitoring is " << (enable ? "enabled" : "disabled")
              << std::endl;
}
