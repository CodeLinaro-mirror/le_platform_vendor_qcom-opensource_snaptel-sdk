...
bool AecsCall::init() {
    ...
    if (menuOptionsAdded_ == false) {
        ...
        std::shared_ptr<ConsoleAppCommand> hangupCommand =
            std::make_shared<ConsoleAppCommand>(
                ConsoleAppCommand("5", "Hangup", {},
                    std::bind(&AecsCall::hangup, this, std::placeholders::_1)));
        ...
        std::shared_ptr<ConsoleAppCommand> retryDroppedAecsCallCommand =
            std::make_shared<ConsoleAppCommand>(
                ConsoleAppCommand("7", "Retry_Dropped_AECS_call", {},
        ...
        std::shared_ptr<ConsoleAppCommand> retryFailedAecsCallCommand =
            std::make_shared<ConsoleAppCommand>(
                ConsoleAppCommand("8", "Retry_Failed_AECS_call", {},
...
void AecsCall::makeCallResponse(telux::common::ErrorCode error,
...
void AecsCall::dialAecsCall(std::vector<std::string> userInput) {
    ...
    telux::common::Status makeCallStatus =
        callMgr_->makeAecsCall(phoneId, phoneNumber,
        std::bind(&AecsCall::makeCallResponse, this, std::placeholders::_1,
                 std::placeholders::_2));
    if (makeCallStatus == telux::common::Status::SUCCESS) {
        std::cout << "makeAecsCall is successful.\n";
        std::cout << std::endl << std::endl;
        PRINT_NOTIFICATION
            << " AECS information: -voice connection establishment in progress" << std::endl;
    } else {
...
void AecsCall::acceptCall(std::vector<std::string> userInput) {
    ...
    if (incomingAecsCall == "y" && aecsOngoing) {
        ...
        if (status != telux::common::Status::SUCCESS) {
            ...
            std::cout << " Incoming call rejected as AECS call is ongoing"
                      << std::endl;
        }
        return;
    }
            ...
            if ((state != telux::tel::CallState::CALL_ENDED &&
                ...
                if (!aecsMgr.isEmergencyMode(call->getPhoneId())) {
                    ...
                    if (status != telux::common::Status::SUCCESS) {
                        ...
                        std::cout << " Ongoing non-AECS call hanged up successfully"
                                  << std::endl;
                    }
                }
            }
    ...
    if (aecsOngoing) {
        ...
        if (status != telux::common::Status::SUCCESS) {
            ...
            std::cout << " Incoming non-AECS call rejected as AECS call is ongoing"
                      << std::endl;
        }
    } else {
...
void AecsCall::getAllCalls(std::vector<std::string> userInput) {
    ...
    for (auto &call : inProgressCalls) {
        std::cout << " Call State: "
                  ...
                  << " SlotId: " << call->getPhoneId()
                  << " isMpty: " << call->isMultiPartyCall()
                  << std::endl;
    }
}
