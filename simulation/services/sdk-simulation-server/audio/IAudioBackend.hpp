...
class IAudioBackend {
 ...
 public:
    ...
    virtual telux::common::ErrorCode getSupportedDevices(
        std::vector<DeviceType> &devices, std::vector<DeviceDirection> &devicesDirection)
        = 0;

    virtual telux::common::ErrorCode getSupportedStreamTypes(std::vector<StreamType> &streamTypes)
        = 0;

    virtual telux::common::ErrorCode createStream(StreamHandle &streamHandle,
        StreamParams streamParams, uint32_t &readBufferMinSize, uint32_t &writeBufferMinSize)
        = 0;
    ...
    virtual telux::common::ErrorCode setDevice(
        StreamHandle streamHandle, std::vector<DeviceType> &deviceTypes)
        = 0;

    virtual telux::common::ErrorCode getDevice(
        StreamHandle streamHandle, std::vector<DeviceType> &deviceTypes)
        = 0;

    virtual telux::common::ErrorCode setVolume(StreamHandle streamHandle, StreamDirection direction,
        std::vector<ChannelVolume> channelsVolume)
        = 0;

    virtual telux::common::ErrorCode getVolume(
        StreamHandle streamHandle, int channelTypeMask, std::vector<ChannelVolume> &channelsVolume)
        = 0;

    virtual telux::common::ErrorCode setMuteState(StreamHandle streamHandle, StreamMute muteInfo,
        std::vector<ChannelVolume> channelsVolume, bool prevMuteState)
        = 0;

    virtual telux::common::ErrorCode getMuteState(
        StreamHandle streamHandle, StreamMute &muteInfo, StreamDirection direction)
        = 0;

    virtual telux::common::ErrorCode write(StreamHandle &streamHandle, uint8_t *data,
    ...
    virtual telux::common::ErrorCode startDtmf(
        StreamHandle streamHandle, uint16_t gain, uint32_t duration, DtmfTone dtmfTone)
        = 0;

    virtual telux::common::ErrorCode stopDtmf(StreamHandle streamHandle, StreamDirection direction)
        = 0;
    ...
    virtual telux::common::ErrorCode setupInTranscodeStream(StreamHandle &streamHandle,
        ...
        std::shared_ptr<IStreamEventListener> streamEventListener, uint32_t &writeMinSize)
        = 0;
