...
class FirewallManagerStub : public IFirewallManager, public IFirewallListener {
 public:
    ...
    telux::common::Status setFirewallConfig(
        FirewallConfig fwConfig, telux::common::ResponseCallback callback = nullptr) override;

    telux::common::Status requestFirewallConfig(
        BackhaulInfo bhInfo, FirewallConfigCb callback) override;

    telux::common::Status addHwAccelerationFirewallEntry(
        FirewallEntryInfo entry, AddFirewallEntryCb callback = nullptr) override;

    telux::common::Status requestHwAccelerationFirewallEntries(
        BackhaulInfo bhInfo, FirewallEntryInfoCb callback) override;

    telux::common::Status addFirewallEntry(
        FirewallEntryInfo entry, AddFirewallEntryCb callback = nullptr) override;
    ...
    telux::common::Status enableDmz(
        DmzConfig config, telux::common::ResponseCallback callback = nullptr) override;
    ...
    telux::common::Status requestFirewallStatus(
        int profileId, FirewallStatusCb callback, SlotId slotId = DEFAULT_SLOT_ID) override;
    ...
    telux::common::Status requestDmzEntry(
        int profileId, DmzEntriesCb dmzCb, SlotId slotId = DEFAULT_SLOT_ID) override;
 ...
 private:
    ...
    telux::common::Status getFirewallEntriesRequest(
        BackhaulInfo bhInfo, FirewallEntryInfoCb callback, bool isHwAccelerated = false);
};
