...
grpc::Status ImsSettingsManagerServerImpl::InitService(ServerContext *context,
    ...
    std::string cbStatus = rootObj[IMS_SETTINGS_MANAGER]["IsSubsystemReady"].asString();
    ...
    if (data.status == telux::common::Status::SUCCESS) {
        ...
        bool voImsEnabled
            = data.stateRootObj[IMS_SETTINGS_MANAGER]["ImsServiceConfigInfo"]["voImsEnabled"]
                  .asBool();
        bool smsEnabled
            = data.stateRootObj[IMS_SETTINGS_MANAGER]["ImsServiceConfigInfo"]["smsEnabled"].asBool();
        bool rttEnabled
            = data.stateRootObj[IMS_SETTINGS_MANAGER]["ImsServiceConfigInfo"]["rttEnabled"].asBool();
...
grpc::Status ImsSettingsManagerServerImpl::SetServiceConfig(ServerContext *context,
    const ::telStub::SetServiceConfigRequest *request, telStub::SetServiceConfigReply *response) {
    LOG(DEBUG, __FUNCTION__);
    ...
    if (data.status == telux::common::Status::SUCCESS) {
        if (request->config().is_ims_service_enabled_valid()) {
            prevImsServiceEnabled
                = data.stateRootObj[IMS_SETTINGS_MANAGER]["ImsServiceConfigInfo"]["imsServiceEnable"
                                                                                  "d"]
                      .asBool();
            data.stateRootObj[IMS_SETTINGS_MANAGER]["ImsServiceConfigInfo"]["imsServiceEnabled"]
                = imsServiceEnabled;
...
grpc::Status ImsSettingsManagerServerImpl::SetSipUserAgent(ServerContext *context,
    const ::telStub::SetSipUserAgentRequest *request, telStub::SetSipUserAgentReply *response) {
    LOG(DEBUG, __FUNCTION__);
    int slotId                = request->phone_id();
...
void ImsSettingsManagerServerImpl::triggerImsServiceConfigsChange(int slotId,
    bool prevImsServiceEnabled, bool imsServiceEnabled, bool prevVoImsEnabled, bool voImsEnabled,
    bool prevSmsEnabled, bool smsEnabled, bool prevRttEnabled, bool rttEnabled) {
    LOG(INFO, __FUNCTION__, " imsServiceEnabled is ", imsServiceEnabled, " voImsEnabled is ",
        voImsEnabled, " smsEnabled is ", smsEnabled, " rttEnabled is ", rttEnabled);
    // check the previous value then update and send notification.
    std::string stateJsonPath = (slotId == SLOT_1) ? "tel/IImsSettingsManagerStateSlot1"
                                                   : "tel/IImsSettingsManagerStateSlot2";
...
grpc::Status ImsSettingsManagerServerImpl::RequestVonr(ServerContext *context,
    const ::telStub::RequestVonrRequest *request, telStub::RequestVonrReply *response) {
    LOG(DEBUG, __FUNCTION__);
    std::string apiJsonPath   = (request->phone_id() == SLOT_1) ? JSON_PATH1 : JSON_PATH2;
    std::string stateJsonPath = (request->phone_id() == SLOT_1) ? JSON_PATH3 : JSON_PATH4;
    std::string subsystem     = IMS_SETTINGS_MANAGER;
    ...
    if (data.status == telux::common::Status::SUCCESS) {
        bool enable = data.stateRootObj[IMS_SETTINGS_MANAGER]["ImsVonrEnable"].asBool();
...
grpc::Status ImsSettingsManagerServerImpl::SetVonr(ServerContext *context,
    const ::telStub::SetVonrRequest *request, telStub::SetVonrReply *response) {
    LOG(DEBUG, __FUNCTION__);
    int slotId                = request->phone_id();
    std::string apiJsonPath   = (slotId == SLOT_1) ? JSON_PATH1 : JSON_PATH2;
    std::string stateJsonPath = (request->phone_id() == SLOT_1) ? JSON_PATH3 : JSON_PATH4;
    ...
    if (data.status == telux::common::Status::SUCCESS) {
        data.stateRootObj[IMS_SETTINGS_MANAGER]["ImsVonrEnable"] = request->enable();
...
void ImsSettingsManagerServerImpl::handleImsServiceConfigsChange(std::string eventParams) {
    ...
    try {
        ...
        std::string stateJsonPath = (slotId == SLOT_1) ? "tel/IImsSettingsManagerStateSlot1"
                                                       : "tel/IImsSettingsManagerStateSlot2";
        bool prevImsServiceEnabled
            = CommonUtils::readSystemDataValue(stateJsonPath, "true",
                  {IMS_SETTINGS_MANAGER, "ImsServiceConfigInfo", "imsServiceEnabled"})
              != "false";
...
void ImsSettingsManagerServerImpl::triggerImsSipUserAgentChange(
    int slotId, std::string prevSipUserAgent, std::string sipUserAgent) {
    LOG(DEBUG, " prevSipUserAgent : ", prevSipUserAgent, " sipUserAgent : ", sipUserAgent);
    std::string stateJsonPath = (slotId == SLOT_1) ? "tel/IImsSettingsManagerStateSlot1"
                                                   : "tel/IImsSettingsManagerStateSlot2";
    if (prevSipUserAgent.compare(sipUserAgent) != 0) {
        // read the latest value
        sipUserAgent = CommonUtils::readSystemDataValue(
            stateJsonPath, "", {IMS_SETTINGS_MANAGER, "ImsSipUserAgentInfo", "sipUserAgent"});
...
void ImsSettingsManagerServerImpl::handleImsSipUserAgentChange(std::string eventParams) {
    ...
    try {
        ...
        std::string stateJsonPath    = (slotId == SLOT_1) ? "tel/IImsSettingsManagerStateSlot1"
                                                          : "tel/IImsSettingsManagerStateSlot2";
        std::string prevSipUserAgent = CommonUtils::readSystemDataValue(
            stateJsonPath, "", {IMS_SETTINGS_MANAGER, "ImsSipUserAgentInfo", "sipUserAgent"});
