...
class VoiceStreamImpl : public AudioStreamImpl,
 ...
 public:
    VoiceStreamImpl(uint32_t streamId, std::shared_ptr<ICommunicator> transportClient);

    ~VoiceStreamImpl();

    telux::common::Status init(void);

    telux::common::Status startAudio(telux::common::ResponseCallback callback = nullptr) override;

    telux::common::Status stopAudio(telux::common::ResponseCallback callback = nullptr) override;
    ...
    telux::common::Status stopDtmfTone(
        StreamDirection direction, telux::common::ResponseCallback callback = nullptr) override;
    ...
    void onPlayDtmfResult(telux::common::ErrorCode ec, uint32_t streamId, int cmdId) override;

    void onStopDtmfResult(telux::common::ErrorCode ec, uint32_t streamId, int cmdId) override;
