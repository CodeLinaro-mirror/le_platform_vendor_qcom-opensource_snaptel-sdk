...
void AecsCallManager::startAudio(int phoneId) {
    ...
    Status status = audioClient_->startVoiceSession(
        phoneId, audioDevices_, voiceSampleRate_,
...
Status AecsCallManager::enterEmergencyMode(int phoneId, bool enableAntennaSwitch) {
    ...
    if (it != emergencyMode_.end() && it->second) {
        std::cout << "AecsCallManager: Emergency mode already enabled on phoneId "
                  << phoneId << std::endl;
        return Status::SUCCESS;
    }

    Status status = callMgr_->setEmergencyMode(
        phoneId, /*emergencyModeEnabled*/ true, enableAntennaSwitch,
        std::bind(&AecsCallManager::setEmergencyModeResponse, this, std::placeholders::_1));
    if (status == Status::SUCCESS) {
        emergencyMode_[phoneId] = true;
        std::cout << "AecsCallManager: Emergency mode enabled on phoneId "
                  << phoneId << std::endl;
    } else {
        std::cout << "AecsCallManager: Failed to enable emergency mode on phoneId "
...
Status AecsCallManager::exitEmergencyMode(int phoneId) {
    ...
    auto it = emergencyMode_.find(phoneId);
    if (it == emergencyMode_.end() || !it->second) {
        std::cout << "AecsCallManager: Emergency mode already disabled on phoneId "
                  << phoneId << std::endl;
        return Status::SUCCESS;
    }
    ...
    if (status == Status::SUCCESS) {
        ...
        std::cout << "AecsCallManager: Emergency mode disabled on phoneId "
                  << phoneId << std::endl;
    } else {
        std::cout << "AecsCallManager: Failed to disable emergency mode on phoneId "
                  << phoneId << ", status=" << (int)status << std::endl;
    }
    return status;
}
