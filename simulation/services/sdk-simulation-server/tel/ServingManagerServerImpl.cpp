...
grpc::Status ServingManagerServerImpl::InitService(ServerContext *context,
    ...
    if (status == telux::common::ServiceStatus::SERVICE_AVAILABLE) {
        std::vector<std::string> filters = {telux::tel::TEL_SERVING_SYSTEM_FILTER, MODEM_FILTER};
...
grpc::Status ServingManagerServerImpl::SetRATPreference(ServerContext *context,
    const ::telStub::SetRATPreferenceRequest *request, telStub::SetRATPreferenceReply *response) {
    ...
    auto f      = std::async(std::launch::async, [this, phoneId, ratPrefs]() {
        ...
        int domain                = stoi(CommonUtils::readSystemDataValue(
            stateJsonPath, "0", {"IServingSystemManager", "ServiceDomainPreference"}));
        std::string gsmBandsStr   = CommonUtils::readSystemDataValue(
            stateJsonPath, "0", {"IServingSystemManager", "BandPreference", "gsmBands"});
        std::vector<int> gsmBands = CommonUtils::convertStringToVector(gsmBandsStr);
        std::string wcdmaBandsStr = CommonUtils::readSystemDataValue(
            stateJsonPath, "0", {"IServingSystemManager", "BandPreference", "wcdmaBands"});
        std::vector<int> wcdmaBands = CommonUtils::convertStringToVector(wcdmaBandsStr);
        std::string lteBandsStr     = CommonUtils::readSystemDataValue(
            stateJsonPath, "0", {"IServingSystemManager", "BandPreference", "lteBands"});
        std::vector<int> lteBands = CommonUtils::convertStringToVector(lteBandsStr);
        std::string nsaBandsStr   = CommonUtils::readSystemDataValue(
            stateJsonPath, "0", {"IServingSystemManager", "BandPreference", "nsaBands"});
        std::vector<int> nsaBands = CommonUtils::convertStringToVector(nsaBandsStr);
        std::string saBandsStr    = CommonUtils::readSystemDataValue(
            stateJsonPath, "0", {"IServingSystemManager", "BandPreference", "saBands"});
        std::vector<int> saBands = CommonUtils::convertStringToVector(saBandsStr);
        this->triggerSystemSelectionPreferenceEvent(
            phoneId, ratPrefs, domain, gsmBands, wcdmaBands, lteBands, nsaBands, saBands);
    }).share();
    taskQ_->add(f);
...
grpc::Status ServingManagerServerImpl::SetServiceDomainPreference(ServerContext *context,
    ...
    auto f      = std::async(std::launch::async, [this, phoneId, domain]() {
        std::string stateJsonPath = (phoneId == SLOT_1) ? "tel/IServingSystemManagerStateSlot1"
                                                             : "tel/IServingSystemManagerStateSlot2";
        std::string rats          = CommonUtils::readSystemDataValue(
            stateJsonPath, "0", {"IServingSystemManager", "RATPreference"});
        ...
        std::vector<int> saBands = CommonUtils::convertStringToVector(saBandsStr);
        this->triggerSystemSelectionPreferenceEvent(
            phoneId, raTs, domain, gsmBands, wcdmaBands, lteBands, nsaBands, saBands);
    }).share();
    taskQ_->add(f);
    // Create response
    if (data.cbDelay != -1) {
...
void ServingManagerServerImpl::triggerSystemSelectionPreferenceEvent(int slotId,
    ...
    std::vector<int> wcdmaBandPrefs, std::vector<int> lteBandPrefs, std::vector<int> nsaBandPrefs,
...
grpc::Status ServingManagerServerImpl::GetDcStatus(ServerContext *context,
    const ::telStub::GetDcStatusRequest *request, telStub::GetDcStatusReply *response) {
    LOG(DEBUG, __FUNCTION__);
    std::string stateJsonPath = (request->phone_id() == SLOT_1)
                                    ? "tel/IServingSystemManagerStateSlot1"
                                    : "tel/IServingSystemManagerStateSlot2";

    int endcAvailability = stoi(CommonUtils::readSystemDataValue(
        stateJsonPath, "0", {"IServingSystemManager", "DcStatus", "endcAvailability"}));
    int dcnrRestriction  = stoi(CommonUtils::readSystemDataValue(
        stateJsonPath, "0", {"IServingSystemManager", "DcStatus", "dcnrRestriction"}));
    response->set_endc_availability(
        static_cast<telStub::EndcAvailability_Status>(endcAvailability));
    response->set_dcnr_restriction(static_cast<telStub::DcnrRestriction_Status>(dcnrRestriction));
    return grpc::Status::OK;
}

grpc::Status ServingManagerServerImpl::GetSystemInfo(ServerContext *context,
    const ::telStub::GetSystemInfoRequest *request, telStub::GetSystemInfoReply *response) {
    LOG(DEBUG, __FUNCTION__);
    std::string apiJsonPath   = (request->phone_id() == SLOT_1) ? JSON_PATH1 : JSON_PATH2;
    std::string stateJsonPath = (request->phone_id() == SLOT_1) ? JSON_PATH3 : JSON_PATH4;
    std::string subsystem     = MANAGER;
    ...
    if (data.status == telux::common::Status::SUCCESS) {
        int serviceDomainPreference
            = data.stateRootObj[MANAGER]["ServingSystemInfo"]["domain"].asInt();
        ...
        int state = data.stateRootObj[MANAGER]["ServingSystemInfo"]["registrationState"].asInt();
...
grpc::Status ServingManagerServerImpl::RequestRFBandInfo(ServerContext *context,
    const ::telStub::RequestRFBandInfoRequest *request, telStub::RequestRFBandInfoReply *response) {
    LOG(DEBUG, __FUNCTION__);
    std::string apiJsonPath   = (request->phone_id() == SLOT_1) ? JSON_PATH1 : JSON_PATH2;
    std::string stateJsonPath = (request->phone_id() == SLOT_1) ? JSON_PATH3 : JSON_PATH4;
    std::string subsystem     = MANAGER;
    ...
    if (data.status == telux::common::Status::SUCCESS) {
        response->set_band(static_cast<telStub::RFBand>(
            data.stateRootObj[MANAGER]["RFBandInfo"]["rFBand"].asInt()));
        response->set_band_width(static_cast<telStub::RFBandWidth>(
            data.stateRootObj[MANAGER]["RFBandInfo"]["bandwidth"].asInt()));
...
grpc::Status ServingManagerServerImpl::GetNetworkRejectInfo(ServerContext *context,
    ...
    if (data.status == telux::common::Status::SUCCESS) {
        int serviceDomain
            = data.stateRootObj[MANAGER]["NetworkRejectInfo"]["ServingSystemInfo"]["domain"].asInt();
        response->set_reject_domain(static_cast<telStub::ServiceDomainInfo_Domain>(serviceDomain));
        int rat
            = data.stateRootObj[MANAGER]["NetworkRejectInfo"]["ServingSystemInfo"]["rat"].asInt();
        response->set_reject_rat(static_cast<telStub::RadioTechnology>(rat));
        int rejectCause = data.stateRootObj[MANAGER]["NetworkRejectInfo"]["rejectCause"].asInt();
        response->set_reject_cause(rejectCause);
        std::string mcc = data.stateRootObj[MANAGER]["NetworkRejectInfo"]["mcc"].asString();
        response->set_mcc(mcc);
        std::string mnc = data.stateRootObj[MANAGER]["NetworkRejectInfo"]["mnc"].asString();
...
grpc::Status ServingManagerServerImpl::GetCallBarringInfo(ServerContext *context,
    ...
    if (data.status == telux::common::Status::SUCCESS) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            result->set_domain(static_cast<telStub::ServiceDomainInfo_Domain>(serviceDomain));
            int callType = requestedInfo["callType"].asInt();
            result->set_call_type(static_cast<telStub::CallsAllowedInCell_Type>(callType));
        }
    }
    // Create response
    response->set_status(static_cast<commonStub::Status>(data.status));
...
grpc::Status ServingManagerServerImpl::GetLteCsCapability(ServerContext *context,
    ...
    if (data.status == telux::common::Status::SUCCESS) {
        ...
        response->set_capability(static_cast<telStub::LteCsCapability>(capability));
    }
    // Create response
    response->set_status(static_cast<commonStub::Status>(data.status));
...
grpc::Status ServingManagerServerImpl::RequestRFBandPreferences(ServerContext *context,
    ...
    if (data.status == telux::common::Status::SUCCESS) {
        std::string gsmBandsStr
            = data.stateRootObj[MANAGER]["BandPreference"]["gsmBands"].asString();
        std::vector<int> gsmBands = CommonUtils::convertStringToVector(gsmBandsStr);
        std::string wcdmaBandsStr
            = data.stateRootObj[MANAGER]["BandPreference"]["wcdmaBands"].asString();
        std::vector<int> wcdmaBands = CommonUtils::convertStringToVector(wcdmaBandsStr);
        std::string lteBandsStr
            = data.stateRootObj[MANAGER]["BandPreference"]["lteBands"].asString();
        std::vector<int> lteBands = CommonUtils::convertStringToVector(lteBandsStr);
        std::string nsaBandsStr
            = data.stateRootObj[MANAGER]["BandPreference"]["nsaBands"].asString();
        std::vector<int> nsaBands = CommonUtils::convertStringToVector(nsaBandsStr);
        std::string saBandsStr = data.stateRootObj[MANAGER]["BandPreference"]["saBands"].asString();
        std::vector<int> saBands = CommonUtils::convertStringToVector(saBandsStr);
        for (auto &g : gsmBands) {
...
grpc::Status ServingManagerServerImpl::SetRFBandPreferences(ServerContext *context,
    ...
    if (data.status == telux::common::Status::SUCCESS) {
        ...
        data.stateRootObj[MANAGER]["BandPreference"]["gsmBands"]   = gsmBandValue;
        data.stateRootObj[MANAGER]["BandPreference"]["wcdmaBands"] = wcdmaBandValue;
        data.stateRootObj[MANAGER]["BandPreference"]["lteBands"]   = lteBandValue;
        data.stateRootObj[MANAGER]["BandPreference"]["nsaBands"]   = nsaBandValue;
        data.stateRootObj[MANAGER]["BandPreference"]["saBands"]    = saBandValue;
        JsonParser::writeToJsonFile(data.stateRootObj, stateJsonPath);
    }
    // Create response
    if (data.cbDelay != -1) {
    ...
    int phoneId = request->phone_id();
    auto f      = std::async(
        std::launch::async, [this, phoneId, gsmBands, wcdmaBands, lteBands, nsaBands, saBands]() {
            std::string stateJsonPath = (phoneId == SLOT_1) ? "tel/IServingSystemManagerStateSlot1"
                                                                 : "tel/IServingSystemManagerStateSlot2";
            int domain                = stoi(CommonUtils::readSystemDataValue(
                stateJsonPath, "0", {"IServingSystemManager", "ServiceDomainPreference"}));
...
grpc::Status ServingManagerServerImpl::RequestRFBandCapability(ServerContext *context,
    ...
    if (data.status == telux::common::Status::SUCCESS) {
        std::string gsmBandsStr
            = data.stateRootObj[MANAGER]["BandCapability"]["gsmBands"].asString();
        std::vector<int> gsmBands = CommonUtils::convertStringToVector(gsmBandsStr);
        std::string wcdmaBandsStr
            = data.stateRootObj[MANAGER]["BandCapability"]["wcdmaBands"].asString();
        std::vector<int> wcdmaBands = CommonUtils::convertStringToVector(wcdmaBandsStr);
        std::string lteBandsStr
            = data.stateRootObj[MANAGER]["BandCapability"]["lteBands"].asString();
        std::vector<int> lteBands = CommonUtils::convertStringToVector(lteBandsStr);
        std::string nrBandsStr = data.stateRootObj[MANAGER]["BandCapability"]["nrBands"].asString();
...
void ServingManagerServerImpl::handleNetworkRejectionUpdateEvent(std::string eventParams) {
    ...
    try {
        ...
        LOG(DEBUG, __FUNCTION__, "The Slot id is: ", slotId, " leftover string is: ", eventParams);
        ...
        std::string stateJsonPath = (slotId == SLOT_1) ? "tel/IServingSystemManagerStateSlot1"
                                                       : "tel/IServingSystemManagerStateSlot2";
        ...
        CommonUtils::writeSystemDataValue<std::string>(
            stateJsonPath, mcc, {"IServingSystemManager", "NetworkRejectInfo", "mcc"});
        CommonUtils::writeSystemDataValue<std::string>(
            stateJsonPath, mnc, {"IServingSystemManager", "NetworkRejectInfo", "mnc"});
...
void ServingManagerServerImpl::handleRfBandInfoUpdateEvent(std::string eventParams) {
    ...
    try {
        ...
        LOG(INFO, __FUNCTION__, " band is ", band, " channel is ", channel, " bandWidth is ",
            bandWidth);

        std::string stateJsonPath = (slotId == SLOT_1) ? "tel/IServingSystemManagerStateSlot1"
                                                       : "tel/IServingSystemManagerStateSlot2";

        CommonUtils::writeSystemDataValue<int>(
            stateJsonPath, band, {"IServingSystemManager", "RFBandInfo", "rFBand"});
        CommonUtils::writeSystemDataValue<int>(
            stateJsonPath, channel, {"IServingSystemManager", "RFBandInfo", "channel"});
        CommonUtils::writeSystemDataValue<int>(
            stateJsonPath, bandWidth, {"IServingSystemManager", "RFBandInfo", "bandwidth"});
...
void ServingManagerServerImpl::handleSystemSelectionPreferenceChanged(std::string eventParams) {
    ...
    try {
        ...
        LOG(INFO, __FUNCTION__, "Band data for json file is: gsm bands ", gsmBandValue,
            ", wcdma bands ", wcdmaBandValue, ", lte bands ", lteBandValue, ", nsa bands ",
            nsaBandValue, ", sa bands ", saBandValue);

        std::string stateJsonPath = (slotId == SLOT_1) ? "tel/IServingSystemManagerStateSlot1"
                                                       : "tel/IServingSystemManagerStateSlot2";

        CommonUtils::writeSystemDataValue<std::string>(
            stateJsonPath, value, {"IServingSystemManager", "RATPreference"});
        CommonUtils::writeSystemDataValue<int>(
            stateJsonPath, domain, {"IServingSystemManager", "ServiceDomainPreference"});
        CommonUtils::writeSystemDataValue<std::string>(
            stateJsonPath, gsmBandValue, {"IServingSystemManager", "BandPreference", "gsmBands"});
        CommonUtils::writeSystemDataValue<std::string>(stateJsonPath, wcdmaBandValue,
            {"IServingSystemManager", "BandPreference", "wcdmaBands"});
        CommonUtils::writeSystemDataValue<std::string>(
            stateJsonPath, lteBandValue, {"IServingSystemManager", "BandPreference", "lteBands"});
        CommonUtils::writeSystemDataValue<std::string>(
            stateJsonPath, nsaBandValue, {"IServingSystemManager", "BandPreference", "nsaBands"});
        CommonUtils::writeSystemDataValue<std::string>(
            stateJsonPath, saBandValue, {"IServingSystemManager", "BandPreference", "saBands"});
...
void ServingManagerServerImpl::handleSystemInfoUpdateEvent(std::string eventParams) {
    ...
    try {
        ...
        LOG(INFO, __FUNCTION__, " Rat is ", currentServingRat, " Domain is ", currentServingDomain,
            " currentRegistrationState is ", currentRegistrationState, " EndcAvailability is ",
            endcAvailability, " DcnrRestriction is ", dcnrRestriction, " SmsRat is ", smsRat,
            " SmsDomain is ", smsDomain, " NtnSmsStatus is ", ntnSmsStatus, " LteCapability is ",
            lteCapability);
        ...
        CommonUtils::writeSystemDataValue<int>(
            stateJsonPath, smsRat, {"IServingSystemManager", "SmsCapability", "rat"});
        CommonUtils::writeSystemDataValue<int>(
            stateJsonPath, smsDomain, {"IServingSystemManager", "SmsCapability", "domain"});
        CommonUtils::writeSystemDataValue<int>(stateJsonPath, ntnSmsStatus,
            {"IServingSystemManager", "SmsCapability", "ntnSmsStatus"});
        CommonUtils::writeSystemDataValue<int>(
            stateJsonPath, lteCapability, {"IServingSystemManager", "LteCsCapability"});
        ...
        systemInfoEvent.set_lte_capability(static_cast<telStub::LteCsCapability>(lteCapability));
        ...
        for (int i = 1; i <= newInfoCount; i++) {
            ...
            result->set_rat(static_cast<telStub::RadioTechnology>(rat));
            result->set_domain(static_cast<telStub::ServiceDomainInfo_Domain>(domain));
            result->set_call_type(static_cast<telStub::CallsAllowedInCell_Type>(callType));
        }
...
void ServingManagerServerImpl::handleNetworkTimeUpdateEvent(std::string eventParams) {
    ...
    try {
        ...
        LOG(INFO, __FUNCTION__, " year is ", year, " month is ", month, " day is ", day,
            " hour is ", hour, " minute is ", minute, " dayOfWeek is ", dayOfWeek, " timeZone is ",
            timeZone, " dstAdj is ", dstAdj, " nitzTime is ", nitzTime, " second is", second);

        std::string stateJsonPath = (slotId == SLOT_1) ? "tel/IServingSystemManagerStateSlot1"
                                                       : "tel/IServingSystemManagerStateSlot2";

        CommonUtils::writeSystemDataValue<int>(
            stateJsonPath, year, {"IServingSystemManager", "NetworkTimeInfo", "year"});
        CommonUtils::writeSystemDataValue<int>(
            stateJsonPath, month, {"IServingSystemManager", "NetworkTimeInfo", "month"});
        CommonUtils::writeSystemDataValue<int>(
            stateJsonPath, day, {"IServingSystemManager", "NetworkTimeInfo", "day"});
        CommonUtils::writeSystemDataValue<int>(
            stateJsonPath, hour, {"IServingSystemManager", "NetworkTimeInfo", "hour"});
        CommonUtils::writeSystemDataValue<int>(
            stateJsonPath, minute, {"IServingSystemManager", "NetworkTimeInfo", "minute"});
        CommonUtils::writeSystemDataValue<int>(
            stateJsonPath, second, {"IServingSystemManager", "NetworkTimeInfo", "second"});
        CommonUtils::writeSystemDataValue<int>(
            stateJsonPath, dayOfWeek, {"IServingSystemManager", "NetworkTimeInfo", "dayOfWeek"});
        CommonUtils::writeSystemDataValue<int>(
            stateJsonPath, timeZone, {"IServingSystemManager", "NetworkTimeInfo", "timeZone"});
        CommonUtils::writeSystemDataValue<int>(
            stateJsonPath, dstAdj, {"IServingSystemManager", "NetworkTimeInfo", "dstAdj"});
        CommonUtils::writeSystemDataValue<std::string>(
            stateJsonPath, nitzTime, {"IServingSystemManager", "NetworkTimeInfo", "nitzTime"});
...
void ServingManagerServerImpl::handleRrcStateEvent(std::string eventParams) {
    ...
    try {
        ...
        CommonUtils::writeSystemDataValue<int>(
            stateJsonPath, wcdmaRS, {"IServingSystemManager", "RrcState", "wcdmaRrcState"});
        CommonUtils::writeSystemDataValue<int>(
            stateJsonPath, lteRS, {"IServingSystemManager", "RrcState", "lteRrcState"});
        CommonUtils::writeSystemDataValue<int>(
            stateJsonPath, nr5gRS, {"IServingSystemManager", "RrcState", "nr5gRrcState"});
