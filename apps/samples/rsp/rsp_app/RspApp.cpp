...
void RemoteSimProfile::addProfile(
    ...
    if (simProfileManager_) {
        ...
        Status status = simProfileManager_->addProfile(
            slotId_, actCode, confCode, isUserConsentRequired, respCb);
...
void RemoteSimProfile::setServerAddress(std::string serverAddress) {
    ...
    if (simProfileManager_) {
        ...
        Status status = simProfileManager_->setServerAddress(slotId_, serverAddress, respCb);
        if (status == Status::SUCCESS) {
...
void RemoteSimProfile::getServerAddress() {
    ...
    if (simProfileManager_) {
        ...
        if (status == Status::SUCCESS) {
            std::cout << "getServerAddress request sent successfully" << std::endl;
        } else {
...
void RemoteSimProfile::memoryReset(int resetOption) {
    ...
    if (simProfileManager_) {
        ...
        Status status = simProfileManager_->memoryReset(slotId_, resetmask, respCb);
        if (status == Status::SUCCESS) {
            std::cout << "memoryReset request sent successfully" << std::endl;
        } else {
...
void RemoteSimProfile::onEidResponse(std::string eid, telux::common::ErrorCode errorCode) {
    if (errorCode == telux::common::ErrorCode::SUCCESS) {
...
void RemoteSimProfile::provideUserConsent(bool isUserConsentRequired, int reason) {
    ...
    if (simProfileManager_) {
        Status status = simProfileManager_->provideUserConsent(
            slotId_, isUserConsentRequired, reasonType, respCb);
        if (status == Status::SUCCESS) {
            std::cout << "provideUserConsent request sent successfully" << std::endl;
        } else {
...
void RemoteSimProfile::provideUserConfirmation(std::string confirmationCode) {
    ...
    if (simProfileManager_) {
        Status status
            = simProfileManager_->provideConfirmationCode(slotId_, confirmationCode, respCb);
        if (status == Status::SUCCESS) {
            std::cout << "provideUserConfirmation request sent successfully" << std::endl;
        } else {
...
void RemoteSimProfile::serverAddressResponse(
    std::string smdpAddress, std::string smdsAddress, telux::common::ErrorCode error) {
...
void RemoteSimProfile::printUsage(char **argv) {
    ...
    std::cout << "\t                                        Reason for not OK (1 - POSTPONE, 0 - "
                 "REJECT)\n";
    ...
    if (telux::common::DeviceConfig::isMultiSimSupported()) {
        std::cout << "   rsp_app --slot-id 1 --add LPA:$XXX.xxx "
                     " 0 \n";
    } else {
        std::cout << "   rsp_app --add LPA:$XXX.xxx "
                     " 0 \n";
    }
    ...
    while (1) {
        ...
        if (telux::common::DeviceConfig::isMultiSimSupported()) {
            static struct option longOptions[] = {{"slot-id", required_argument, 0, 's'},
                ...
                {"user-confirmation", required_argument, 0, 'f'}, {"eid", no_argument, 0, 'i'},
                {"profile-list", no_argument, 0, 'p'}, {"get-address", no_argument, 0, 'g'},
        ...
        switch (c) {
            ...
            case 'a':
                ...
                std::cout << "Adding profile with activation code: " << activationCode_
                          << std::endl;
                std::cout << "User consent supported for add profile : " << userConsent_
                          << std::endl;
