...
telux::common::Status ImsServingSystemManagerStub::init(telux::common::InitResponseCb callback) {
    LOG(DEBUG, __FUNCTION__);
...
telux::common::Status ImsServingSystemManagerStub::requestRegistrationInfo(
    ImsRegistrationInfoCb callback) {
    LOG(DEBUG, __FUNCTION__);
    ...
    if ((status == telux::common::Status::SUCCESS) && (isCallbackNeeded)) {
        auto f1 = std::async(std::launch::async, [this, info, error, callback, delay]() {
            if (callback) {
