...
class AecsMakeCallCallback {
 public:
    void makeCallResponse(ErrorCode error, std::shared_ptr<ICall> call = nullptr) {
...
void retryDroppedAecsCall(int phoneId) {
    ...
    for (auto &call : inProgressCalls) {
        if (isEmergencyMode(call->getPhoneId()) && call->getCallState() == CallState::CALL_ENDED
            && call->isAecsCallDrop()) {
    ...
    while (true) {
        telux::common::Status status = callMgr_->makeAecsCall(phoneId, aecsId,
...
void retryMsdOverSms(int phoneId) {
    ...
    while (true) {
        Status status
            = smsMgr->sendRawSms(rawPdus, std::bind(&AecsSmsCommandCallback::sendSmsResponse, cbObj,
...
class AecsSmsListener : public ISmsListener {
 public:
    void onIncomingSms(int phoneId, std::shared_ptr<SmsMessage> smsMsg) {
        ...
        if (partInfo) {
            std::cout << "Received SMS on phone ID " << phoneId << " from: " << smsMsg->getSender()
    ...
    void onIncomingSms(int phoneId, std::shared_ptr<std::vector<SmsMessage>> msgs) {
        ...
        for (SmsMessage smsMsg : messages) {
            ...
            if (partInfo) {
                std::cout
                    << "\033[1;35mSegment: \033[0m" << static_cast<int>(partInfo->segmentNumber)
                    << "\n SMS Part on phone ID " << phoneId << " from: " << smsMsg.getSender()
                    << " to: " << smsMsg.getReceiver() << "\n Message Part: " << smsMsg.getText()
                    << "\n PDU: " << smsMsg.getPdu()
                    << "\n RefNumber:" << static_cast<int>(partInfo->refNumber)
                    << " NumberOfSegments:" << static_cast<int>(partInfo->numberOfSegments)
                    << " SegmentNumber: " << static_cast<int>(partInfo->segmentNumber) << std::endl;
            }
        }
    ...
    void onDeliveryReport(int phoneId, int msgRef, std::string receiverAddress, ErrorCode error) {
        std::cout << std::endl << std::endl;
        std::cout << "Received delivery report from phone ID " << phoneId
...
static Status sendMsdOverSms(int phoneId, const std::vector<uint8_t> &msdPdu) {
    ...
    std::cout << "[MSD SMS] Sending MSD over SMS (" << msdPdu.size() << " bytes, 1 PDU) ...\n";
    auto cbObj = std::make_shared<AecsSmsCommandCallback>();
    Status st
        = smsMgr->sendRawSms(rawPdus, std::bind(&AecsSmsCommandCallback::sendSmsResponse, cbObj,
                                          std::placeholders::_1, std::placeholders::_2));
    std::cout << "[MSD SMS] sendRawSms() returned status=" << static_cast<int>(st) << "\n";
    return st;
}
...
class AecsCallListener : public ICallListener {
 public:
    void onIncomingCall(std::shared_ptr<ICall> call) override {
        ...
        std::cout << "\n[Listener] Incoming call: number=" << call->getRemotePartyNumber()
    ...
    void onCallInfoChange(std::shared_ptr<ICall> call) override {
        ...
        std::cout << "\n[Listener] Call info change: index=" << call->getCallIndex()
                  << " phoneId=" << call->getPhoneId() << " state=" << static_cast<int>(state)
                  << "\n";
        ...
        switch (state) {
            ...
            case CallState::CALL_ENDED:
                ...
                std::cout
                    << " Cause of call termination: " << static_cast<int>(call->getCallEndCause())
                    << std::endl;
...
static void terminateCallsOnOtherPhones(std::shared_ptr<ICallManager> cm, int targetPhoneId) {
    ...
    for (auto &c : calls) {
        if (!c)
            continue;
        if (c->getCallState() != CallState::CALL_ENDED && c->getPhoneId() != targetPhoneId) {
            std::cout << "[Helper] Hanging up call on other phoneId=" << c->getPhoneId() << "\n";
            auto st = c->hangup();
...
static bool enterEmergencyMode(std::shared_ptr<ICallManager> cm, int phoneId) {
    ...
    if (it != emergencyMode_.end() && it->second) {
        std::cout << "[EM] Emergency mode already enabled on phoneId " << phoneId << std::endl;
        return true;
    }
    Status st = cm->setEmergencyMode(phoneId, true, false);
    if (st == Status::SUCCESS) {
        emergencyMode_[phoneId] = true;
        std::cout << "[EM] Emergency mode enabled on phoneId " << phoneId << std::endl;
    } else {
        std::cout << "[EM] Failed to enable emergency mode on phoneId " << phoneId
...
static void exitEmergencyMode(std::shared_ptr<ICallManager> cm, int phoneId) {
    ...
    auto it = emergencyMode_.find(phoneId);
    if (it == emergencyMode_.end() || !it->second) {
        std::cout << "[EM] Emergency mode already disabled on phoneId " << phoneId << std::endl;
        return;
    }
    Status st = cm->setEmergencyMode(phoneId, false, false);
    if (st == Status::SUCCESS) {
        emergencyMode_[phoneId] = false;
        std::cout << "[EM] Emergency mode disabled on phoneId " << phoneId << std::endl;
    } else {
        std::cout << "[EM] Failed to disable emergency mode on phoneId " << phoneId
                  << ", status=" << (int)st << std::endl;
    }
}
...
int main() {
    ...
    if (svcStatus != ServiceStatus::SERVICE_AVAILABLE) {
        std::cerr << "ERROR: CallManager not ready, status=" << static_cast<int>(svcStatus) << "\n";
        return 1;
    }
    ...
    if (shouldOriginate) {
        std::cout << "\nPlacing AECS call on phoneId=" << phoneId << " to '" << aecsId << "'\n";
        Status makeStatus = callMgr->makeAecsCall(phoneId, aecsId,
            std::bind(&AecsMakeCallCallback::makeCallResponse, cbObj, std::placeholders::_1,
                std::placeholders::_2));
        std::cout << "makeAecsCall() returned status=" << static_cast<int>(makeStatus) << "\n";
    ...
    {
        std::lock_guard<std::mutex> lock(gCallMutex);
        if (gAecsCall && gAecsCall->getCallState() != CallState::CALL_ENDED) {
