...
grpc::Status SubscriptionManagerServerImpl::InitService(ServerContext *context,
    ...
    if (readStatus.ok()) {
        ...
        std::string cbStatus = rootObj["ISubscriptionManager"]["IsSubsystemReady"].asString();
...
grpc::Status SubscriptionManagerServerImpl::GetSubscription(ServerContext *context,
    ...
    std::string icc_id = rootObj["ISubscriptionManager"]["Subscription"][i]["iccId"].asString();
...
void SubscriptionManagerServerImpl::handlesubscriptionInfoChanged(std::string eventParams) {
    ...
    LOG(DEBUG, __FUNCTION__, "The fetched mcc is: ", mcc, "The leftover string is: ", eventParams);
    ...
    LOG(DEBUG, __FUNCTION__, "Carrier name is", carrierName, "Phone number is", phoneNumber,
        "iccid is", iccId, "mcc is", mcc, "mnc is", mnc, "imsi is", imsi, "gid1 is", gid1,
        "gid2 is", gid2);
