...
bool ConnectionHandler::startDataCall(std::shared_ptr<Connection> connection) {
    ...
    while (!connection->dataCall && attempt < maxAttempts) {
        auto retStat = connection->dataConnectionManager->startDataCall(connection->profileId,
            connection->ipFamily, dataCallCallback, telux::data::OperationType::DATA_LOCAL);
