...
class ThermalJsonImpl {
 public:
    ...
    telux::common::Status getThermalZoneById(
        int tZoneId, std::shared_ptr<telux::therm::ThermalZoneImpl> &tz);
    telux::common::Status getCoolingDeviceById(
        int cDevId, std::shared_ptr<telux::therm::CoolingDeviceImpl> &cDev);

    std::map<int, int> getCoolingDeviceLevel(int tZoneId, int tripId, int trend, int cDevId = -1);
