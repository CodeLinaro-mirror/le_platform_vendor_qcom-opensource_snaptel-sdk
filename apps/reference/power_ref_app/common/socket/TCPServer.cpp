...
private:
  ...
  void listenTCPSync() {
    ...
    do {
      ...
      if (!this->connectionConfig_->clientIpAddr.empty() &&
        ...
        LOG(WARNING, __FUNCTION__, " rejected client IP: ", std::string(clientIp));
        close(this->clientSocket_);
        continue;
      }
  ...
  bool updateConnectionParams() {
    ...
    if (getsockname(soc, (struct sockaddr *)&localAddr, &addrLen) == -1) {
      // std::string err = " getsockname failed: "+ std::string(strerror(errno));
      LOG(ERROR, __FUNCTION__, " getsockname failed: ", std::string(strerror(errno)));
      return false;
    }
    ...
    if (getpeername(soc, (struct sockaddr *)&peerAddr, &addrLen) == -1) {
      LOG(ERROR, __FUNCTION__, " getpeername failed: ", std::string(strerror(errno)));
      return false;
    }
