/*
 * Generated by asn1c-0.9.29 (http://lionet.info/asn1c)
 * From ASN.1 module "IEEE-1609-3-WSA"
 * 	found in "../Ieee1609Dot3Wsa.asn"
 * 	`asn1c -fcompound-names -gen-PER`
 */

#include "ChannelOptions.h"

static asn_TYPE_member_t asn_MBR_ChannelOptions_1[] = {
    {ATF_POINTER, 3, offsetof(struct ChannelOptions, reserved1), (ASN_TAG_CLASS_CONTEXT | (0 << 2)),
        -1, /* IMPLICIT tag at current level */
        &asn_DEF_NULL, 0, /* Defer constraints checking to the member type */
        0, /* No PER visible constraints */
        0, "reserved1"},
    {ATF_POINTER, 2, offsetof(struct ChannelOptions, reserved2), (ASN_TAG_CLASS_CONTEXT | (1 << 2)),
        -1, /* IMPLICIT tag at current level */
        &asn_DEF_NULL, 0, /* Defer constraints checking to the member type */
        0, /* No PER visible constraints */
        0, "reserved2"},
    {ATF_POINTER, 1, offsetof(struct ChannelOptions, extensions),
        (ASN_TAG_CLASS_CONTEXT | (2 << 2)), -1, /* IMPLICIT tag at current level */
        &asn_DEF_ServiceInfoExts, 0, /* Defer constraints checking to the member type */
        0, /* No PER visible constraints */
        0, "extensions"},
};
static const int asn_MAP_ChannelOptions_oms_1[] = {0, 1, 2};
static const ber_tlv_tag_t asn_DEF_ChannelOptions_tags_1[]
    = {(ASN_TAG_CLASS_UNIVERSAL | (16 << 2))};
static const asn_TYPE_tag2member_t asn_MAP_ChannelOptions_tag2el_1[] = {
    {(ASN_TAG_CLASS_CONTEXT | (0 << 2)), 0, 0, 0}, /* reserved1 */
    {(ASN_TAG_CLASS_CONTEXT | (1 << 2)), 1, 0, 0}, /* reserved2 */
    {(ASN_TAG_CLASS_CONTEXT | (2 << 2)), 2, 0, 0} /* extensions */
};
static asn_SEQUENCE_specifics_t asn_SPC_ChannelOptions_specs_1 = {
    sizeof(struct ChannelOptions), offsetof(struct ChannelOptions, _asn_ctx),
    asn_MAP_ChannelOptions_tag2el_1, 3, /* Count of tags in the map */
    asn_MAP_ChannelOptions_oms_1, /* Optional members */
    3, 0, /* Root/Additions */
    -1, /* Start extensions */
    -1 /* Stop extensions */
};
asn_TYPE_descriptor_t asn_DEF_ChannelOptions = {
    "ChannelOptions", "ChannelOptions", SEQUENCE_free, SEQUENCE_print, SEQUENCE_constraint,
    SEQUENCE_decode_ber, SEQUENCE_encode_der, SEQUENCE_decode_xer, SEQUENCE_encode_xer,
    SEQUENCE_decode_uper, SEQUENCE_encode_uper, 0, /* Use generic outmost tag fetcher */
    asn_DEF_ChannelOptions_tags_1,
    sizeof(asn_DEF_ChannelOptions_tags_1) / sizeof(asn_DEF_ChannelOptions_tags_1[0]), /* 1 */
    asn_DEF_ChannelOptions_tags_1, /* Same as above */
    sizeof(asn_DEF_ChannelOptions_tags_1) / sizeof(asn_DEF_ChannelOptions_tags_1[0]), /* 1 */
    0, /* No PER visible constraints */
    asn_MBR_ChannelOptions_1, 3, /* Elements count */
    &asn_SPC_ChannelOptions_specs_1 /* Additional specs */
};
