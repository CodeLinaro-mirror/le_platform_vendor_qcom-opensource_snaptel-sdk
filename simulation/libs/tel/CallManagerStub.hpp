...
class CallManagerStub : public ICallManager,
 ...
 public:
    ...
    telux::common::Status makeECall(int phoneId, const ECallMsdData &eCallMsdData, int category,
        int variant, std::shared_ptr<IMakeCallCallback> callback,
    ...
    telux::common::Status makeECall(int phoneId, const std::vector<uint8_t> &msdPdu, int category,
        int variant, MakeCallCallback callback,
        TestECallConfig config = {telux::tel::TestECallConfigType::DEFAULT_SDN_URI, ""}) override;
    telux::common::Status makeECall(int phoneId, const std::string dialNumber,
        const std::vector<uint8_t> &msdPdu, int category, MakeCallCallback callback) override;
    ...
    telux::common::Status requestECallHlapTimerStatus(
        int phoneId, ECallHlapTimerStatusCallback callback) override;
    ...
    telux::common::Status hangupForegroundResumeBackground(
        int phoneId, common::ResponseCallback callback) override;
    ...
    telux::common::Status requestEcallHlapTimer(
        int phoneId, HlapTimerType type, ECallHlapTimerCallback callback) override;
    ...
    telux::common::ErrorCode encodeECallMsd(
        telux::tel::ECallMsdData eCallMsdData, std::vector<uint8_t> &data) override;
    telux::common::Status makeRttCall(int phoneId, const std::string &dialNumber,
        std::shared_ptr<IMakeCallCallback> callback) override;
    telux::common::Status sendRtt(
        int phoneId, std::string message, common::ResponseCallback callback) override;
    ...
    telux::common::Status restartECallHlapTimer(int phoneId, EcallHlapTimerId timerId, int duration,
        common::ResponseCallback callback) override;
    telux::common::ErrorCode getECallRedialConfig(
        std::vector<int> &callOrigTimeGap, std::vector<int> &callDropTimeGap) override;
    telux::common::Status makeECall(int phoneId, const std::string dialNumber,
    ...
    telux::common::Status makeAecsCall(
        int phoneId, const std::string &eCallIdentifier, MakeCallCallback callback) override;
    void updateLastFailCause(
 ...
 private:
    ...
    template <typename T>
    T createRequest(
        int phoneId, const std::string dialNumber, bool isMsdTransmitted, CallApi inputApi) {
        T request;
        CallInfo callInfo;
