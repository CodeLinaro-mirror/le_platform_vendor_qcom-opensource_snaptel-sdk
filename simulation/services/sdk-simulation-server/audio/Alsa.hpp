...
class Alsa : public IAudioBackend,
 ...
 public:
    ...
    telux::common::ErrorCode getSupportedDevices(
        std::vector<DeviceType> &devices, std::vector<DeviceDirection> &devicesDirection) override;

    telux::common::ErrorCode getSupportedStreamTypes(std::vector<StreamType> &streamTypes) override;

    telux::common::ErrorCode createStream(StreamHandle &streamHandle, StreamParams streamParams,
        uint32_t &readBufferMinSize, uint32_t &writeBufferMinSize) override;
    ...
    telux::common::ErrorCode setDevice(
        StreamHandle streamHandle, std::vector<DeviceType> &deviceTypes) override;

    telux::common::ErrorCode getDevice(
        StreamHandle streamHandle, std::vector<DeviceType> &deviceTypes) override;

    telux::common::ErrorCode setVolume(StreamHandle streamHandle, StreamDirection direction,
        std::vector<ChannelVolume> channelsVolume) override;

    telux::common::ErrorCode getVolume(StreamHandle streamHandle, int channelTypeMask,
        std::vector<ChannelVolume> &channelsVolume) override;

    telux::common::ErrorCode setMuteState(StreamHandle streamHandle, StreamMute muteInfo,
        std::vector<ChannelVolume> channelsVolume, bool prevMuteState) override;

    telux::common::ErrorCode getMuteState(
        StreamHandle streamHandle, StreamMute &muteInfo, StreamDirection direction) override;

    telux::common::ErrorCode write(StreamHandle &streamHandle, uint8_t *data,
    ...
    telux::common::ErrorCode startDtmf(
        StreamHandle streamHandle, uint16_t gain, uint16_t duration, DtmfTone dtmfTone) override;

    telux::common::ErrorCode stopDtmf(
        StreamHandle streamHandle, StreamDirection direction) override;
    ...
    telux::common::ErrorCode setupInTranscodeStream(StreamHandle &streamHandle, uint32_t streamId,
        TranscodingFormatInfo inInfo, std::shared_ptr<IStreamEventListener> streamEventListener,
        uint32_t &writeMinSize) override;

    telux::common::ErrorCode setupOutTranscodeStream(StreamHandle &streamHandle, uint32_t streamId,
        TranscodingFormatInfo outInfo, std::shared_ptr<IStreamEventListener> streamEventListener,
 ...
 private:
    ...
    int loadMappingArray(std::string key, MappedValueType mappedValueType, uint32_t numOfValues,
    ...
    telux::common::ErrorCode setBufferSize(
        StreamHandle streamHandle, size_t &inSize, size_t &outSize);
    telux::common::ErrorCode startLoopback(
