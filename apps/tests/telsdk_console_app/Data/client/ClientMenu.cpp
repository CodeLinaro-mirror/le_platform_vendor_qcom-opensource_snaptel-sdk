...
bool ClientMenu::init() {
    ...
    if (menuOptionsAdded_ == false) {
        ...
        std::shared_ptr<ConsoleAppCommand> resetDataUsageStatsCmd
            = std::make_shared<ConsoleAppCommand>(ConsoleAppCommand("2", "Reset_Data_Usage_Stats",
                {}, std::bind(&ClientMenu::resetDataUsageStats, this, std::placeholders::_1)));

        std::vector<std::shared_ptr<ConsoleAppCommand>> commandsList
            = {getDeviceDataUsageStatsCmd, resetDataUsageStatsCmd};
...
bool ClientMenu::initClientManager() {
    ...
    // std:: string opTypeStr = (opType == telux::data::OperationType::DATA_LOCAL)? "Local" :
    // "Remote";
    if (clientMgr) {
