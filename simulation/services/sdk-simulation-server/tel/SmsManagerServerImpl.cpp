...
grpc::Status SmsManagerServerImpl::InitService(ServerContext *context,
    ...
    if (readStatus.ok()) {
        ...
        std::string cbStatus = rootObj[TEL_SMS_MANAGER]["IsSubsystemReady"].asString();
...
void SmsManagerServerImpl::getJsonForSystemData(
...
void SmsManagerServerImpl::getJsonForApiResponseSlot(
    int phoneId, std::string &jsonfilename, Json::Value &rootObj) {
...
grpc::Status SmsManagerServerImpl::SetSmscAddress(ServerContext *context,
    const ::telStub::SetSmscAddressRequest *request, telStub::SetSmscAddressReply *response) {
    LOG(DEBUG, __FUNCTION__);
    int phoneId              = request->phone_id();
...
grpc::Status SmsManagerServerImpl::GetSmscAddress(ServerContext *context,
    const ::telStub::GetSmscAddressRequest *request, telStub::GetSmscAddressReply *response) {
    LOG(DEBUG, __FUNCTION__);
    int phoneId              = request->phone_id();
    std::string jsonfilename = "";
    ...
    if (readStatus.ok()) {
        ...
        CommonUtils::getValues(jsonObjApiResponse, TEL_SMS_MANAGER, apiname, status, error, delay);
        if (status == telux::common::Status::SUCCESS) {
            smscAddress = rootObj[TEL_SMS_MANAGER]["setSmscAddress"]["smscAddress"].asString();
        }
        // Create response
        bool isCallback = isCallbackNeeded(jsonObjApiResponse, apiname);
            ...
            for (int i = 0; i < size; i++) {
                ...
                if (type == telux::tel::SmsTagType::UNKNOWN) {
                    ...
                    if (typeInput == tagType) {
                        Info.msgIndex
                            = rootObj[TEL_SMS_MANAGER][storageName][i]["smsMetaInfo_msgIndex"]
                                  .asInt();
    ...
    if (readStatus.ok()) {
        ...
        if (status == telux::common::Status::SUCCESS) {
            for (index_for_db = 0; index_for_db < size; index_for_db++) {
                uint32_t Id = -1;
                Id          = std::stoi(
                    rootObj[TEL_SMS_MANAGER][storageName][index_for_db]["smsMetaInfo_msgIndex"]
                        .asString());
...
void SmsManagerServerImpl::parseMessageAtIndex(
    int phoneId, int index, SmsMsg &msg, std::string storageName) {
    ...
    if (readStatus.ok()) {
        ...
        std::string encoding = rootObj[TEL_SMS_MANAGER][storageName][index]["encoding"].asString();
        msg.encoding         = Helper::getencodingMethod(encoding);
        msg.pdu              = rootObj[TEL_SMS_MANAGER][storageName][index]["pdu"].asString();
        msg.msgIndex = rootObj[TEL_SMS_MANAGER][storageName][index]["smsMetaInfo_msgIndex"].asInt();
        msg.messageInfoRefNumber
            = rootObj[TEL_SMS_MANAGER][storageName][index]["messagePartInfo_refNumber"].asInt();
        msg.messageInfoSegments
            = rootObj[TEL_SMS_MANAGER][storageName][index]["messagePartInfo_numberOfSegments"]
                  .asInt();
        msg.messageInfoSegmentNumber
            = rootObj[TEL_SMS_MANAGER][storageName][index]["messagePartInfo_segmentNumber"].asInt();
        ...
        std::string tagType
            = rootObj[TEL_SMS_MANAGER][storageName][index]["smsMetaInfo_tagType"].asString();
        msg.tagType = Helper::getTagType(tagType);
        msg.isMetaInfoValid
            = rootObj[TEL_SMS_MANAGER][storageName][index]["isMetaInfoValid"].asBool();
        ...
        if (status == telux::common::Status::SUCCESS) {
            ...
            for (index_for_db = 0; index_for_db < size; index_for_db++) {
                if (deltype == telux::tel::DeleteType::DELETE_MESSAGES_BY_TAG) {
                    ...
                    uint32_t Id = rootObj[TEL_SMS_MANAGER][storageName][index_for_db]["smsMetaInfo_"
                                                                                      "msgIndex"]
                                      .asInt();
                    if (Id == messageIndex) {
...
telux::common::ErrorCode SmsManagerServerImpl::deletedSmsatIndex(
    ...
    if (readStatus.ok()) {
        ...
        for (auto itr = index.begin(); itr != index.end(); itr++) {
            ...
            for (int i = 0; i < numprofiles; i++) {
                if (i != *itr) {
                    newRoot[storageName][newCount] = rootObj[TEL_SMS_MANAGER][storageName][i];
                    newCount++;
                } else {
...
grpc::Status SmsManagerServerImpl::RequestPreferredStorage(ServerContext *context,
    ...
    if (readStatus.ok()) {
        ...
        if (status == telux::common::Status::SUCCESS) {
            std::string storage
                = rootObj[TEL_SMS_MANAGER]["setPreferredStorage"]["storageType"].asString();
...
grpc::Status SmsManagerServerImpl::SetTag(
    ServerContext *context, const telStub::SetTagRequest *request, telStub::SetTagReply *response) {
...
grpc::Status SmsManagerServerImpl::RequestStorageDetails(ServerContext *context,
    ...
    if (readStatus.ok()) {
        ...
        if (status == telux::common::Status::SUCCESS) {
            ...
            maxCount
                = jsonObjApiResponse[TEL_SMS_MANAGER]["requestStorageDetails"][maxCountName].asInt();
...
grpc::Status SmsManagerServerImpl::GetMessageAttributes(ServerContext *context,
    ...
    if (readStatus.ok()) {
        getJsonForSystemData(phoneId, jsonfilename, rootObj);
        std::string val
            = rootObj[TEL_SMS_MANAGER]["calculateMessageAttributes"]["messageAttributes_encoding"]
                  .asString();
        telux::tel::SmsEncoding encoding = Helper::getencodingMethod(val);
        int numberOfSegments
            = rootObj[TEL_SMS_MANAGER]["calculateMessageAttributes"]["messageAttributes_"
                                                                     "numberOfSegments"]
                  .asInt();
        int segmentSize = rootObj[TEL_SMS_MANAGER]["calculateMessageAttributes"]["messageAttributes"
                                                                                 "_segmentSize"]
                              .asInt();
        int numberOfCharsLeftInLastSegment
            = rootObj[TEL_SMS_MANAGER]["calculateMessageAttributes"]["messageAttributes_"
                                                                     "numberOfCharsLeftInLastSegmen"
                                                                     "t"]
                  .asInt();
...
grpc::Status SmsManagerServerImpl::SendSmsWithoutSmsc(ServerContext *context,
    const telStub::SendSmsWithoutSmscRequest *request, telStub::SendSmsWithoutSmscReply *response) {
    LOG(DEBUG, __FUNCTION__);
    std::string jsonfilename = "";
    Json::Value rootObj;
    int phoneId = request->phone_id();
    ...
    if (readStatus.ok()) {
        ...
        int noOfSegments
            = rootObj[TEL_SMS_MANAGER]["sendSmsDeprecated"][0]["numberOfSegments"].asInt();
        ...
        int sentCallbackDelay
            = rootObj[TEL_SMS_MANAGER]["sendSmsDeprecated"][1]["sentCallbackDelay"].asInt();
        std::string refs
            = rootObj[TEL_SMS_MANAGER]["sendSmsDeprecated"][1]["sentCallbackMsgRefs"].asString();
        tmp = rootObj[TEL_SMS_MANAGER]["sendSmsDeprecated"][2]["deliveryCallbackErrorCode"]
                  .asString();
        telux::common::ErrorCode deliveryCallbackErrorCode = CommonUtils::mapErrorCode(tmp);
        int deliveryCallbackDelay
            = rootObj[TEL_SMS_MANAGER]["sendSmsDeprecated"][2]["deliveryCallbackDelay"].asInt();
...
grpc::Status SmsManagerServerImpl::SendSms(ServerContext *context,
    ...
    if (readStatus.ok()) {
        ...
        telux::common::ErrorCode sentCallbackErrorCode = CommonUtils::mapErrorCode(tmp);
        int smsResponseCbDelay
            = rootObj[TEL_SMS_MANAGER]["sendSms"][1]["smsResponseCbDelay"].asInt();
        std::string refs
            = rootObj[TEL_SMS_MANAGER]["sendSms"][1]["smsResponseCbMsgRefs"].asString();
        int smsResponseCbRejectCode
            = rootObj[TEL_SMS_MANAGER]["sendSms"][1]["smsResponseCbRejectCode"].asInt();

        for (int i = 1; i <= noOfSegments; i++) {
            SmsDeliveryInfo records;
            std::string error
                = rootObj[TEL_SMS_MANAGER]["sendSms"][i + 1]["onDeliveryReportErrorCode"].asString();
            records.errorCode = CommonUtils::mapErrorCode(error);
            records.msgRef
                = rootObj[TEL_SMS_MANAGER]["sendSms"][i + 1]["onDeliveryReportMsgRef"].asInt();
            records.cbDelay
                = rootObj[TEL_SMS_MANAGER]["sendSms"][i + 1]["onDeliveryReportCallbackDelay"]
                      .asInt();
...
grpc::Status SmsManagerServerImpl::SendRawSms(ServerContext *context,
    ...
    if (readStatus.ok()) {
        ...
        std::string recieverAddress
            = rootObj[TEL_SMS_MANAGER]["sendRawSms"][0]["receiverAddress"].asString();
        ...
        telux::common::ErrorCode sentCallbackErrorCode = CommonUtils::mapErrorCode(tmp);
        int smsResponseCbDelay
            = rootObj[TEL_SMS_MANAGER]["sendRawSms"][1]["smsResponseCbDelay"].asInt();
        std::string refs
            = rootObj[TEL_SMS_MANAGER]["sendRawSms"][1]["smsResponseCbMsgRefs"].asString();
        int smsResponseCbRejectCode
            = rootObj[TEL_SMS_MANAGER]["sendRawSms"][1]["smsResponseCbRejectCode"].asInt();

        for (int i = 1; i <= size; i++) {
            SmsDeliveryInfo records;
            std::string error
                = rootObj[TEL_SMS_MANAGER]["sendRawSms"][i + 1]["onDeliveryReportErrorCode"]
                      .asString();
            records.errorCode = CommonUtils::mapErrorCode(error);
            records.msgRef
                = rootObj[TEL_SMS_MANAGER]["sendRawSms"][i + 1]["onDeliveryReportMsgRef"].asInt();
            records.cbDelay
                = rootObj[TEL_SMS_MANAGER]["sendRawSms"][i + 1]["onDeliveryReportCallbackDelay"]
                      .asInt();
            infos.emplace_back(records);
        }
...
int SmsManagerServerImpl::getNewSmsIndex(int phoneId, std::string storageName) {
    ...
    if (rootObj[TEL_SMS_MANAGER][storageName][0]["smsMetaInfo_msgIndex"].asInt() == 0) {
        ...
        for (int i = 0; i < size - 1; i++) {
            currentIndex = rootObj[TEL_SMS_MANAGER][storageName][i]["smsMetaInfo_msgIndex"].asInt();
            nextIndex
                = rootObj[TEL_SMS_MANAGER][storageName][i + 1]["smsMetaInfo_msgIndex"].asInt();
...
void SmsManagerServerImpl::handleIncomingSms(std::string eventParams) {
    ...
    telux::common::ErrorCode error = JsonParser::readFromJsonFile(rootObjSubscription, JSON_PATH5);
    if (error != ErrorCode::SUCCESS) {
        LOG(ERROR, __FUNCTION__, " Reading JSON File failed! ");
    }
    receiver
        = rootObjSubscription["ISubscriptionManager"]["Subscription"][phoneId - 1]["phoneNumber"]
              .asString();
    ...
    std::string storage = rootObj[TEL_SMS_MANAGER]["setPreferredStorage"]["storageType"].asString();
    ...
    auto f = std::async(
        std::launch::async, [this, phoneId, numberOfSegments, refNumber, segmentNumber, msgIndex,
                                tagType, encoding, isMetaInfoValid, pdu, receiver, sender, text]() {
...
void SmsManagerServerImpl::sortDatabase(
    int phoneId, Json::Value newSms, int index, std::string storageName) {
    std::string jsonfilename = "";
    Json::Value rootObj;
    ...
    if (index > currentSMSCount - 1) {
        ...
        for (int i = currentSMSCount - 1; i > index; --i) {
            rootObj[TEL_SMS_MANAGER][storageName][i] = rootObj[TEL_SMS_MANAGER][storageName][i - 1];
        }
...
void SmsManagerServerImpl::triggerIncomingSmsEvent(int phoneId, int numberOfSegments, int refNumber,
