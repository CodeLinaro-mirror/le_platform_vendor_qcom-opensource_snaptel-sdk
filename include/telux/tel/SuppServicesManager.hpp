...
struct ForwardReq {
    ...
    std::string number
        = ""; /**< Number to which call has to be forwarded. This parameter is required only for
                 registration purpose only. @ref telux::tel::ForwardOperation::REGISTER */
    uint8_t noReplyTimer = 0; /**< Timer for no reply operation. Required only for no reply forward
                                 reason. @ref telux::tel::ForwardReason::NOREPLY. */
};
 ...
 *
 */
using GetCallWaitingPrefExCb = std::function<void(
 ...
 *
 */
using GetForwardingPrefExCb = std::function<void(std::vector<ForwardInfo> forwardInfoList,
 ...
 *
 */
using GetForwardingPrefCb = std::function<void(std::vector<ForwardInfo> forwardInfoList,
...
class ISuppServicesManager {
 public:
     ...
     *
     */
    virtual telux::common::Status setCallWaitingPref(
        SuppServicesStatus suppSvcStatus, SetSuppSvcPrefCallback callback = nullptr)
        = 0;
     ...
     *
     */
    virtual telux::common::Status setForwardingPref(
        ForwardReq forwardReq, SetSuppSvcPrefCallback callback = nullptr)
        = 0;
     ...
     *
     */
    virtual telux::common::Status requestForwardingPref(
     ...
     *
     */
    virtual telux::common::Status requestOirPref(ServiceClass serviceClass, GetOirPrefCb callback)
        = 0;
