...
template <typename T> class TCPServerWorker {
...
public:
  TCPServer(std::shared_ptr<TCPServerWorker<T>> worker, int serverPort,
            std::string strServerAddr, std::string serverIpInterface)
      : serverPort_(serverPort), worker_(worker), strServerAddr_(strServerAddr),
    ...
    do {
      ...
      if (!strServerInterface_.empty()) {
        if (setsockopt(socket_, SOL_SOCKET, SO_BINDTODEVICE,
                       strServerInterface_.c_str(),
                       strServerInterface_.size()) != 0) {
      ...
      case AF_INET:
        ...
        break;
      case AF_INET6:
        inet_ntop(AF_INET6, &(((struct sockaddr_in6 *)clientAddr)->sin6_addr),
                  addrbuf, 40);
      ...
      while (true) {
        ...
        if (FD_ISSET(socket_, &read_fds)) {
          ...
          ssize_t n =
              recv(socket_, static_cast<void *>(&msg), sizeof(msg) - 1, 0);
          std::cout << " length : " << n << " ";
          if (n <= 0) {
  ...
  void disconnect() {
    ...
    if (socket_ != -1) {
      if (shutdown(socket_, SHUT_RDWR) == -1) {
        std::cout << "client  shutdown  " << std::string(strerror(errno))
                  << std::endl;
      }
      if (close(socket_) == -1) {
    ...
    // Close exit pipe file descriptors
    if (exitPipe_[0] != -1) close(exitPipe_[0]);
    if (exitPipe_[1] != -1) close(exitPipe_[1]);
