...
int Hpcm::createTXPlayStream() {
    ...
    ec = p.get_future().get();
    if (ec != telux::common::ErrorCode::SUCCESS) {
        std::cout << "failed create tx playback stream, err " << static_cast<int>(ec) << std::endl;
        return -EIO;
    }
...
int Hpcm::deleteTXPlayStream() {
    ...
    status = audioManager_->deleteStream(
        txPlayStream_, [&p, this](telux::common::ErrorCode result) { p.set_value(result); });
    ...
    ec = p.get_future().get();
    if (ec != telux::common::ErrorCode::SUCCESS) {
        std::cout << "failed delete tx playback stream, err " << static_cast<int>(ec) << std::endl;
        return -EIO;
    }
...
int Hpcm::createTXCaptureStream() {
    ...
    ec = p.get_future().get();
    if (ec != telux::common::ErrorCode::SUCCESS) {
        std::cout << "failed create tx capture stream, err " << static_cast<int>(ec) << std::endl;
        return -EIO;
    }
...
int Hpcm::deleteTXCaptureStream() {
    ...
    status = audioManager_->deleteStream(
        txCaptureStream_, [&p, this](telux::common::ErrorCode result) { p.set_value(result); });
    ...
    ec = p.get_future().get();
    if (ec != telux::common::ErrorCode::SUCCESS) {
        std::cout << "failed delete tx capture stream, err " << static_cast<int>(ec) << std::endl;
        return -EIO;
    }
...
int Hpcm::createRXPlayStream() {
    ...
    ec = p.get_future().get();
    if (ec != telux::common::ErrorCode::SUCCESS) {
        std::cout << "failed create rx playback stream, err " << static_cast<int>(ec) << std::endl;
        return -EIO;
    }
...
int Hpcm::deleteRXPlayStream() {
    ...
    status = audioManager_->deleteStream(
        rxPlayStream_, [&p, this](telux::common::ErrorCode result) { p.set_value(result); });
    ...
    ec = p.get_future().get();
    if (ec != telux::common::ErrorCode::SUCCESS) {
        std::cout << "failed delete rx playback stream, err " << static_cast<int>(ec) << std::endl;
        return -EIO;
    }
...
int Hpcm::createRXCaptureStream() {
    ...
    ec = p.get_future().get();
    if (ec != telux::common::ErrorCode::SUCCESS) {
        std::cout << "failed create rx capture stream, err " << static_cast<int>(ec) << std::endl;
        return -EIO;
    }
...
int Hpcm::deleteRXCaptureStream() {
    ...
    status = audioManager_->deleteStream(
        rxCaptureStream_, [&p, this](telux::common::ErrorCode result) { p.set_value(result); });
    ...
    ec = p.get_future().get();
    if (ec != telux::common::ErrorCode::SUCCESS) {
        std::cout << "failed delete rx capture stream, err " << static_cast<int>(ec) << std::endl;
        return -EIO;
    }
...
void Hpcm::writeCompleteTX(std::shared_ptr<telux::audio::IStreamBuffer> buffer,
...
void Hpcm::readFromTXwriteOnTX() {
    ...
    auto txReadCompleteCb
        = std::bind(&Hpcm::readCompleteTX, this, std::placeholders::_1, std::placeholders::_2);

    auto txWriteCompleteCb = std::bind(&Hpcm::writeCompleteTX, this, std::placeholders::_1,
    ...
    while (keepRunning_) {
        if (txReadDone_ && txWritePossible_) {
             ...
             */
            std::memcpy(streamBuffer->getRawBuffer(), tmpBufferPtr->getRawBuffer(), txReadSize_);
        ...
        txReadWaiterCv_.wait(txReadLock, [this] {
            return ((txReadPossible_ || (txReadDone_ && txWritePossible_)) ? true : false);
        });
    }
...
void Hpcm::writeCompleteRX(std::shared_ptr<telux::audio::IStreamBuffer> buffer,
    uint32_t bytesWritten, telux::common::ErrorCode error) {
...
void Hpcm::readFromRXwriteOnRX() {
    ...
    auto rxReadCompleteCb
        = std::bind(&Hpcm::readCompleteRX, this, std::placeholders::_1, std::placeholders::_2);

    auto rxWriteCompleteCb = std::bind(&Hpcm::writeCompleteRX, this, std::placeholders::_1,
        std::placeholders::_2, std::placeholders::_3);
    ...
    while (keepRunning_) {
        if (rxReadDone_ && rxWritePossible_) {
             ...
             * modification. If required, an application can modify it and then write back.
             */
            std::memcpy(streamBuffer->getRawBuffer(), tmpBufferPtr->getRawBuffer(), rxReadSize_);
        ...
        if (rxReadPossible_) {
            ...
            status = rxCaptureStream_->read(streamBuffer, rxReadSize_, rxReadCompleteCb);
            if (status != telux::common::Status::SUCCESS) {
        ...
        rxReadWaiterCv_.wait(rxReadLock, [this] {
            return ((rxReadPossible_ || (rxReadDone_ && rxWritePossible_)) ? true : false);
        });
    }
